...
const uploadShowcaseImages = multer({
  limits: { fileSize: 20 * 1024 * 1024 } /* 20MB Security Limit */,
...
const uploadFolioImages = multer({
  limits: { fileSize: 20 * 1024 * 1024 } /* 20MB Security Limit */,
...
const uploadCopyPCBImages = multer({
  limits: { fileSize: 20 * 1024 * 1024 } /* 20MB Security Limit */,
...
const storageComponentImages = multer.diskStorage({
  destination(req, file, cb) {
    const dir = "componentImages/images/"; // ย้ายมาเก็บที่ root componentImages/images/ ตรงๆ
    ensureDirectoryExistence(dir);
    cb(null, dir);
  },
  filename(req, file, cb) {
...
const uploadComponentImages = multer({
  limits: { fileSize: 20 * 1024 * 1024 } /* 20MB Security Limit */,
...
const uploadPaymentSlipImages = multer({
  limits: { fileSize: 20 * 1024 * 1024 } /* 20MB Security Limit */,
