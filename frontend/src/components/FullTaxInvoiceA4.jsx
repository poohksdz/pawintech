...
const FullTaxInvoiceA4 = ({ order, companyInfo, printMode, isQuotation = false, isAdmin = false, docType, slotSignatures }) => {
    ...
    // Support both flat fields (CopyPCB, AssemblyPCB, CustomPCB) and nested billingAddress
    const billing = (() => {
    ...
    // 2. กำหนดประเภทเอกสาร — ถ้า docType ถูกส่งมาจากแอดมิน ใช้ค่านั้น, ถ้าไม่ ใช้ status อัตโนมัติ
    let effectiveDocType = docType; // docType prop: "quotation" | "proforma" | "taxinvoice" | undefined
    if (!effectiveDocType) {
    ...
    if (effectiveDocType === "quotation") {
        ...
        // Tax Invoice mode (Paid): 5 Pages
        invoicePages = [
            {
                titleTH: `ต้นฉบับ${tiTitleTH}`, titleEN: `ORIGINAL ${tiTitleEN}`,
                recipient: "สำหรับลูกค้า", docSet: "เอกสารออกเป็นชุด",
                colorBg: "bg-[#5F9EA0]", type: "taxinvoice"
            },
            {
                titleTH: `สำเนา${tiTitleTH}`, titleEN: `COPY ${tiTitleEN}`,
                recipient: "สำหรับบัญชี", docSet: "เอกสารออกเป็นชุด",
                colorBg: "bg-red-600", type: "taxinvoice"
            },
            {
                titleTH: `ต้นฉบับ${rcTitleTH}`, titleEN: `ORIGINAL ${rcTitleEN}`,
                recipient: "สำหรับลูกค้า", docSet: "เอกสารออกเป็นชุด",
                colorBg: "bg-orange-500", type: "receipt"
            },
            {
                titleTH: `สำเนา${rcTitleTH}`, titleEN: `COPY ${rcTitleEN}`,
                recipient: "สำหรับลูกค้า", docSet: "เอกสารออกเป็นชุด",
                colorBg: "bg-green-600", type: "receipt"
            },
            {
                titleTH: piTitleTH, titleEN: piTitleEN,
                recipient: "สำหรับลูกค้า", docSet: "ต้นฉบับ",
                colorBg: "bg-[#8B0000]", type: "proforma"
            },
        ];
    }
        ...
        <div className={`${printMode === "full" ? "print-area print-block" : printMode === "short" ? "print-area print-block" : "no-print block"}`}>
            {invoicePages.map((pageDef, pageIndex) => (
                <div key={pageIndex} className={`${pageIndex === invoicePages.length - 1 ? "last-print-page" : "break-after-page"} font-prompt text-black p-[10mm] bg-white w-[210mm] min-h-[296mm] h-[296mm] box-border overflow-hidden mx-auto text-[11px] leading-tight shadow-none border-none relative print:m-0 print:p-[10mm]`}>
                    ...
                    {/* แก้ไข: ปรับลด w-20 ออก และใช้ mr-2 (margin-right) เพื่อให้เส้นประชิดตัวหนังสือ */}
