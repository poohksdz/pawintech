...
const ProductCarousel = () => {
  ...
  useEffect(() => {
    ...
    const showcasesToShow = data.filter(showcase => showcase.present === "presentOne");
  ...
  if (isLoading) return <div className="animate-pulse w-full h-64 bg-slate-200 rounded-2xl mb-6"></div>;
            ...
            <img
              src={
                typeof showcase.image === 'string'
                  ? showcase.image
                  : (showcase.image?.image || showcase.image?.path || showcase.image?.url || '/images/sample.jpg')
              }
          ...
          <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-2">
            {showcasesToShow.map((_, idx) => (
              <button
                ...
                className={`w-2 h-2 rounded-full transition-all ${activeIndex === idx ? 'bg-white w-6' : 'bg-white/50 hover:bg-white/80'
                  }`}
