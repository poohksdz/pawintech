...
const ProductCarousel4 = () => {
  ...
  useEffect(() => {
    ...
    const showcasesToShow = data.filter(showcase => showcase.present === 'presentFour');
  ...
  // Filter for presentFour and sort by displayOrder
  const showcasesToShow = data
    ? data.filter(showcase => showcase.present === 'presentFour')
      .sort((a, b) => (a.displayOrder || 0) - (b.displayOrder || 0))
    : [];

  if (showcasesToShow.length === 0) return <Message variant="info">No showcases available.</Message>;
  ...
  return (
    <div className="relative w-full h-full m-0 p-0 overflow-hidden group bg-slate-100">
      <div
      ...
      >
        {showcasesToShow.map((showcase, index) => (
          <div
            ...
            style={{ cursor: showcase.navigateLink ? 'pointer' : 'default' }}
          >
            <img
              src={showcase.image}
