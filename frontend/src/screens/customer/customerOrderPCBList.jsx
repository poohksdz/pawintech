...
import { FaBoxOpen, FaSearch, FaChevronRight, FaTruck, FaMicrochip, FaRegClock, FaCopy } from 'react-icons/fa';
...
const CustomerOrderPCBList = () => {
  ...
  if (isLoading) return <div className="flex justify-center items-center min-h-[400px]"><Loader /></div>;
  ...
  if (!orders || orders.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-20 px-4 text-center bg-white rounded-[2rem] border border-slate-100 shadow-sm mt-8">
        ...
        <h3 className="text-xl font-black text-slate-800 tracking-tight mb-2">{translations.emptyState}</h3>
        <p className="text-sm font-medium text-slate-400">You haven't placed any product orders yet.</p>
      </div>
    );
  }
    ...
    <div className="animate-pageFade">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-black text-slate-800 tracking-tight m-0">{translations.productorders}</h1>
            <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mt-1">Total: {orders.length} Orders</p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-[2rem] border border-slate-100 shadow-sm overflow-hidden">
        <div className="hidden md:grid grid-cols-12 gap-4 items-center bg-slate-50/50 px-6 py-4 border-b border-slate-100 text-[10px] font-black uppercase tracking-widest text-slate-400">
          ...
          <div className="col-span-2 text-right">{translations.orderTotalLbl}</div>
          <div className="col-span-2 text-center">{translations.orderDeliveredLbl}</div>
        </div>
              ...
              >
                ...
                <div className="w-full md:hidden flex justify-between items-center mb-2">
                  <span className="font-mono text-sm font-bold text-slate-800">
                    {order.paymentComfirmID || order.id.substring(0, 8)}
                  </span>
                ...
                <div className="col-span-12 md:col-span-2 w-full md:w-auto flex justify-between md:justify-center items-center">
                  <span className="md:hidden text-xs font-bold uppercase tracking-widest text-slate-400">{translations.qty}</span>
                ...
                <div className="col-span-12 md:col-span-2 w-full md:w-auto flex justify-between md:right text-right items-center">
                  <span className="md:hidden text-xs font-bold uppercase tracking-widest text-slate-400">{translations.orderTotalLbl}</span>
                ...
                <div className="col-span-12 md:col-span-2 w-full md:w-auto flex justify-between md:justify-center items-center mt-2 md:mt-0 pt-4 md:pt-0 border-t md:border-0 border-slate-100">
                  <span className="md:hidden text-xs font-bold uppercase tracking-widest text-slate-400">{translations.orderDeliveredLbl}</span>
                    ...
                    {order.isDelivered && order.deliveryID && (
                      <button
                        onClick={(e) => {
                          ...
                          toast.success(language === 'thai' ? 'คัดลอกเลขพัสดุแล้ว!' : 'Tracking copied!');
                        }}
                      ...
                      >
                        <span className="text-[9px] font-bold text-slate-400 uppercase tracking-tight flex items-center gap-1">
                          ...
                          <FaCopy className="opacity-0 group-hover/trk:opacity-100 transition-opacity" size={8} />
                        </span>
                        <span className="font-mono text-xs font-black text-indigo-600">{order.deliveryID}</span>
                      </button>
                    )}
