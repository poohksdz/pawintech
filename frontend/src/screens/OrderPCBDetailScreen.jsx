...
const OrderPCBDetailScreen = () => {
    ...
    const { data: order, isLoading, error, refetch } = useGetOrderPCBByIdQuery(id);
    const [updateOrderPCB, { isLoading: isUpdating }] = useUpdateOrderPCBMutation();

    // Fetch related orders if payment ID exists
    const {
        data: orderSamepaymentID,
        isLoading: isLoadingSamepaymentID,
    ...
    if (isLoading || isLoadingSamepaymentID) return <div className="flex min-h-screen items-center justify-center bg-slate-50"><Loader /></div>;
    if (error) return <div className="mx-auto max-w-2xl p-8"><Message variant="danger">{error?.data?.message || 'Failed to load order'}</Message></div>;
    ...
    const financials = {
        subtotal: relatedOrders.reduce((s, i) => {
            const total = Number(i.quoted_price_to_customer || i.total_amount_cost || 0);
        ...
        total: relatedOrders.reduce((s, i) => s + Number(i.quoted_price_to_customer || i.total_amount_cost || 0), 0)
    };
    ...
    const handleApprove = async () => {
        try {
            ...
            toast.success(language === 'thai' ? 'อนุมัติเรียบร้อยแล้ว เพิ่มรายการลงตะกร้าแล้ว' : 'Approved! Item added to cart.');
                ...
                <div className="mb-10 border-b-2 border-slate-200 pb-8">
                    ...
                    <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
                        <div className="flex flex-col items-start gap-4 sm:flex-row sm:items-center sm:gap-6 min-w-0">
                            ...
                            <div className="min-w-0 flex-1">
                                ...
                                <div className="mt-2 flex flex-wrap items-center gap-2 text-sm font-medium text-slate-500 sm:gap-4">
                                    <span>Order <span className="font-mono font-bold text-slate-700">#{order.orderID}</span></span>
                                    <span className="hidden h-1 w-1 rounded-full bg-slate-300 sm:block" />
                                    <span>{t.placedOn}: <span className="font-semibold text-slate-700">{formatDateSafe(order.created_at)}</span></span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="flex shrink-0 items-center gap-2 overflow-x-auto rounded-2xl bg-white p-2 md:p-3 shadow-sm ring-1 ring-slate-200 hide-scrollbar">
                            <StatusBadge active={!isPaid} completed={isPaid} icon={<FaClock />} label={t.status.verify} />
                            <div className="h-px w-3 md:w-6 shrink-0 bg-slate-200" />
                            <StatusBadge active={isPaid && !isManufacturing} completed={isManufacturing} icon={<FaTools />} label={t.status.produce} />
                            <div className="h-px w-3 md:w-6 shrink-0 bg-slate-200" />
                            <StatusBadge active={isManufacturing && !isDelivered} completed={isDelivered} icon={<FaTruck />} label={t.status.deliver} />
                        </div>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 gap-8 lg:grid-cols-3">
                    ...
                    <div className="space-y-8 lg:col-span-2">
                        ...
                        <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
                            <div className="mb-6 flex items-center gap-3 border-b border-slate-100 pb-4">
                                ...
                                <h2 className="text-lg font-black uppercase tracking-wide text-slate-800">{t.specifications}</h2>
                            </div>

                            <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
                                <InfoBlock label={t.labels.size} value={`${order.length_cm} × ${order.width_cm} mm`} icon={<FaRulerCombined />} />
                                <InfoBlock label={t.labels.layers} value={`${order.layers} Layers`} icon={<FaLayerGroup />} />
                                <InfoBlock label={t.labels.thickness} value={`${order.thickness_mm} mm`} icon={<FaRulerCombined />} />
                                <InfoBlock label="Quantity" value={`${order.pcb_quantity} Units`} icon={<FaBoxOpen />} />

                                <InfoBlock label={t.labels.material} value={order.base_material} icon={<FaLayerGroup />} />
                                <InfoBlock label={t.labels.color} value={order.color} icon={<FaPalette />} />
                                <InfoBlock label={t.labels.finish} value={order.surface_finish} icon={<FaMicrochip />} />
                                <InfoBlock label={t.labels.copper} value={`${order.copper_weight_oz} oz`} icon={<FaMicrochip />} />
                            </div>
                                ...
                                >
                                    <div className="flex items-center gap-4 min-w-0">
                                        ...
                                        <div className="min-w-0 text-left">
                                            <div className="truncate font-bold text-slate-900 group-hover:text-blue-900">{t.gerber}</div>
                                            <div className="truncate text-xs font-medium text-slate-500">{getGerberFilename(order.gerberZip)}</div>
                                        </div>
                                    </div>
                                    <span className="hidden shrink-0 rounded-full bg-white px-6 py-2 text-[10px] font-black uppercase tracking-widest text-blue-600 shadow-sm transition-all group-hover:bg-blue-600 group-hover:text-white sm:block">
                                        Download Now
                                    </span>
                                </button>
                            )}
                        ...
                        <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
                            <div className="mb-6 flex items-center gap-3 border-b border-slate-100 pb-4">
                                ...
                                <h2 className="text-lg font-black uppercase tracking-wide text-slate-800">{t.fulfillment}</h2>
                            </div>
                            ...
                            <div className="grid grid-cols-1 gap-6 md:grid-cols-2 md:gap-8">
                                ...
                                <div className="flex flex-col h-full">
                                    <div className="mb-4 flex shrink-0 items-center gap-2 text-[10px] font-black uppercase tracking-widest text-slate-400">
                                        <FaMapMarkerAlt className="shrink-0" /> <span className="truncate">{t.shipping}</span>
                                    </div>

                                    {order.receivePlace === 'bysending' ? (
                                        <div className="flex flex-1 flex-col rounded-2xl bg-slate-50 p-5">
                                            <p className="mb-1 break-words text-base font-bold text-slate-900">{order.shippingName}</p>
                                            <p className="mb-2 break-words text-sm font-medium text-blue-600">{order.shippingPhone}</p>
                                            ...
                                            {isDelivered && order.deliveryID && (
                                                <div className="mt-auto pt-4">
                                                    <div className="flex items-center justify-between gap-2 rounded-xl bg-slate-900 px-4 py-3 text-white">
                                                        <span className="shrink-0 text-[10px] font-black uppercase tracking-widest text-slate-400">{t.labels.tracking}</span>
                                                        <span className="truncate font-mono text-xs font-bold text-emerald-400">{order.deliveryID}</span>
                                                    </div>
                                                </div>
                                            )}
                                        </div>
                                    ) : (
                                        <div className="flex min-h-[160px] flex-1 flex-col items-center justify-center rounded-2xl border-2 border-dashed border-slate-200 bg-slate-50 p-6 text-center">
                                            <FaBoxOpen className="mb-3 shrink-0 text-slate-300" size={32} />
                                            <span className="text-xs font-bold uppercase tracking-widest text-slate-400">Warehouse Pickup</span>
                                        </div>
                                    )}
                                </div>
                                ...
                                <div className="flex flex-col h-full">
                                    <div className="mb-4 flex shrink-0 items-center gap-2 text-[10px] font-black uppercase tracking-widest text-slate-400">
                                        <FaFileInvoice className="shrink-0" /> <span className="truncate">{t.billing}</span>
                                    </div>
                                    <div className="flex flex-1 flex-col rounded-2xl bg-slate-50 p-5">
                                        <p className="mb-1 break-words text-base font-bold text-slate-900">{order.billingName}</p>
                                        <p className="mb-3 break-words text-sm leading-relaxed text-slate-600">{order.billinggAddress}</p>
                                        {order.billingTax && (
                    ...
                    <div className="lg:col-span-1">
                        <div className="sticky top-10 space-y-6">
                            ...
                            {/* Financial Dashboard */}
                            <div className="overflow-hidden rounded-3xl bg-slate-900 p-6 shadow-xl sm:p-8">
                                <div className="mb-6 flex items-center gap-3 border-b border-white/10 pb-4">
                                    <FaMoneyBillWave className="shrink-0 text-emerald-400" size={20} />
                                    <h2 className="text-sm font-black uppercase tracking-wide text-white">{t.summary}</h2>
                                </div>

                                {/* Order Lines */}
                                <div className="mb-8 space-y-4">
                                    {relatedOrders.map((item, idx) => {
                                        const total = Number(item.quoted_price_to_customer || item.total_amount_cost || 0);
                                        const ems = Number(item.ems || 0);
                                        const vat = Number(item.vatPrice || 0);
                                        const itemCost = item.pcb_cost ? Number(item.pcb_cost) : (total - ems - vat);

                                        return (
                                            <div key={idx} className="flex items-center justify-between gap-4 text-sm">
                                                <div className="flex min-w-0 flex-col">
                                                    <span className="truncate font-bold text-slate-200" title={item.projectname}>{item.projectname}</span>
                                                    <div className="flex items-center gap-2">
                                                        <span className="text-[10px] uppercase tracking-widest text-slate-500">Qty: {item.pcb_quantity}</span>
                                                        {item.gerberZip && (
                                                            <button
                                                                onClick={() => handleGerberDownload(item.gerberZip)}
                                                ...
                                                <span className="shrink-0 font-mono text-slate-300">฿{itemCost.toLocaleString()}</span>
                                            </div>
                                        );
                                    })}
                                </div>
                                ...
                                <div className="mb-8 space-y-3 border-t border-white/10 pt-6">
                                    <div className="flex justify-between gap-4 text-sm text-slate-400">
                                        <span className="shrink-0 text-[10px] font-bold uppercase tracking-widest">{t.labels.shipping}</span>
                                        <span className="shrink-0 font-mono text-slate-300">฿{financials.shipping.toLocaleString()}</span>
                                    </div>
                                    <div className="flex justify-between gap-4 text-sm text-emerald-400/80">
                                        <span className="shrink-0 text-[10px] font-bold uppercase tracking-widest">{t.labels.vat}</span>
                                        <span className="shrink-0 font-mono text-emerald-400">฿{financials.tax.toLocaleString()}</span>
                                    </div>
                                </div>

                                <div className="rounded-2xl bg-white/5 p-4 text-center">
                                    <span className="mb-1 block text-[10px] font-black uppercase tracking-widest text-slate-400">{t.labels.total}</span>
                                    <span className="break-words font-display text-3xl font-black text-white sm:text-4xl">฿{financials.total.toLocaleString()}</span>
                                </div>
                                        ...
                                        >
                                            {isUpdating ? (
                                                ...
                                                <>
                                                    ...
                                                    <span>{language === 'thai' ? 'อนุมัติ & รับออเดอร์' : 'Approve & Order'}</span>
                                                </>
                                            )}
                                        </button>
                                ...
                                {order.paymentSlip ? (
                                    <div className="relative h-64 overflow-hidden rounded-[1.5rem]">
                                        ...
                                        <div className="absolute inset-0 flex flex-col items-center justify-center bg-slate-900/40 opacity-0 backdrop-blur-sm transition-all duration-300 group-hover:opacity-100">
                                            ...
                                            <span className="text-[10px] font-bold uppercase tracking-widest text-white">View Receipt</span>
                                        </div>
                                    </div>
                                ) : (
                                    <div className="flex h-64 flex-col items-center justify-center gap-4 text-center p-4">
                                        ...
                                        <span className="text-[10px] font-bold uppercase tracking-widest text-slate-400">Awaiting Payment Confirm</span>
                                    </div>
                                )}
                            </div>
                            ...
                            >
                                ...
                                >
                                    <img
                                        ...
                                        alt="Receipt Zoom"
                                    />
                                    <div className="pointer-events-none absolute bottom-6 left-1/2 -translate-x-1/2 whitespace-nowrap rounded-full bg-white/10 px-6 py-2 text-[10px] font-bold uppercase tracking-widest text-white backdrop-blur-md">
...
const StatusBadge = ({ active, completed, icon, label }) => (
    <div className={`flex shrink-0 items-center gap-1.5 md:gap-2 rounded-xl px-2 py-1.5 md:px-3 md:py-2 transition-colors whitespace-nowrap ${completed ? 'bg-emerald-50 text-emerald-700' :
        active ? 'bg-blue-50 text-blue-700 ring-1 ring-blue-200' :
            'text-slate-400 opacity-50'
        }`}>
        {completed ? <FaCheckCircle className="shrink-0" /> : <span className="shrink-0">{icon}</span>}
        <span className="truncate text-[9px] md:text-[10px] font-bold uppercase tracking-widest">{label}</span>
    </div>
);
...
const InfoBlock = ({ label, value, icon }) => (
    <div className="flex min-w-0 flex-col rounded-2xl border border-slate-100 bg-slate-50/50 p-4">
        ...
        <div className="truncate font-semibold text-slate-900" title={value}>{value || '-'}</div>
    </div>
);
