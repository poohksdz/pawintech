...
import { Table, Button, Modal, Form, Col, Row, InputGroup, Tabs, Tab, FormControl } from 'react-bootstrap';
import { FaSearch } from 'react-icons/fa';
import { useGetStockRequestUserQuery } from '../../../slices/stockRequestApiSlice';
...
const StockUserRequestDashboardScreen = () => {
  ...
  const { data, isLoading, error } = useGetStockRequestUserQuery(); 
    ...
    const products = data?.requestgoods || [];
...
const addToStockIssueHandler = (product) => {
  try {
    ...
    toast.success('Component added to issue cart');
  } catch (error) {
    console.error('Failed to add to stock issue:', error);
    toast.error('Failed to add component to stock issue');
  }
};
    ...
    <Row className="align-items-center justify-content-between mb-3">
      <Col md="6">
        <div className="d-flex justify-content-between align-items-center"> 
          <h1 className="mb-0">Requested Components</h1>
        </div>
      </Col> 
    ...
    <div style={containerStyle}>
     ...
     <thead>
        <tr>
          ...
          <th className="py-2 mx-2">FROM</th>
          <th className="py-2 mx-2">ISSUE</th>
        ...
        <tbody>
          {filteredProducts.map((p, index) => (
            <tr key={p.ID}>
              ...
              <td className="wrap-text" title={p.requestedUser}>{p.requestedUser}</td>
               <td>
                 <Button
                  variant="primary"
...
<td className="wrap-text">{p.note}</td>
                ...
                <td>
                  <Button
                    ...
                    onClick={() => navigate(`/componentrequestlist/${p.ID}`)}
                  >
                    More
                  </Button>
                </td> 
