...
import { useGetStockFootprintDetailsQuery, useUpdateStockFootprintMutation } from '../../../slices/stockFootprintApiSlice';
...
const StockEditFootprintScreen = () => {
  ...
  const { data: footprint, isLoading, error } = useGetStockFootprintDetailsQuery(id);
  const { data: categories = [], isLoading: isLoadingCategories } = useGetStockCategoriesQuery();

  const [updateStockFootprint, { isLoading: isUpdating }] = useUpdateStockFootprintMutation();
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      ...
      toast.error(err?.data?.message || err.error || 'Failed to update footprint');
    }
  };

  if (isLoading) return <div className="min-h-screen flex items-center justify-center bg-slate-50"><Loader /></div>;
  if (error) return <div className="max-w-3xl mx-auto py-10 px-4"><Message variant="danger">{error?.data?.message || error.error}</Message></div>;
        ...
        <CardBody>
          <form onSubmit={handleUpdate} className="space-y-6">
            ...
            <div className="pt-6 flex items-center justify-end gap-4 border-t border-slate-100">
              ...
              >
                {isUpdating ? <><FaSyncAlt size={14} className="animate-spin" /> Updating...</> : <><FaSave size={16} /> Update Footprint</>}
              </button>
            </div>
          </form>
        </CardBody>
