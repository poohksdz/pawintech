...
import {
  Table, Button, Modal, Form, Col, Row, InputGroup, Badge
  ...
  return (
    <>
      <Row className="align-items-center justify-content-between mb-3 mt-3">
        <Col md="6"><h3>All Requested Importance</h3></Col>
        <Col md="4">
          <InputGroup>
            <Form.Control 
        ...
        <Table striped bordered hover className="table-sm align-middle">
          <thead className="bg-light sticky-top">
            <tr className="text-center">
              <th>#</th><th>Image</th><th>Part Info</th><th>Quantities</th><th>Status</th><th>Actions</th>
            </tr>
          </thead>
          <tbody>
            {filteredProducts.map((p, index) => (
              <tr key={p.ID}>
                ...
                <td className="text-center">
                    {p.img ? <img src={`/componentImages${p.img}`} width="40" alt="p" style={{objectFit: 'contain'}}/> : '-'}
                </td>
                <td>
                ...
                <td className="text-center">{p.process ? <Badge bg="info">{p.process}</Badge> : '-'}</td>
                <td className="text-center">
                  <Button variant="outline-info" size="sm" className="me-1" onClick={() => openDetailModal(p)}>
                    <FaInfoCircle />
                  </Button>
                  <Button variant="outline-danger" size="sm" onClick={() => openCancelModal(p)}>
                    <FaTimesCircle />
                  </Button>
                </td>
              </tr>
      ...
      <Modal show={showDetailModal} onHide={() => setShowDetailModal(false)} size="lg" centered>
        ...
        <Modal.Body>
          {selectedDetailItem && (
            <Row>
                <Col md={6}><strong>P/N:</strong> {selectedDetailItem.electotronixPN}</Col>
                <Col md={6}><strong>Category:</strong> {selectedDetailItem.category}</Col>
                <Col md={6} className="mt-2"><strong>Manufacturer:</strong> {selectedDetailItem.manufacture}</Col>
                <Col md={12} className="mt-3"><strong>Description:</strong> <p>{selectedDetailItem.description || '-'}</p></Col>
            </Row>
          )}
        </Modal.Body>
      ...
      <Modal show={showCancelModal} onHide={() => setShowCancelModal(false)} centered>
        ...
        <Modal.Footer>
          ...
          <Button variant="danger" onClick={handleConfirmCancel}>Confirm Cancel</Button>
        </Modal.Footer>
      </Modal>
    </>
  );
