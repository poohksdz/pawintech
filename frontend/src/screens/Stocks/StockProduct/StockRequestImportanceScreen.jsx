...
import {
  Table, Button, Modal, Form, Col, Row, InputGroup, Card, Badge
...
const StockRequestImportanceScreen = () => {
  ...
  const [selectedProductForCancel, setSelectedProductForCancel] = useState(null);
  ...
  return (
    <>
      ...
      <div className="row g-2 mb-3">
        ...
        <div className="col">
          <select className="form-select form-select-sm" name="footprint" value={formData.footprint} onChange={handleChange}>
            <option value="">All Footprint</option>
            {footprintData.map(fp => <option key={fp.ID} value={fp.namefootprint}>{fp.namefootprint}</option>)}
          </select>
        </div>
        <div className="col">
          <select className="form-select form-select-sm" name="manufacturer" value={formData.manufacturer} onChange={handleChange}>
            <option value="">All Manufacturer</option>
            {manufactureData.map(mfg => <option key={mfg.ID} value={mfg.namemanufacture}>{mfg.namemanufacture}</option>)}
          </select>
        </div>
        <div className="col">
          <select className="form-select form-select-sm" name="supplier" value={formData.supplier} onChange={handleChange}>
      ...
      <div style={containerStyle}>
        <Table striped bordered hover className="table-sm center-table sticky-header align-middle">
          ...
          <tbody>
            {filteredProducts.map((p, index) => {
              const currentStock = existingQtyData?.products?.find(prod => prod.electotronixPN === p.electotronixPN)?.quantity || 0;
              ...
              return (
                <tr key={p.ID}>
                  ...
                  <td className="text-center">
                    {p.img ? (
                      <img src={`/componentImages${p.img}`} alt="prod" width="50" height="50" style={{ objectFit: 'contain' }} />
                    ) : (
                      <span className="text-muted small">No Img</span>
                    )}
                  </td>
                  <td>
                    <div><strong>No:</strong> {p.requestno}</div>
                    <div className="small text-muted">{p.requestdate} {p.requesttime}</div>
                    <div className="small text-primary"><strong>By:</strong> {p.requestedUser}</div>
                  </td>
                  <td>
                    <div className="fw-bold">{p.electotronixPN}</div>
                    <div className="small">Mfr: {p.manufacture}</div>
                    <div className="small text-muted">Mfr P/N: {p.manufacturePN}</div>
                  </td>
                  <td className="text-center">
                  ...
                  <td className="text-center">
                    {p.process ? <Badge bg="info">{p.process}</Badge> : <span className="text-muted">-</span>}
                  </td>
                  <td className="text-center">
      ...
      <Modal show={showDetailModal} onHide={closeDetailModal} size="lg" centered>
        <Modal.Header closeButton className="bg-light">
          <Modal.Title>
            Product Details <span className="text-primary fs-6">({selectedDetailItem?.electotronixPN})</span>
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
          {selectedDetailItem && (
            <Form>
              <h6 className="border-bottom pb-2 mb-3 text-muted">Technical Specifications</h6>
              <Row className="mb-3">
                <Col md={3}><strong>Category:</strong></Col>
                <Col md={3}>{selectedDetailItem.category}</Col>
                <Col md={3}><strong>Subcategory:</strong></Col>
                <Col md={3}>{selectedDetailItem.subcategory}</Col>
              </Row>
              <Row className="mb-3">
                <Col md={3}><strong>Value:</strong></Col>
                <Col md={3}>{selectedDetailItem.value}</Col>
                <Col md={3}><strong>Footprint:</strong></Col>
                <Col md={3}>{selectedDetailItem.footprint}</Col>
              </Row>
              <Row className="mb-3">
                <Col md={3}><strong>Position:</strong></Col>
                <Col md={3}>{selectedDetailItem.position}</Col>
                <Col md={3}><strong>Weight:</strong></Col>
                <Col md={3}>{selectedDetailItem.weight}</Col>
              </Row>

              <h6 className="border-bottom pb-2 mb-3 mt-4 text-muted">Supply Chain</h6>
              <Row className="mb-3">
                <Col md={3}><strong>Supplier:</strong></Col>
                <Col md={3}>{selectedDetailItem.supplier}</Col>
                <Col md={3}><strong>Supplier P/N:</strong></Col>
                <Col md={3}>{selectedDetailItem.supplierPN}</Col>
              </Row>
              <Row className="mb-3">
                <Col md={3}><strong>Price:</strong></Col>
                <Col md={3}>{selectedDetailItem.unitprice}</Col>
                <Col md={3}><strong>MOQ / SPQ:</strong></Col>
                <Col md={3}>{selectedDetailItem.moq} / {selectedDetailItem.spq}</Col>
              </Row>
              <Row className="mb-3">
                <Col md={3}><strong>Link:</strong></Col>
                <Col md={9}>
                    {selectedDetailItem.link ? <a href={selectedDetailItem.link} target="_blank" rel="noopener noreferrer">Product Link</a> : '-'}
                </Col>
              </Row>
              <Row className="mb-3">
                <Col md={3}><strong>Alternative:</strong></Col>
                <Col md={9}>{selectedDetailItem.alternative || '-'}</Col>
              </Row>

              <h6 className="border-bottom pb-2 mb-3 mt-4 text-muted">Additional Information</h6>
              <Row className="mb-3">
                <Col md={12}>
                  <strong>Description:</strong>
                  <p className="text-muted small mt-1">{selectedDetailItem.description || 'No description available.'}</p>
                </Col>
              </Row>
              <Row className="mb-3">
                <Col md={12}>
      ...
      <Modal show={showCancelModal} onHide={closeCancelModal} centered>
        ...
        <Modal.Body>
          ...
          <Form.Group controlId="cancelMessage">
            <Form.Label className="fw-bold">Reason for Cancellation <span className="text-danger">*</span></Form.Label>
            <Form.Control
              as="textarea"
              rows={3}
              value={cancelMessage}
