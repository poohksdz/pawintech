...
const StockRequestDashboardScreen = () => {
  ...
  const t = {
    en: {
      ...
      noRecordsDesc:
        "We couldn't find any pending requests matching your current filters.",
      headers: {
        reqInfo: "Req Info",
        requester: "Requester",
        component: "Component",
    ...
    thai: {
      ...
      noRecordsDesc:
        "ไม่พบรายการเบิกพัสดุที่รอการอนุมัติที่ตรงกับตัวกรองของคุณ",
      headers: {
  ...
  const filteredProducts = useMemo(() => {
    ...
    return filtered.sort(
      (a, b) =>
        moment(b.requestdate).valueOf() - moment(a.requestdate).valueOf(),
    );
  ...
  const stats = useMemo(
    () => ({
      ...
      critical: (existingQtyData?.products || []).filter(
        (p) => p.quantity <= (p.minStock || 5),
      ).length,
    }),
          ...
          <div className="hidden lg:flex bg-slate-900 rounded-2xl p-4 md:p-6 shadow-md items-center justify-between relative overflow-hidden">
            ...
            <div className="relative z-10 text-white">
              ...
              <h3 className="text-lg font-bold flex items-center gap-2">
                <FaArrowRight className="text-indigo-400" size={16} />{" "}
                {t.adminPanel}
              </h3>
            </div>
          </div>
                      ...
                      {filteredProducts.map((p, idx) => {
                        const stock =
                          existingQtyData?.products?.find(
                            (e) => e.electotronixPN === p.electotronixPN,
                          )?.quantity || 0;
                    ...
                    return (
                      ...
                      >
                        <div className="flex justify-between items-start mb-4">
                          ...
                          <div
                            className={`px-3 py-1 rounded-xl text-[11px] font-bold ${isEnough ? "bg-emerald-50 text-emerald-600" : "bg-rose-50 text-rose-600"}`}
                          >
                            {p.requestqty} Req
                          </div>
                        </div>
                        ...
                        <div className="flex items-center gap-4 bg-slate-50 p-3 rounded-2xl mb-4 border border-slate-100">
                          ...
                          <div className="min-w-0">
                            ...
                            <div className="text-xs text-slate-500 font-medium flex items-center gap-1.5 mt-1">
                              <FaUser className="text-slate-400" size={10} />{" "}
                              {p.requestedUser}
                            </div>
                ...
                >
                  ...
                  <div className="flex gap-3">
                    ...
                    <button
                      ...
                      className="flex-1 py-4 bg-slate-900 text-white font-bold rounded-2xl hover:bg-slate-800 transition-colors uppercase text-[11px] tracking-widest shadow-lg shadow-slate-200"
                    >
                      {t.modal.saveChanges}
                    </button>
                  </div>
