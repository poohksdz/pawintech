...
const StockUserRequestDashboardScreen = () => {
  ...
  const {
    ...
    isLoading,
    error,
  } = useGetStockRequestUserQuery(userInfo._id);
  const [updateStockRequestCancel, { isLoading: isCancelling }] =
    useUpdateStockRequestCancelMutation();
  ...
  const t = {
    en: {
      ...
      noRecordsDesc:
        "We couldn't find any component requests matching your search criteria.",
  ...
  const handleCopy = (e, text) => {
    ...
    if (text) {
      ...
      toast.success(`${t.toast.copied} ${text}`, {
        icon: "📋",
  ...
  const handleCancel = async () => {
    ...
    try {
      ...
      toast.error(
        err?.data?.message || err.error || "Failed to cancel request",
      );
    }
  };
  ...
  const thisWeekCount = products.filter(
    (r) => moment().diff(moment(r.requestdate), "days") <= 7,
  ).length;
                    ...
                    <table className="w-full text-left border-collapse">
                      <thead>
                        <tr className="border-b border-slate-100 text-[10px] font-black text-slate-400 uppercase tracking-widest bg-slate-50/50">
                          <th className="py-4 pl-6 pr-4 w-48">
                            {t.headers.idDate}
                          </th>
                          ...
                          <th className="py-4 pr-6 pl-4 w-24 text-center">
                            {t.headers.action}
                          </th>
                        </tr>
                      </thead>
                          ...
                          >
                            ...
                            <td className="py-4 pl-6 pr-4 align-middle">
                              <div className="flex flex-col gap-1.5">
                                ...
                                >
                                  <FaHashtag
                                    className="text-slate-400 group-hover/ref:text-indigo-400"
                                    size={10}
                                  />
                                  {p.requestno}
                                  <FaRegCopy
                                    className="opacity-0 group-hover/ref:opacity-100 transition-opacity ml-1"
                                    size={10}
                                  />
                                </div>
                            ...
                            <td className="py-4 px-4 align-middle">
                              {p.note && p.note.trim() !== "" ? (
                                <div className="flex items-start gap-2 bg-amber-50/80 p-2.5 rounded-xl border border-amber-100/50">
                                  ...
                                  />
                                  <span
                                    className="text-[11px] font-medium text-slate-600 leading-snug line-clamp-2"
                                    title={p.note}
                                  >
                                    {p.note}
                                  </span>
