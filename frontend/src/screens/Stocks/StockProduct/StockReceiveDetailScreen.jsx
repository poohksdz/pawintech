...
// FIX: เปลี่ยนชื่อ Hook ให้ตรงกับที่ export ใน slice
...
const StockReceiveDetailScreen = () => {
  const { id } = useParams();
  const navigate = useNavigate();

  // FIX: ใช้ชื่อ Hook ที่ถูกต้อง
  const {
    data: product,
    isLoading,
    isError,
  } = useGetStockRequestDetailsQuery(id);

  if (isLoading) return <Loader />;
  if (isError)
    return <Message variant="danger">Error loading request details</Message>;
    ...
    <Container fluid className="stock-detail-page">
      ...
      <div className="page-header">
        <Button
          ...
          onClick={() => navigate("/componentreceivelist")}
        >
          <div className="icon-circle">
            <FaArrowLeft size={12} />
          </div>
          <span>Back to Receive List</span>
        </Button>
      </div>
              ...
              <div className="hero-top">
                <div className="flex-grow-1">
                  ...
                  <h1 className="hero-mpn">
                    <CopyItem
                      text={product.electotronixPN || product.manufacturePN}
                      label="MPN"
                    />
                  </h1>
                ...
                {/* Request Quantity Box (Blue Theme) */}
      ...
      <Row className="g-4">
        ...
        <Col lg={4}>
          <Card className="info-card h-100">
            <Card.Header className="info-header">
              <div className="header-icon bg-primary-subtle text-primary">
                <FaClipboardList />
              </div>
            ...
            <Card.Body className="p-4 pt-2">
              ...
              <DetailRow
                icon={<FaCalendarAlt />}
                label="Request Date"
                value={product.requestdate}
              />
              <DetailRow
                icon={<FaClock />}
                label="Request Time"
                value={product.requesttime}
              />
        ...
        <Col lg={4}>
          <Card className="info-card h-100">
            <Card.Header className="info-header">
              <div className="header-icon bg-cyan-subtle text-info">
                <FaLayerGroup />
              </div>
...
export default StockReceiveDetailScreen;
