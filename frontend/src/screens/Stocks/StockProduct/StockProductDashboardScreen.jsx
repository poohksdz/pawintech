...
import {
    ...
    FaBoxOpen, FaChevronDown, FaPlus, FaStar, FaRegStar
} from 'react-icons/fa';
import { MdNotifications } from 'react-icons/md';
import { useGetStockProductsQuery, useToggleStarProductMutation } from '../../../slices/stockProductApiSlice';
...
const StockProductDashboardScreen = () => {
    ...
    const [sendTestNotification, { isLoading: isTestingNoti }] = useSendTestNotificationMutation();
    ...
    const [formData, setFormData] = useState({
        category: '', subcategory: '', manufacturer: '', footprint: '', supplier: '', sortBy: ''
    });
    ...
    const t = {
        en: {
            ...
            noDescription: 'Detailed component specifications not available at this time.',
    ...
    useEffect(() => { if (products) setLocalProducts(products); }, [products]);
    ...
    useEffect(() => {
        ...
        if (formData.sortBy === 'star-asc') {
            filtered = [...filtered].sort((a, b) => (a.starRating || 0) - (b.starRating || 0));
        } else if (formData.sortBy === 'star-desc') {
            filtered = [...filtered].sort((a, b) => (b.starRating || 0) - (a.starRating || 0));
    ...
    const handleTriggerSearch = () => { setSearchQuery(searchInput); setShowMobileFilter(false); };
    const handleKeyDown = (e) => { if (e.key === 'Enter') handleTriggerSearch(); };
    const handleChange = (e) => setFormData(prev => ({ ...prev, [e.target.name]: e.target.value }));
    const handleReset = () => {
        setSearchInput(''); setSearchQuery('');
        setFormData({ category: '', subcategory: '', manufacturer: '', footprint: '', supplier: '', sortBy: '' });
    };

    const addToStockCartHandler = (product, qty) => {
        if (!qty || Number(qty) < 1) { toast.error('กรุณาระบุจำนวนให้ถูกต้อง'); return; }
        if (Number(qty) > product.quantity) { toast.error('จำนวนที่เบิกเกินกว่าสต็อกที่มีอยู่'); return; }

        dispatch(addToStockCart({ ...product, _id: String(product.ID), reqqty: parseInt(qty), important: !!product.important, note: '' }));
    ...
    const handleImportantClick = (e, product) => {
        e.stopPropagation();
        setCurrentProduct(product); setIsUnmarking(!!product.important); setShowImportantModal(true);
    };

    const handleUpdateImportant = (productWithReason) => {
        setLocalProducts(prev => prev.map(p => p.ID === productWithReason.ID ? { ...p, important: !p.important, reason: productWithReason.reason } : p));
        toast.success(productWithReason.important ? 'ยกเลิกการทำเครื่องหมายสำคัญแล้ว' : 'ทำเครื่องหมายสำคัญสำเร็จ');
    };
    ...
    const handleStarToggle = async (e, product) => {
        ...
        try {
            ...
            toast.success(product.starRating > 0 ? 'ยกเลิกการติดดาวสำเร็จ' : 'ติดดาว 5 ดาวสำเร็จ');
            refetch();
        } catch (err) {
    ...
    if (error) return <div className="p-6 sm:p-10"><Message variant="danger">{error?.data?.message || error.error || 'เกิดข้อผิดพลาดในการโหลดข้อมูล'}</Message></div>;
        ...
        <React.Fragment>
                        ...
                        <aside className="w-64 shrink-0 hidden lg:block">
                            <div className="sticky top-8">
                                <div className="flex justify-between items-center mb-6">
                                    <h3 className="text-sm font-semibold text-slate-900">{t.filters}</h3>
                                    <button onClick={handleReset} className="text-[11px] font-medium text-slate-500 hover:text-slate-900 transition-colors">{t.clearAll}</button>
                                </div>

                                <div className="space-y-1">
                                    <CustomDropdown label={t.categoryLabel} name="category" value={formData.category} options={categoryData?.map(c => ({ key: c.ID, value: c.category, label: c.category }))} placeholder={t.anyCategory} onChange={handleChange} />
                                    <CustomDropdown label={t.subcategoryLabel} name="subcategory" value={formData.subcategory} options={subcategoryData?.filter(s => formData.category && s.category === formData.category).map(s => ({ key: s.ID, value: s.subcategory, label: s.subcategory }))} disabled={!formData.category} placeholder={t.anySubcategory} onChange={handleChange} />
                                    <CustomDropdown label={t.manufacturerLabel} name="manufacturer" value={formData.manufacturer} options={manufactureData?.map(m => ({ key: m.ID, value: m.namemanufacture, label: m.namemanufacture }))} placeholder={t.anyManufacturer} onChange={handleChange} />
                                    <CustomDropdown label={t.footprintLabel} name="footprint" value={formData.footprint} options={footprintData?.map(fp => ({ key: fp.ID, value: fp.namefootprint, label: fp.namefootprint }))} placeholder={t.anyFootprint} onChange={handleChange} />
                                    <CustomDropdown label={t.supplierLabel} name="supplier" value={formData.supplier} options={supplierData?.map(s => ({ key: s.ID, value: s.namesupplier, label: s.namesupplier }))} placeholder={t.anySupplier} onChange={handleChange} />
                                    <CustomDropdown
                                        label={t.sortBy}
                                        name="sortBy"
                                        value={formData.sortBy}
                                        options={[
                                            { key: 'star-asc', value: 'star-asc', label: t.sortStarsLow },
                                            { key: 'star-desc', value: 'star-desc', label: t.sortStarsHigh },
                                ...
                                {userInfo?.isAdmin && (
                                    <div className="mt-10 pt-6 border-t border-slate-100">
                                        <h4 className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-4">{t.devUtils}</h4>
                                        <button
                        ...
                        <main className="flex-1 min-w-0">
                            <AnimatePresence mode='wait'>
                                {currentItems.length === 0 ? (
                                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="py-32 flex flex-col items-center justify-center text-center border border-dashed border-slate-200 rounded-3xl bg-slate-50/50">
                                        ...
                                        <h3 className="text-lg font-semibold text-slate-800">{t.noComponents}</h3>
                                        <p className="text-slate-500 text-sm mt-1">{t.adjustFilters}</p>
                                        <button onClick={handleReset} className="mt-6 text-sm font-medium text-slate-900 border-b border-slate-900 pb-0.5 hover:text-slate-600 hover:border-slate-600 transition-colors">{t.clearFilters}</button>
                                    </motion.div>
                                ) : (
                                            ...
                                            >
                                                ...
                                                >
                                                    <div className="absolute top-3 right-3 z-10 flex flex-col gap-2">
                                                        ...
                                                        >
                                                            {p.important ? <FaExclamationCircle size={14} /> : <FaCheckCircle size={14} />}
                                                        </button>

                                                        {(userInfo?.role === 'admin' || userInfo?.role === 'stock') && (
                                                            <button
                                                            ...
                                                            >
                                                                {p.isStarred ? <FaStar size={14} /> : <FaRegStar size={14} />}
                                                            </button>
                                                        )}
                                                    </div>
                                                ...
                                                <div className="flex flex-col flex-1">
                                                    <div className="flex justify-between items-center mb-1.5">
                                                        <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider truncate pr-2">{p.category}</p>
                                                        <span className="text-[10px] text-slate-300 font-medium">#{p.ID}</span>
                                                    </div>

                                                    <h3
                                                        className="text-sm sm:text-base font-bold text-slate-900 leading-tight mb-1.5 h-10 sm:h-12 line-clamp-2 cursor-pointer hover:text-slate-600 transition-colors"
                                                        onClick={() => handleViewDetail(p)}
                                                    ...
                                                    <p className="text-[11px] text-slate-500 line-clamp-2 h-8 mb-3 leading-relaxed" title={p.description || p.manufacture}>
                                                        {p.description || p.manufacture}
                                                    </p>
                                                    ...
                                                    <div className="mt-auto flex flex-col pt-4 border-t border-slate-100 gap-3">
                                                        <div className="flex justify-between items-end">
                                                            <div>
                                                                <p className="text-[10px] text-slate-400 font-bold uppercase tracking-wider mb-0.5">{t.stock}</p>
                                                                <p className={`text-sm font-bold ${p.quantity > 0 ? 'text-slate-900' : 'text-red-500'}`}>
                                                            ...
                                                            <div className="flex items-center gap-1.5">
                                                                <input
                                                                    ...
                                                                    max={p.quantity}
                                                                    value={reqqty[p.ID] || ''}
                                                                    onChange={(e) => setReqqty(prev => ({ ...prev, [p.ID]: e.target.value }))}
                            ...
                            {totalPages > 1 && (
                                <div className="mt-12 flex items-center justify-center gap-2">
                                    <button disabled={currentPage === 1} onClick={() => setCurrentPage(c => c - 1)} className="w-8 h-8 flex items-center justify-center rounded-full text-slate-500 hover:bg-slate-100 disabled:opacity-30 disabled:hover:bg-transparent transition-colors">
                                        <FaChevronLeft size={10} />
                                    </button>
                                    <div className="flex items-center gap-1">
                                        {Array.from({ length: totalPages }, (_, i) => i + 1).map(pageNum => {
                                            if (totalPages > 5 && (pageNum < currentPage - 1 || pageNum > currentPage + 1) && pageNum !== 1 && pageNum !== totalPages) {
                                                if (pageNum === currentPage - 2 || pageNum === currentPage + 2) return <span key={pageNum} className="text-slate-400 text-xs px-1">...</span>;
                                                return null;
                                            }
                                            return (
                                                <button
                                    ...
                                    <button disabled={currentPage === totalPages} onClick={() => setCurrentPage(c => c + 1)} className="w-8 h-8 flex items-center justify-center rounded-full text-slate-500 hover:bg-slate-100 disabled:opacity-30 disabled:hover:bg-transparent transition-colors">
            ...
            {typeof document !== 'undefined' && createPortal(
                <AnimatePresence>
                    ...
                    {showMobileFilter && (
                        <div key="mobile-filter" className="fixed inset-0 z-[9999] flex justify-end">
                            <motion.div
                                initial={{ opacity: 0 }}
                                animate={{ opacity: 1 }}
                                exit={{ opacity: 0 }}
                            ...
                            >
                                <div className="flex justify-between items-center p-5 border-b border-slate-100">
                                    <h2 className="text-base font-semibold text-slate-900">{t.filters}</h2>
                                    <button onClick={() => setShowMobileFilter(false)} className="p-2 text-slate-400 hover:bg-slate-50 rounded-full"><FaTimes size={14} /></button>
                                </div>
                                <div className="flex-1 overflow-y-auto p-5 custom-scrollbar">
                                    <div className="mb-6">
                                        <label className="text-[11px] font-semibold text-slate-500 mb-2 block tracking-wide">{t.searchMpn}</label>
                                        <div className="relative">
                                            <FaSearch className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" size={12} />
                                            <input type="text" value={searchInput} onChange={(e) => setSearchInput(e.target.value)} placeholder={t.searchPlaceholder} className="w-full border border-slate-200 rounded-xl pl-9 pr-4 py-2.5 text-sm outline-none focus:border-slate-400" />
                                        </div>
                                    </div>
                                    <CustomDropdown label={t.categoryLabel} name="category" value={formData.category} options={categoryData?.map(c => ({ key: c.ID, value: c.category, label: c.category }))} onChange={handleChange} />
                                    <CustomDropdown label={t.subcategoryLabel} name="subcategory" value={formData.subcategory} options={subcategoryData?.filter(s => formData.category && s.category === formData.category).map(s => ({ key: s.ID, value: s.subcategory, label: s.subcategory }))} disabled={!formData.category} onChange={handleChange} />
                                    <CustomDropdown label={t.manufacturerLabel} name="manufacturer" value={formData.manufacturer} options={manufactureData?.map(m => ({ key: m.ID, value: m.namemanufacture, label: m.namemanufacture }))} onChange={handleChange} />
                                    <CustomDropdown label={t.footprintLabel} name="footprint" value={formData.footprint} options={footprintData?.map(fp => ({ key: fp.ID, value: fp.namefootprint, label: fp.namefootprint }))} onChange={handleChange} />
                                    <CustomDropdown label={t.supplierLabel} name="supplier" value={formData.supplier} options={supplierData?.map(s => ({ key: s.ID, value: s.namesupplier, label: s.namesupplier }))} onChange={handleChange} />
                                    <CustomDropdown
                                        label={t.sortBy}
                                        name="sortBy"
                                        value={formData.sortBy}
                                    ...
                                    {userInfo?.isAdmin && (
                                        <div className="mt-6 pt-4 border-t border-slate-100">
                                            ...
                                            >
                                                <MdNotifications size={14} /> {isTestingNoti ? t.sending : t.sendTestNoti}
                                            </button>
                                        </div>
                                    )}
                                </div>
                                <div className="p-5 border-t border-slate-100 flex gap-3 bg-white">
                                    <button onClick={handleReset} className="flex-1 py-2.5 border border-slate-200 text-slate-600 rounded-xl text-sm font-medium hover:bg-slate-50">{t.clear}</button>
                                    <button onClick={handleTriggerSearch} className="flex-1 py-2.5 bg-slate-900 text-white rounded-xl text-sm font-medium hover:bg-slate-800">{t.apply}</button>
                                </div>
                            </motion.div>
                        </div>
                    )}
                    ...
                    {showDetailModal && selectedDetailProduct && (
                        <div key="detail-modal" className="fixed inset-0 z-[9999] flex items-center justify-center p-4 sm:p-6 overflow-hidden">
                            <motion.div
                                initial={{ opacity: 0 }}
                                animate={{ opacity: 1 }}
                                exit={{ opacity: 0 }}
                            ...
                            >
                                <button onClick={() => setShowDetailModal(false)} className="absolute top-4 right-4 z-20 p-2 text-slate-400 hover:text-slate-800 hover:bg-slate-100 rounded-full transition-colors"><FaTimes size={14} /></button>
                                ...
                                <div className="w-full md:w-2/5 p-4 sm:p-10 bg-slate-50 flex flex-col items-center justify-center border-b md:border-b-0 md:border-r border-slate-100 shrink-0">
                                    <div className="w-28 h-28 sm:w-56 sm:h-56 bg-white border border-slate-200 rounded-2xl p-4 flex items-center justify-center mb-4 sm:mb-6 shadow-sm shadow-slate-200/50">
                                        {selectedDetailProduct.img ? <img src={`/componentImages${selectedDetailProduct.img}`} className="max-w-full max-h-full object-contain mix-blend-multiply transition-transform hover:scale-110" alt="product" /> : <FaBoxOpen className="text-slate-200" size={48} />}
                                    </div>
                                    <h3 className="text-base sm:text-xl font-bold text-slate-900 text-center mb-1 leading-tight px-4">{selectedDetailProduct.electotronixPN || selectedDetailProduct.manufacturePN}</h3>
                                    <p className="text-[9px] sm:text-[10px] text-slate-400 uppercase tracking-[0.2em] text-center font-bold">{selectedDetailProduct.manufacture}</p>
                                </div>
                                ...
                                <div className="flex-1 p-5 sm:p-10 overflow-y-auto custom-scrollbar bg-white">
                                    <h4 className="text-[10px] sm:text-xs font-bold text-slate-900 uppercase tracking-widest border-b border-slate-100 pb-3 mb-5 flex items-center gap-2">
                                        <span className="w-1.5 h-1.5 rounded-full bg-slate-900"></span> {t.techSpecs}
                                    </h4>
                                    <div className="grid grid-cols-2 gap-y-5 gap-x-4 sm:gap-y-6 sm:gap-x-6 mb-8 sm:mb-10">
                                        {[
                                            { label: t.categoryLabel, value: selectedDetailProduct.category },
                                            { label: t.subcategoryLabel, value: selectedDetailProduct.subcategory },
                                            { label: 'MPN / PN', value: selectedDetailProduct.manufacturePN },
                                            { label: t.footprintLabel, value: selectedDetailProduct.footprint || '-' },
                                            { label: t.warehousePos, value: selectedDetailProduct.position || '-' },
                                            { label: t.preferredSupplier, value: selectedDetailProduct.supplier },
                                            { label: t.currentInventory, value: `${Number(selectedDetailProduct.quantity || 0).toLocaleString()} ${t.unit}`, highlight: true },
                                            { label: t.unitPrice, value: formatPrice(selectedDetailProduct.price) }
                                        ].map((spec, i) => (
                                            <div key={i} className="group">
                                                <span className="text-[10px] text-slate-400 font-bold uppercase tracking-wider block mb-1 group-hover:text-slate-600 transition-colors">{spec.label}</span>
                                                <span className={`text-sm font-semibold break-words ${spec.highlight ? 'text-slate-900' : 'text-slate-700'}`}>{spec.value}</span>
                                            </div>
                                        ))}
                                    </div>
                                    ...
                                    <h4 className="text-xs font-bold text-slate-900 uppercase tracking-widest border-b border-slate-100 pb-3 mb-4 flex items-center gap-2">
                                        <span className="w-1.5 h-1.5 rounded-full bg-slate-300"></span> {t.overview}
                                    </h4>
                                    ...
                                    {(selectedDetailProduct.isStarred || selectedDetailProduct.lastStarredAt || selectedDetailProduct.lastUnstarredAt) && (
                                        <div className="mt-6 pt-6 border-t border-slate-100">
                                            <h4 className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-3">{t.activityHistory}</h4>
                                            <div className="space-y-2">

                                                {selectedDetailProduct.lastStarredAt && (
                                                    <div className="flex items-center gap-2 text-xs text-slate-500 italic">
                                                        <FaStar size={10} className="text-amber-400" />
                                                        <span>{t.lastStarred}: {new Date(selectedDetailProduct.lastStarredAt).toLocaleString()}</span>
                                                    </div>
                                                )}
                                                {selectedDetailProduct.lastUnstarredAt && (
                                                    <div className="flex items-center gap-2 text-xs text-slate-500 italic">
                                                        <FaRegStar size={10} className="text-slate-400" />
                                                        <span>{t.lastUnstarred}: {new Date(selectedDetailProduct.lastUnstarredAt).toLocaleString()}</span>
                                                    </div>
                                                )}
                                            </div>
                                        </div>
                    ...
                    {showImportantModal && currentProduct && (
                        <div key="important-modal" className="fixed inset-0 z-[9999] flex items-center justify-center p-4 overflow-hidden">
                            <motion.div
                                initial={{ opacity: 0 }}
                                animate={{ opacity: 1 }}
                                exit={{ opacity: 0 }}
                            ...
                            >
                                <div className={`w-10 h-10 sm:w-12 sm:h-12 rounded-full flex items-center justify-center mx-auto mb-3 sm:mb-4 bg-slate-50 border ${isUnmarking ? 'text-red-500 border-red-100' : 'text-slate-800 border-slate-200'}`}>
                                    {isUnmarking ? <FaExclamationCircle size={18} /> : <FaCheckCircle size={18} />}
                                </div>
                                <h3 className="text-base sm:text-lg font-bold text-slate-900 mb-1">{isUnmarking ? t.removePriority : t.markPriority}</h3>
                                <p className="text-xs sm:text-sm text-slate-400 mb-4 sm:mb-5">{t.reasonsub}</p>
                                ...
                                <div className="flex gap-3">
                                    <button onClick={() => setShowImportantModal(false)} className="flex-1 py-2.5 border border-slate-200 text-slate-600 rounded-xl text-sm font-medium hover:bg-slate-50 transition-colors">{t.cancel}</button>
                                    <button
                                        onClick={() => { handleUpdateImportant({ ...currentProduct, reason: isUnmarking ? null : reason }); setShowImportantModal(false); setReason(''); }}
