...
import { removeStockFromCart, clearStockCartItems, addToStockCart } from '../../../slices/stockCartApiSlice';
...
const StockCartScreen = () => {
   ...
   const filteredProducts = useMemo(() => {
      ...
      if (searchQuery.trim()) {
         ...
         filtered = filtered.filter(i => i.electotronixPN?.toLowerCase().includes(q) || i.manufacturePN?.toLowerCase().includes(q));
      }
      return filtered;
   ...
   const handleNoteChange = (p, n) => dispatch(addToStockCart({ ...p, note: n }));

   const requestnowHandler = async () => {
      if (selectedItems.length === 0) return toast.error(language === 'thai' ? 'กรุณาเลือกรายการที่ต้องการเบิก' : 'Please select items to request');
      try {
         ...
         toast.success(language === 'thai' ? 'สร้างรายการเบิกสำเร็จ!' : 'Request submitted successfully!');
         navigate('/componentuserrequestlist');
      } catch (err) {
         toast.error(language === 'thai' ? 'ไม่สามารถสร้างรายการเบิกได้' : 'Failed to submit request');
      }
   };
   ...
   const removeHandler = (id) => {
      ...
      toast.success(language === 'thai' ? 'ลบรายการเรียบร้อยแล้ว' : 'Item removed');
   };
         ...
         <div className="w-full max-w-[1200px] flex flex-col lg:flex-row gap-0 md:border md:border-slate-200/60 md:shadow-md md:rounded-2xl overflow-hidden">
            ...
            <div className="flex-1 p-0 md:p-8 bg-white text-start">
               <div className="flex flex-col md:flex-row md:justify-between md:items-center gap-4 mb-10 border-b border-slate-200 pb-6">
                  <h1 className="text-[18px] font-black text-slate-900 uppercase tracking-tight">Request Cart</h1>
               ...
               <div className="hidden md:grid grid-cols-12 pb-3 mb-6 border-b border-gray-100 text-[11px] uppercase tracking-[0.2em] text-gray-400 font-bold">
                  <div className="col-span-1 flex items-center justify-center">
                     <CustomCheckbox
                        checked={validItems.length > 0 && selectedIds.length === validItems.length}
                  ...
                  <AnimatePresence>
                     {filteredProducts.length === 0 ? (
                        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="py-20 text-center flex flex-col items-center justify-center">
                           ...
                           <h3 className="text-xl font-bold text-gray-800 mb-2">{language === 'thai' ? 'ตะกร้าของคุณยังว่างเปล่า' : 'Your request cart is empty'}</h3>
                           <Link to="/stockproductdashboard" className="text-blue-500 hover:underline mt-4 tracking-wide uppercase font-bold text-[12px]">Back to Dashboard</Link>
                        </motion.div>
                     ) : (
                                    ...
                                    <div className="col-span-7 flex flex-col md:flex-row gap-3 md:gap-5 w-full">
                                       ...
                                       <div className="flex gap-3 md:gap-5 items-start md:items-center w-full">
                                          ...
                                          <div className="w-16 h-16 md:w-24 md:h-24 bg-[#f3f4f6] shrink-0 flex items-center justify-center overflow-hidden rounded-xl border border-gray-100 p-2 shadow-sm group-hover:shadow-md transition-shadow">
                                             {p.img ? (
                                                ...
                                                <FaBoxOpen size={24} className="text-gray-400 md:hidden" />
                                             )}
                                             <div className="hidden md:block">
                                                {!p.img && <FaBoxOpen size={36} className="text-gray-400" />}
                                             </div>
                                          </div>

                                          {/* Details Context */}
                                          <div className="flex flex-col flex-1 min-w-0 py-0.5 md:py-0 text-start md:pr-6 relative">
                                             <span className="text-[8px] md:text-[10px] font-black uppercase tracking-widest text-slate-400 mb-0.5 md:mb-1 truncate block w-full">{p.manufacture || 'No Brand'}</span>
                                             <Link to={`/componenteditlist/${p.ID}`} className="text-[13px] md:text-[14px] font-black text-slate-900 uppercase hover:text-blue-600 transition-colors leading-snug mb-0.5 truncate block w-full pr-6 md:pr-0">
                                                {p.electotronixPN || p.manufacturePN}
                                             </Link>
                                             <span className="text-[9px] md:text-[11px] text-slate-400 font-bold leading-relaxed line-clamp-1">{p.description || "No description provided"}</span>
                                       ...
                                       <div className="flex md:hidden items-center justify-between w-full bg-slate-50 border border-slate-200 px-3 py-1.5 rounded-xl">
                                          <div className="flex flex-col min-w-0">
                                             <span className="text-[8px] font-black tracking-[0.15em] uppercase text-slate-400 mb-0.5">Units</span>
                                             <div className="flex items-center gap-1.5">
                                                <span className="font-bold text-[14px] text-slate-900 leading-none">
                                                   {p.reqqty}
                                                </span>
                                                <span className="text-[9px] font-bold text-slate-400 uppercase tracking-tighter">Requested</span>
                                             </div>
                                          </div>
               ...
               <div className="mt-8 pt-4 flex flex-col sm:flex-row gap-6">
                  <Link to="/cart" className="text-[11px] font-bold text-gray-400 hover:text-black transition-colors uppercase tracking-[0.2em] flex items-center gap-2 w-fit">
                     <FaArrowLeft size={10} /> Back to Cart
                  </Link>
                  <Link to="/componentcheckboom" className="text-[11px] font-bold text-indigo-400 hover:text-indigo-600 transition-colors uppercase tracking-[0.2em] flex items-center gap-2 w-fit">
            ...
            <div className="w-full lg:w-[350px] bg-white p-6 lg:p-8 shrink-0 flex flex-col text-start border-l border-slate-100">
               <h2 className="text-[14px] font-bold text-gray-900 uppercase tracking-wide mb-8">Request Summary</h2>
                  ...
                  <div className="mt-4">
                     ...
                     <div className="bg-gray-50 border border-dashed border-gray-200 rounded p-6 mb-8">
                        <p className="text-[11px] text-gray-500 font-medium leading-relaxed mb-4">
                           <strong className="text-gray-900 uppercase tracking-widest block mb-1 text-[10px]">Processing Policy</strong>
                           Requests are reviewed by the stock manager. Availability depends on current warehouse health.
                        </p>
                     ...
                     <div className="flex justify-between items-center mb-4 text-[13px]">
                        <span className="text-gray-600 uppercase font-semibold">Total Categories</span>
                     ...
                     <div className="flex justify-between items-center mb-8 border-b border-gray-100 pb-6 text-[13px]">
                        ...
                        <span className="text-gray-900 font-black text-[20px] tracking-tighter">{selectedItems.reduce((acc, curr) => acc + (Number(curr.reqqty) || 0), 0).toLocaleString()}</span>
                     </div>
                  </div>
               </div>
            </div>
         ...
         {showConfirmModal && createPortal(
            <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 overflow-hidden" style={{ width: '100vw', height: '100vh', top: 0, left: 0 }}>
               ...
               <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }} className="relative bg-white rounded shadow-2xl w-full max-w-sm p-8 text-center border-t-4 border-black font-sans z-10">
                  ...
                  <p className="text-[14px] text-gray-600 mb-8 font-medium">Are you sure you want to remove this component from your request cart?</p>
                  <div className="flex gap-3">
                     ...
                     <button onClick={() => removeHandler(itemToRemoveId)} className="flex-1 py-3 bg-black text-white font-bold text-[11px] uppercase tracking-widest hover:bg-gray-800 transition-colors shadow-lg">Remove</button>
                  </div>
               </motion.div>
            </div>,
            document.body
