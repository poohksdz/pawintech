...
const DefaultInvoiceListEditScreen = () => {
  ...
  const { data, isLoading, error, refetch } = useGetDefaultInvoicesQuery({
    pageNumber,
  });
  const [deleteDefaultInvoice, { isLoading: loadingDelete }] =
    useDeleteDefaultInvoiceMutation();
  ...
  const handleConfirmDefaultChange = async () => {
    try {
      ...
      const unsetPromises = Object.keys(selectedInvoices)
        ...
        .map((id) =>
          updateUseingDefaultInvoice({ defaultInvoiceId: id, set_default: 0 }),
        );

      await Promise.all(unsetPromises);

      // Set the new default
      await updateUseingDefaultInvoice({
        defaultInvoiceId: pendingDefaultId,
        set_default: 1,
      });
  ...
  return (
    <>
            ...
            <tbody>
              {data.invoices.map((inv, index) => (
                <tr key={inv.id} className="text-center align-middle">
                  <td>{index + 1}</td>
                  <td>
                    {inv.logo && <img src={inv.logo} alt="logo" width={50} />}
                  </td>
                  ...
                  <td>
                    <Button
                      as={Link}
                      to={`/admin/defaultinvoicelist/${inv.id}`}
                      variant="info"
                  ...
                  <td>
                    <Button
                      as={Link}
                      to={`/admin/defaultinvoicelist/${inv.id}/edit`}
                      variant="light"
      ...
      <Modal show={showDefaultModal} onHide={handleCancelDefaultChange}>
        ...
        <Modal.Body>
          Are you sure you want to set this invoice as the default?
        </Modal.Body>
        <Modal.Footer>
          <Button variant="light" onClick={handleCancelDefaultChange}>
            Cancel
          </Button>
          <Button variant="primary" onClick={handleConfirmDefaultChange}>
            Yes, Set Default
          </Button>
        </Modal.Footer>
      </Modal>
