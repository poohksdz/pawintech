...
const DefaultInvoiceSetScreen = () => {
  const navigate = useNavigate();
  const [createDefaultInvoice, { isLoading }] =
    useCreateDefaultInvoiceMutation();
  const [uploadImage] = useUploadDefaultInvoiceImageMutation();
  ...
  const handleSubmit = async (e) => {
    ...
    const salesManagerUrl = await handleUploadImage(
      salesManager,
      "salesManager",
    );
    ...
    try {
      ...
      toast.error(error?.data?.message || "Failed to create Default Invoice");
    }
  };
...
export default DefaultInvoiceSetScreen;
