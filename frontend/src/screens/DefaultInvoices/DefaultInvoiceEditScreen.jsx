...
const DefaultInvoiceEditScreen = () => {
  ...
  const { data, isLoading: loadingData } =
    useGetDefaultInvoiceDetailsQuery(id);
  const [updateDefaultInvoice, { isLoading: updating }] =
    useUpdateDefaultInvoiceMutation();
  ...
  useEffect(() => {
    if (data) {
      const q = data.invoice || data.Invoice || data;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.error(error?.data?.message || "Failed to update Default Invoice");
    }
  };
        ...
        <form onSubmit={handleSubmit} className="space-y-6 lg:space-y-8">
          ...
          <div className="flex justify-end pt-4">
            <Button
              ...
              disabled={updating}
            >
              {updating ? "Saving..." : "Save Default Invoice"}
            </Button>
          </div>
        </form>
