...
const OrderPCBPaymentScreen = () => {
  ...
  const calculatedTotal = useMemo(
    () =>
      reduxOrders.reduce(
        (acc, item) => acc + Number(item.total_amount_cost || item.price || 0),
        0,
      ),
    [reduxOrders],
  );
  ...
  const [createOrderPCB, { isLoading: isCreatingOrder }] =
  ...
  useEffect(() => {
    if (!pcbOrderDetails || reduxOrders.length === 0) {
      ...
      const tempOrderID =
        firstItem?.id || firstItem?.orderID || urlOrderId || generateOrderID();
  ...
  const submitHandler = async (e) => {
    ...
    if (!image)
      return toast.warning(
        language === "thai"
          ? "กรุณาอัปโหลดสลิปให้เสร็จสมบูรณ์"
          : "Please upload slip",
      );
    if (!transferedName)
      return toast.warning(
        language === "thai" ? "กรุณาเลือกบัญชีธนาคาร" : "Please select bank",
      );
    if (!orderID)
      return toast.error(
        language === "thai" ? "ไม่พบรหัสคำสั่งซื้อ" : "Order ID not found",
      );
    ...
    try {
      ...
      toast.success(
        language === "thai"
          ? "ชำระเงินสำเร็จ แอดมินกำลังตรวจสอบ"
          : "Payment successful, waiting for review",
      );
  ...
  const copyToClipboard = (text) => {
    ...
    toast.success(
      language === "thai" ? "คัดลอกเลขบัญชีแล้ว!" : "Account number copied!",
    );
  };
        ...
        >
          ...
          <div className="inline-flex items-center gap-2 px-6 py-2 rounded-full bg-blue-50 text-blue-600 border border-blue-100 shadow-sm">
            ...
            <span className="text-[11px] font-black uppercase tracking-[0.2em]">
              {language === "thai"
                ? "สั่งผลิตแผ่นปริ้นท์ (Standard PCB)"
          ...
          >
            <div className="bg-white rounded-[3rem] shadow-xl shadow-slate-200/50 border border-slate-100 overflow-hidden relative">
              <div className="p-3 flex gap-2 bg-slate-50/50 border-b border-slate-100">
                ...
                >
                  <FaQrcode />{" "}
                  {language === "thai" ? "สแกนจ่าย (QR)" : "Scan & Pay (QR)"}
                </button>
                <button
                  type="button"
                ...
                >
                  <FaUniversity />{" "}
                  {language === "thai" ? "โอนผ่านบัญชี" : "Bank Transfer"}
                </button>
              </div>
                      ...
                      <div className="bg-emerald-50/50 border border-emerald-100 p-8 rounded-[2.5rem] text-start">
                        <div className="flex items-center gap-5 mb-6">
                          ...
                          <div>
                            <p className="text-[10px] font-black text-emerald-600 uppercase tracking-widest mb-1">
                              {language === "thai" ? "ชื่อธนาคาร" : "Bank Name"}
                            </p>
                        ...
                        <div className="bg-white p-6 rounded-2xl flex items-center justify-between border border-emerald-100 shadow-sm">
                          ...
                          <button
                            ...
                            className="w-12 h-12 bg-slate-50 text-slate-300 hover:bg-emerald-600 hover:text-white rounded-xl transition-all flex items-center justify-center shadow-inner"
                          >
                            <FaCopy />
                          </button>
                        </div>
                        <div className="mt-6 flex flex-col gap-1">
                          <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">
                            {language === "thai" ? "ชื่อบัญชี" : "Account Name"}
                          </span>
            ...
            <div className="bg-slate-900 rounded-[2.5rem] p-8 text-white flex justify-between items-center shadow-2xl shadow-slate-300 border border-slate-800">
              <div>
                ...
                <p className="font-mono font-black text-2xl tracking-tight text-blue-400 uppercase">
            ...
            <div className="bg-white rounded-[3rem] shadow-xl shadow-slate-200/50 border border-slate-100 overflow-hidden relative">
              <div className="bg-slate-50/50 px-10 py-6 border-b border-slate-100 flex items-center justify-between">
                ...
                <span className="text-[10px] font-black text-slate-400 bg-white px-4 py-2 rounded-xl border border-slate-200 shadow-sm uppercase tracking-widest">
                  {reduxOrders.length}{" "}
                  {language === "thai" ? "รายการในคำสั่งซื้อ" : "Items"}
                </span>
              </div>
              ...
              >
                  ...
                  <div className="md:col-span-2 space-y-2">
                    ...
                    <div className="relative">
                      ...
                      >
                        ...
                        <option value="Kasikorn Bank (KBANK)">
                          Kasikorn Bank (KBANK)
                        </option>
                        <option value="Siam Commercial Bank (SCB)">
                          Siam Commercial Bank (SCB)
                        </option>
                        <option value="Bangkok Bank (BBL)">
                          Bangkok Bank (BBL)
                        </option>
                        <option value="Krung Thai Bank (KTB)">
                          Krung Thai Bank (KTB)
                        </option>
                        <option value="PromptPay / Other">
                          PromptPay / Other
                        </option>
                      </select>
                      <div className="absolute top-4 left-6 text-[10px] font-black text-blue-500 uppercase tracking-widest pointer-events-none">
                        {language === "thai" ? "เลือกธนาคาร" : "Select Bank"}
                      </div>
                ...
                <div className="space-y-3">
                  ...
                  <div
                    className={`relative group border-2 border-dashed rounded-[2.5rem] p-12 text-center transition-all duration-500 overflow-hidden ${image ? "border-emerald-400 bg-emerald-50/20" : "border-slate-200 bg-slate-50/50 hover:border-blue-400 hover:bg-white"}`}
                  >
                    <input
                      type="file"
                    ...
                    {previewUrl ? (
                      <div className="relative inline-block">
                        <img
                          src={previewUrl}
                          alt="Preview"
                          className="max-h-64 mx-auto rounded-3xl shadow-2xl border-4 border-white transform transition-transform group-hover:scale-[1.02]"
                        />
                    ...
                    {isImageUploading && (
                      <div className="absolute inset-0 bg-white/60 backdrop-blur-sm flex items-center justify-center rounded-3xl z-30">
                        <Loader />
                      </div>
                    )}
                  </div>
                ...
                <div className="text-center">
                  <div className="inline-flex items-center gap-2 text-slate-300 text-[9px] font-black uppercase tracking-[0.3em]">
                    <FaShieldAlt className="text-blue-500" /> Fully Encrypted &
                    Secure
                  </div>
                </div>
              </form>
          ...
          >
            ...
            >
              ...
              <p className="text-sm text-slate-500 mb-10 font-medium leading-relaxed">
                {language === "thai"
                  ? "กรุณาตรวจสอบข้อมูลและสลิปให้ถูกต้องก่อนกดยืนยัน"
                  : "Please ensure all payment details and the slip are correct before confirming."}
              </p>
              <div className="flex gap-4">
                ...
                <button
                  ...
                  className="flex-1 py-4 bg-blue-600 text-white font-black text-[11px] uppercase tracking-widest hover:bg-blue-700 transition-all shadow-xl shadow-blue-200 rounded-2xl border-none"
                >
                  {isCreatingOrder ? (
                    <Loader size="xs" />
                  ) : language === "thai" ? (
