...
import { Table, Button, Row, Col, Image, Card, Modal, Form} from 'react-bootstrap';
...
const ShowcaselListScreen = () => {
  ...
  const [ updateOrderPresentShowcase ] =
    useUpdateOrderPresentShowcaseMutation();
    ...
    <>
      <Row className='align-items-center'>
        ...
        <Col className='text-end'>
          <Button className='my-3' as={Link}
                      to={`/admin/showcase/create`} >
...
<Table striped bordered hover responsive className='table-sm'>
  ...
  <tbody style={{ fontSize: '20px', textAlign: 'center' }}>
              {data.map((showcase, index) => {
                const name = language === 'thai' ? showcase.nameThai : showcase.name;
                return (
                  <tr key={showcase._id}>
                    ...
                    <td style={{ textAlign: 'center', verticalAlign: 'middle' }}>
                      <div> 
                          {showcase.present} {showcase.displayOrder !== 0 && `(${showcase.displayOrder})`} 
                                                <Button
                        variant='primary'
