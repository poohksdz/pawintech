...
import { Table, Form, Button, Container, Row, Col, InputGroup, Nav, Card, Badge } from 'react-bootstrap';
import { useUpdateShippingRatesMutation, useGetOwnShippingRatesQuery } from '../../slices/orderpcbSlice';
import { toast } from 'react-toastify';
import Loader from '../../components/Loader';
import Message from '../../components/Message';
import {
  FaSave, FaSlidersH, FaLayerGroup, FaTruck, FaChevronRight, FaGlobeAsia, FaCoins, FaBars, FaTimes
} from 'react-icons/fa';
...
const OrderPCBEditScreen = () => {
  ...
  const [updateShippingRates, { isLoading: isUpdating }] = useUpdateShippingRatesMutation();
  ...
  useEffect(() => {
    if (data) {
      ...
      const mapPrice = (list) => (list || []).map(item => ({ ...item, price: Number(item.price) }));
      ...
      const updatedRates = generateRates().map((rate) => {
        const emsRate = data.shippingRates.find(r => Number(r.weight_kg).toFixed(1) === rate.kg.toFixed(1) && r.shipping_type === 'EMS');
        const dhlRate = data.shippingRates.find(r => Number(r.weight_kg).toFixed(1) === rate.kg.toFixed(1) && r.shipping_type === 'DHL');
        return { ...rate, ems: emsRate ? emsRate.price : 0, dhl: dhlRate ? dhlRate.price : 0 };
      });
  ...
  const handleSubmit = async () => {
    const payload = {
      base_price: settings.basePrice, price_per_cm2: settings.pricePerCm,
      extra_service_fee: settings.extraService, profit_margin: settings.profitMargin,
      exchange_rate: settings.exchangeRate, vat_percent: settings.vat,
      build_time: settings.build_time, dhl_service_fixed: settings.dhlRate,
      ...
      baseMaterials, surfaceFinishes, copperWeights, pcbColors,
    };
    try {
  ...
  const SettingsTable = ({ title, items, colorTheme, icon }) => (
    <Card className="shadow-sm border-0 mb-4 overflow-hidden">
      <div className={`p-3 border-bottom d-flex align-items-center gap-2 bg-${colorTheme}-subtle text-${colorTheme}`}>
      ...
      <Table bordered hover responsive className="mb-0 align-middle">
        <tbody>
          {items.map((item, idx) => (
            <tr key={idx}>
              <td className={`bg-light text-secondary fw-medium`} style={{ width: '40%', minWidth: '140px', borderRight: '1px solid #dee2e6' }}>
  ...
  const MaterialTable = ({ title, data, setData, colorClass }) => (
    <Card className="shadow-sm border-0 h-100 overflow-hidden material-card">
      <div className={`px-3 py-2 border-bottom ${colorClass} text-white d-flex justify-content-between align-items-center`}>
      ...
      <div className="table-responsive">
        <Table bordered hover size="sm" className="mb-0 align-middle">
          <tbody>
            {data.map((item, index) => (
              <tr key={index}>
                <td className="ps-3 text-secondary small fw-bold" style={{ minWidth: '120px' }}>{item.name}</td>
                <td className="p-0" style={{ width: '100px' }}>
                  <Form.Control
                    ...
                    onChange={(e) => {
                      ...
                      setData(prev => prev.map((ent, i) => i === index ? { ...ent, price: val } : ent));
                    }}
  ...
  return (
    <div className="d-flex" style={{ height: '100vh', overflow: 'hidden', backgroundColor: '#f0f2f5', position: 'relative' }}>
      ...
      >
        <div className="p-4 border-bottom border-secondary border-opacity-25 d-flex justify-content-between align-items-center">
          <div>
            <div className="d-flex align-items-center gap-2 mb-1">
              <div className="bg-primary rounded p-1"><FaSlidersH className="text-white" /></div>
              <h5 className="fw-bold mb-0">PCB Admin</h5>
            </div>
            <small className="text-white-50 ms-1">Pricing Control</small>
          </div>
          {/* Mobile Close Button */}
          <Button variant="link" className="text-white d-md-none p-0" onClick={() => setShowSidebar(false)}>
            <FaTimes size={24} />
          </Button>
        </div>
        ...
        <Nav className="flex-column p-3 gap-2">
          {[
            { id: 'general', label: 'General Config', icon: <FaGlobeAsia />, color: '#3b82f6' },
            { id: 'materials', label: 'Materials', icon: <FaLayerGroup />, color: '#10b981' },
            { id: 'shipping', label: 'Shipping Rates', icon: <FaTruck />, color: '#8b5cf6' },
          ].map(item => (
            <Nav.Link
              key={item.id}
              ...
              style={activeTab === item.id ? { backgroundColor: item.color, color: 'white' } : {}}
            >
              <span className="me-3 fs-5">{item.icon}</span>
              <span className="fw-medium">{item.label}</span>
              {activeTab === item.id && <FaChevronRight className="ms-auto small opacity-75" />}
            </Nav.Link>
          ))}
        </Nav>
        ...
        <div className="mt-auto p-4 border-top border-secondary border-opacity-25 bg-black bg-opacity-25">
          <Button
            ...
            disabled={isUpdating}
          >
            {isUpdating ? 'Saving...' : <><FaSave className="me-2" /> SAVE CHANGES</>}
          </Button>
        </div>
      </div>
      ...
      <div className="flex-grow-1 d-flex flex-column overflow-hidden w-100">
        ...
        {/* Header (Responsive) */}
        <div className="bg-white border-bottom px-3 px-md-5 py-3 shadow-sm d-flex justify-content-between align-items-center" style={{ height: '70px', zIndex: 5 }}>
          <div className="d-flex align-items-center gap-3">
            {/* Mobile Toggle Button */}
            <Button variant="light" className="d-md-none border text-secondary" onClick={() => setShowSidebar(true)}>
              <FaBars />
            </Button>

            <div>
              <span className="text-muted text-uppercase small fw-bold ls-1 d-none d-sm-block">Current Section</span>
              <h5 className="mb-0 fw-bold d-none d-sm-block" style={{
                color: activeTab === 'general' ? '#3b82f6' : activeTab === 'materials' ? '#10b981' : '#8b5cf6'
              }}>
              ...
              <h5 className="mb-0 fw-bold d-sm-none text-dark">
                {activeTab === 'general' ? 'General' : activeTab === 'materials' ? 'Materials' : 'Shipping'}
              </h5>
            </div>
          </div>
        </div>
                  ...
                  <Col xs={12} lg={6}>
                    <SettingsTable
                      ...
                      items={[
                        ...
                        { label: 'Price per mm²', key: 'pricePerCm', unit: '฿/mm²' },
                        { label: 'Extra Service', key: 'extraService', unit: '฿' },
                  ...
                  <Col xs={12} lg={6}>
                    <SettingsTable
                      ...
                      items={[
                        ...
                        { label: 'Build Time', key: 'build_time', unit: 'Days' },
                      ]}
                    />
                  </Col>
                </Row>
            ...
            {activeTab === 'materials' && (
              <div className="fade-in">
                <Row className="g-3 g-md-4">
                  <Col xs={12} md={6} xl={3}><MaterialTable title="Base Materials" data={baseMaterials} setData={setBaseMaterials} colorClass="bg-success" /></Col>
                  <Col xs={12} md={6} xl={3}><MaterialTable title="Copper Weights" data={copperWeights} setData={setCopperWeights} colorClass="bg-teal" /></Col>
                  <Col xs={12} md={6} xl={3}><MaterialTable title="Surface Finishes" data={surfaceFinishes} setData={setSurfaceFinishes} colorClass="bg-primary" /></Col>
                  <Col xs={12} md={6} xl={3}><MaterialTable title="PCB Colors" data={pcbColors} setData={setPcbColors} colorClass="bg-indigo" /></Col>
                </Row>
              </div>
            )}
                ...
                <Card className="shadow-sm border-0 overflow-hidden">
                  <Card.Header className="bg-white py-3 d-flex justify-content-between align-items-center">
                    <h6 className="mb-0 fw-bold text-dark d-flex align-items-center">
                      ...
                      <span className="d-none d-sm-inline">BULK SHIPPING RATES</span>
                  ...
                  <Card.Body className="p-0">
                    <div style={{ height: 'calc(100vh - 200px)', overflow: 'auto' }}>
                      <Table bordered hover className="mb-0 text-center align-middle sticky-header-table">
                        <thead className="text-white" style={{ position: 'sticky', top: 0, zIndex: 10 }}>
                          <tr>
                            <th className="py-3 bg-secondary" style={{ width: '20%', minWidth: '80px' }}>KG</th>
                            <th className="py-3" style={{ width: '40%', minWidth: '100px', backgroundColor: '#3b82f6' }}>EMS (฿)</th>
                            <th className="py-3" style={{ width: '40%', minWidth: '100px', backgroundColor: '#f59e0b' }}>DHL (฿)</th>
                          </tr>
                        </thead>
                        <tbody>
                          {rates.map((row, index) => (
                            <tr key={index}>
                              <td className="fw-bold text-dark bg-light">{row.kg.toFixed(1)}</td>
                              <td className="p-0 position-relative bg-blue-light">
                                <Form.Control
                                  type="number"
                                  value={row.ems}
                                  onChange={(e) => {
                                    const newRates = [...rates]; newRates[index].ems = Number(e.target.value); setRates(newRates);
                                  }}
                              ...
                              <td className="p-0 position-relative bg-orange-light">
                                <Form.Control
                                  ...
                                  value={row.dhl}
                                  onChange={(e) => {
                                    const newRates = [...rates]; newRates[index].dhl = Number(e.target.value); setRates(newRates);
                                  }}
      ...
      <style jsx global>{`
        ...
        .hover-light:hover { color: #fff !important; background-color: rgba(255,255,255,0.1); }
