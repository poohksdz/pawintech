...
const ServiceListScreen = () => {
  ...
  const handleCheckboxChange = async (serviceId) => {
    ...
    const showFrontCount = Object.values(selectedServices).filter(
      (v) => v === 1,
    ).length;
        ...
        <div className="bg-white rounded-[2rem] shadow-sm border border-slate-100 p-6 md:p-8 mb-8 flex flex-col md:flex-row items-center justify-between gap-6">
          <div className="flex items-center gap-5 w-full md:w-auto">
            ...
            <div>
              ...
              <p className="text-sm md:text-base text-slate-500 font-medium m-0">
                {t.subtitle}
              </p>
            </div>
          </div>
          ...
          <>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 md:gap-8">
              {data.services.map((service) => (
                <div
                  key={service.ID}
                  className="bg-white rounded-[1.5rem] shadow-sm border border-slate-100 overflow-hidden flex flex-col group hover:shadow-lg hover:shadow-slate-200/50 hover:-translate-y-1 transition-all duration-300"
                >
                  ...
                  <div className="p-5 md:p-6 flex flex-col flex-grow">
                    <h3
                      className="font-bold text-slate-900 text-lg mb-1 truncate"
                      title={
                        language === "thai"
                    ...
                    <div className="mt-auto pt-4 border-t border-slate-100 flex items-center justify-between">
                      ...
                      >
                        ...
                        >
                          <span
                            className={`pointer-events-none inline-block h-4 w-4 transform rounded-full bg-white shadow-sm ring-0 transition duration-200 ease-in-out ${!!selectedServices[service.ID] ? "translate-x-2" : "-translate-x-2"}`}
                          />
                        </button>
