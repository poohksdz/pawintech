...
import { Table, Button, Row, Col, Modal, Card, Container, Badge, InputGroup, Form } from 'react-bootstrap';
import { toast } from 'react-toastify';
import { FaEdit, FaTrash, FaSearch, FaMicrochip, FaMoneyBillWave, FaCalendarAlt, FaClipboardList } from 'react-icons/fa';
...
const OrderPCBEditListsScreen = () => {
    ...
    const [deleteOrderPCB, { isLoading: loadingDelete }] = useDeleteOrderPCBMutation();
    ...
    const stats = useMemo(() => {
        ...
        return {
            count: data.length,
            revenue: data.reduce((acc, item) => acc + (parseFloat(item.quoted_price_to_customer) || 0), 0)
        };
    }, [data]);
    ...
    return (
        <div className="bg-light min-vh-100 py-4 font-prompt">
            <Container fluid="lg">
                ...
                <div className="d-flex flex-column flex-md-row justify-content-between align-items-md-center mb-5 gap-3 animate__animated animate__fadeInDown">
                    <div className="d-flex align-items-center gap-3">
                        <div className="bg-white p-3 rounded-4 shadow-sm text-primary d-flex align-items-center justify-content-center border" style={{ width: 60, height: 60 }}>
                        ...
                        <Card className="border-0 shadow-sm rounded-4 h-100 overflow-hidden card-stat">
                            <Card.Body className="p-4 position-relative">
                                <div className="d-flex justify-content-between align-items-start z-1 position-relative">
                                    <div>
                                        <div className="text-muted fw-bold text-uppercase small mb-2">{t.TotalOrders}</div>
                                        <h2 className="display-5 fw-bold text-dark mb-0">{stats.count}</h2>
                                    </div>
                    ...
                    <Col xs={12} md={6}>
                        <Card className="border-0 shadow-sm rounded-4 h-100 overflow-hidden card-stat bg-primary text-white">
                            <Card.Body className="p-4 position-relative">
                                <div className="d-flex justify-content-between align-items-start z-1 position-relative">
                                    <div>
                                        <div className="text-white text-opacity-75 fw-bold text-uppercase small mb-2">{t.TotalRevenue}</div>
                                        <h2 className="display-5 fw-bold mb-0">{formatCurrency(stats.revenue)}</h2>
                                    </div>
                        ...
                        <Row className="g-3 align-items-center">
                            <Col xs={12} md={6} lg={5}>
                                <InputGroup className="search-bar shadow-sm rounded-pill">
                                    ...
                                    />
                                    {searchTerm && (
                                        <Button variant="link" className="text-muted text-decoration-none" onClick={() => setSearchTerm('')}>x</Button>
                                    )}
                                </InputGroup>
                            </Col>
                            <Col xs={12} md={6} lg={7} className="text-md-end">
                                <Badge bg="light" text="dark" className="border px-3 py-2 rounded-pill fw-normal">
                                    {t.Showing} <span className="fw-bold">{filteredOrders.length}</span> {t.Items}
                                </Badge>
                            </Col>
                        </Row>
                    </div>
                        ...
                        {isLoading ? (
                            ...
                            <div className="text-center py-5">
                                <div className="bg-white p-4 rounded-circle shadow-sm d-inline-block mb-3">
                                    <FaMicrochip size={40} className="text-secondary opacity-50" />
                                </div>
                                ...
                                <div className="table-responsive d-none d-lg-block">
                                    <Table hover className="align-middle mb-0 custom-table">
                                        <thead className="bg-white text-secondary text-uppercase small">
                                            <tr>
                                                <th className="py-3 ps-4 border-bottom" style={{ width: '5%' }}>#</th>
                                                <th className="py-3 border-bottom" style={{ width: '15%' }}>{t.PCBID}</th>
                                                <th className="py-3 border-bottom" style={{ width: '30%' }}>{t.ProjectName}</th>
                                                <th className="py-3 border-bottom text-center" style={{ width: '10%' }}>{t.Qty}</th>
                                                <th className="py-3 border-bottom" style={{ width: '15%' }}>{t.TotalPrice}</th>
                                                <th className="py-3 border-bottom" style={{ width: '15%' }}>{t.Date}</th>
                                                <th className="py-3 pe-4 border-bottom text-end" style={{ width: '10%' }}>{t.Action}</th>
                                            </tr>
                                        </thead>
                                        <tbody className="bg-white">
                                            {filteredOrders.map((pcb, index) => (
                                                <tr key={pcb.id} className="row-hover">
                                                    ...
                                                    <td>
                                                        <Badge bg="light" text="dark" className="border font-monospace text-secondary fw-normal">
                                                            {pcb.orderID}
                                                        </Badge>
                                                    </td>
                                                    <td>
                                                        <div className="fw-bold text-dark text-truncate" style={{ maxWidth: '250px' }} title={pcb.projectname}>
                                                            {pcb.projectname}
                                                        </div>
                                                    </td>
                                                    ...
                                                    <td className="text-muted small">
                                                        <div className="d-flex align-items-center gap-2">
                                                            ...
                                                            {new Date(pcb.created_at).toLocaleDateString(language === 'thai' ? 'th-TH' : 'en-GB')}
                                                        </div>
                                                    </td>
                                                    ...
                                                    <Card.Body className="p-3">
                                                        <div className="d-flex justify-content-between align-items-start mb-3">
                                                            <div className="d-flex align-items-center gap-2">
                                                                ...
                                                                <div>
                                                                    <div className="font-monospace text-muted small" style={{ fontSize: '0.75rem' }}>{pcb.orderID}</div>
                                                                    <div className="fw-bold text-dark text-truncate" style={{ maxWidth: '200px' }}>{pcb.projectname}</div>
                                                                </div>
                                                            </div>
                                                            <Badge bg="success" className="bg-opacity-10 text-success border border-success fw-bold">
                                                        ...
                                                        <div className="bg-light rounded-3 p-2 mb-3 d-flex justify-content-between small">
                                                            <div className="text-muted"><FaClipboardList className="me-1" /> Qty: <span className="fw-bold text-dark">{pcb.pcb_quantity}</span></div>
                                                            <div className="text-muted"><FaCalendarAlt className="me-1" /> {new Date(pcb.created_at).toLocaleDateString()}</div>
                                                        </div>
            ...
            {/* Delete Modal */}
            <Modal show={showModal} onHide={() => setShowModal(false)} centered backdrop="static" contentClassName="border-0 shadow-lg rounded-4">
                <Modal.Body className="p-4 text-center">
                    ...
                    <div className="d-flex gap-2 justify-content-center">
                        ...
                        <Button variant="danger" onClick={handleConfirmDelete} className="rounded-pill px-4 fw-bold shadow-sm">
                            {t.Delete}
                        </Button>
                    </div>
                </Modal.Body>
