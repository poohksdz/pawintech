  ...
  if (!userInfo) {
    return (
      <div className="min-h-screen flex items-center justify-center font-['Prompt']">
        กรุณาล็อกอินเพื่อดูสถานะออเดอร์
      </div>
    );
  }

  return (
    <div className="min-h-screen bg-[#F5F7FA] py-4 md:py-6 md:py-10 px-4 md:px-8 font-['Prompt']">
      <div className="max-w-4xl mx-auto space-y-8">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-slate-500 font-medium text-sm">
              ติดตามสถานะออเดอร์ Custom PCB ของคุณ
            </p>
          </div>
        </div>
        ...
        {loading ? (
          ...
          <div className="p-4 bg-rose-100 text-rose-600 rounded-xl font-bold">
            {error}
          </div>
        ) : orders.length === 0 ? (
          <div className="bg-white rounded-3xl py-20 text-center shadow-sm border border-slate-100">
            ...
            <Link
              to="/custompcb"
              className="mt-4 inline-block bg-blue-600 text-white px-4 md:px-6 py-2 rounded-full font-bold shadow-md hover:bg-blue-700 transition"
            >
              เริ่มสั่งทำ PCB เลย
            </Link>
          </div>
          ...
          <div className="space-y-6">
            {orders.map((order) => {
              ...
              return (
                <div
                  key={order._id}
                  className="bg-white rounded-[2rem] p-4 md:p-8 shadow-sm border border-slate-100 transition-all hover:shadow-md"
                >
                  {/* Order Info Header */}
                  <div className="flex flex-wrap justify-between items-start gap-4 mb-8 border-b border-slate-100 pb-4">
                    ...
                    <div className="text-right">
                      ...
                      <p className="text-xl font-black text-blue-600">
                        {Number(
                          order.transferedAmount || order.confirmed_price,
                        ).toLocaleString()}{" "}
                        ฿
                      </p>
                    </div>
                  ...
                  {currentStatus === "rejected" ? (
                    <div className="bg-rose-50 border border-rose-200 rounded-2xl p-4 md:p-6 text-center">
                      <FaTimesCircle
                        size={40}
                        className="mx-auto text-rose-500 mb-3"
                      />
                      <h3 className="text-lg font-bold text-rose-700">
                        สลิปการโอนเงินไม่ถูกต้อง / ถูกปฏิเสธ
                      </h3>
                      <p className="text-sm text-rose-500 mt-1">
                        กรุณาติดต่อแอดมินผ่านไลน์เพื่อตรวจสอบข้อมูลอีกครั้ง
                      </p>
                    </div>
                  ) : (
                    ...
                    <div className="relative pt-4 pb-2">
                      ...
                      <div className="relative z-10 flex justify-between items-center w-full">
                        ...
                        <div className="flex flex-col items-center gap-3 flex-1">
                          <div
                            className={`w-12 h-12 rounded-full flex items-center justify-center shadow-sm transition-all duration-300 ${["pending", "accepted", "delivered"].includes(currentStatus) ? "bg-blue-600 text-white shadow-blue-200" : "bg-slate-200 text-slate-400"}`}
                          >
                            <FaFileInvoice size={20} />
                          </div>
                          <p
                            className={`text-xs md:text-sm font-bold text-center ${["pending", "accepted", "delivered"].includes(currentStatus) ? "text-slate-800" : "text-slate-400"}`}
                          >
                            รอตรวจสลิป
                          </p>
                        </div>
                        ...
                        <div className="flex flex-col items-center gap-3 flex-1">
                          <div
                            className={`w-12 h-12 rounded-full flex items-center justify-center shadow-sm transition-all duration-300 ${["accepted", "delivered"].includes(currentStatus) ? "bg-blue-600 text-white shadow-blue-200" : "bg-white border-2 border-slate-200 text-slate-300"}`}
                          >
                            <FaCog
                              size={20}
                              className={
                          ...
                          <p
                            className={`text-xs md:text-sm font-bold text-center ${["accepted", "delivered"].includes(currentStatus) ? "text-slate-800" : "text-slate-400"}`}
                          >
                            เตรียมสินค้า
                          </p>
                        </div>
                        ...
                        <div className="flex flex-col items-center gap-3 flex-1">
                          <div
                            className={`w-12 h-12 rounded-full flex items-center justify-center shadow-sm transition-all duration-300 ${currentStatus === "delivered" ? "bg-emerald-500 text-white shadow-emerald-200" : "bg-white border-2 border-slate-200 text-slate-300"}`}
                          >
                            <FaTruck size={20} />
                          </div>
                          <p
                            className={`text-xs md:text-sm font-bold text-center ${currentStatus === "delivered" ? "text-emerald-600" : "text-slate-400"}`}
                          >
                            จัดส่งสำเร็จ
                          </p>
                        </div>
                  ...
                  {currentStatus === "delivered" && order.deliveryID && (
                    <div className="mt-8 bg-emerald-50 border border-emerald-100 rounded-2xl p-5 flex flex-col md:flex-row items-center justify-between gap-4">
                      ...
                      >
                        <span className="font-mono font-black text-emerald-700 text-lg tracking-wider">
                          {order.deliveryID}
                        </span>
