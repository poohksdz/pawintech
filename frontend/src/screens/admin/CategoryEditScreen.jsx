...
import { FaArrowLeft, FaSave, FaEdit, FaLanguage, FaBarcode } from 'react-icons/fa';
import Loader from '../../components/Loader';
import Message from '../../components/Message';
import { useGetCategoryDetailsQuery, useUpdateCategoryMutation } from '../../slices/categorySlice';
...
const CategoryEditScreen = () => {
  ...
  const t = {
    en: {
      ...
      lbl: { en: 'Category Name (English)', th: 'Category Name (Thai)', code: 'Category Code / Short Name' },
      btn: { save: 'Update Category', back: 'Go Back', saving: 'Updating...' },
      ph: { en: 'Enter category name in English', th: 'Enter category name in Thai', code: 'Enter unique code' }
    },
    thai: {
      title: 'แก้ไขหมวดหมู่',
      subtitle: 'อัปเดตข้อมูลหมวดหมู่สินค้า',
      lbl: { en: 'ชื่อหมวดหมู่ (ภาษาอังกฤษ)', th: 'ชื่อหมวดหมู่ (ภาษาไทย)', code: 'รหัสหมวดหมู่ / ชื่อย่อ' },
      btn: { save: 'บันทึกการเปลี่ยนแปลง', back: 'ย้อนกลับ', saving: 'กำลังบันทึก...' },
      ph: { en: 'ระบุชื่อหมวดหมู่ภาษาอังกฤษ', th: 'ระบุชื่อหมวดหมู่ภาษาไทย', code: 'ระบุรหัสที่ไม่ซ้ำกัน' }
    }
  }[language || 'en'];
  ...
  return (
    <Container className="py-4 font-prompt">
      <Link to="/admin/categorylist" className="btn btn-light mb-3 shadow-sm border text-decoration-none">
                    ...
                    <Card.Body className="p-4">
                        <Form onSubmit={submitHandler}>
                            <Row className="g-3">
                                <Col md={12}>
                                    <Form.Group className="mb-3" controlId="categoryName">
                                        <Form.Label className="fw-bold text-muted small"><FaLanguage className="me-1"/> {t.lbl.en} <span className="text-danger">*</span></Form.Label>
                                        <Form.Control
                                            type="text"
                                            placeholder={t.ph.en}
                                            value={categoryName}
                                ...
                                <Col md={12}>
                                    <Form.Group className="mb-3" controlId="categoryNameThai">
                                        <Form.Label className="fw-bold text-muted small"><FaLanguage className="me-1"/> {t.lbl.th} <span className="text-danger">*</span></Form.Label>
                                        <Form.Control
                                            type="text"
                                            placeholder={t.ph.th}
                                ...
                                <Col md={12}>
                                    <Form.Group className="mb-4" controlId="categoryShortName">
                                        <Form.Label className="fw-bold text-muted small"><FaBarcode className="me-1"/> {t.lbl.code} <span className="text-danger">*</span></Form.Label>
                                        <Form.Control
                                            type="text"
                            ...
                            <div className="d-grid pt-2">
                                <Button type="submit" variant="warning" size="lg" className="text-dark shadow-sm fw-bold" disabled={loadingUpdate}>
