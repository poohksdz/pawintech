...
const OrderListsScreen = () => {
  ...
  // ---------------- API ----------------
  const { data: allOrdersData, isLoading, error } = useGetAllUnifiedOrdersQuery()
  ...
  const statusBadge = status => {
    const baseClass = "px-2 py-1 text-xs font-bold uppercase tracking-wider rounded-md";
    switch (status) {
      case 'accepted':
        return <span className={`${baseClass} bg-emerald-100 text-emerald-800`}>Accepted</span>
      case 'pending':
        return <span className={`${baseClass} bg-amber-100 text-amber-800`}>Pending</span>
      case 'manufacturing':
        return <span className={`${baseClass} bg-blue-100 text-blue-800`}>Manufacturing</span>
      case 'delivered':
        return <span className={`${baseClass} bg-indigo-100 text-indigo-800`}>Delivered</span>
      default:
  ...
  const typeBadge = type => {
    ...
    return <span className={`px-2 py-1 text-xs font-bold uppercase tracking-wider rounded-md ${colors[type]}`}>{type}</span>
  }
  ...
  // ---------------- RENDER ----------------
  if (isLoading)
    return <div className="min-h-screen flex justify-center items-center bg-slate-50"><Loader /></div>

  if (error)
    return <div className="max-w-4xl mx-auto py-10 px-4"><Message variant="danger">Failed to load orders</Message></div>
    ...
    <div className="bg-slate-50 min-h-screen p-4 sm:p-6 lg:p-8 font-sans text-slate-900">
      <div className="max-w-screen-2xl mx-auto space-y-6">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
          <div>
            <h1 className="text-xl sm:text-3xl font-black text-slate-800 tracking-tight m-0 uppercase md:normal-case">Order Lists</h1>
            <p className="text-slate-500 text-[10px] md:text-sm mt-1 uppercase tracking-wider font-medium">Manage and review all orders</p>
          </div>
          <Button
            variant="primary"
              ...
              <tbody className="divide-y divide-slate-100">
                {filteredOrders.length > 0 ? (
                  filteredOrders.map((o, i) => (
                    <tr key={`${o.type}-${o.id}`} className="hover:bg-slate-50 transition-colors">
                      ...
                      <td className="px-6 py-4 font-mono text-slate-700">{o.orderID}</td>
                      <td className="px-6 py-4 font-medium text-slate-800 truncate max-w-[200px]" title={o.projectname}>{o.projectname}</td>
                      <td className="px-6 py-4">
                        {typeBadge(o.type)}
                      </td>
                      <td className="px-6 py-4 text-slate-600 truncate max-w-[150px]">{o.company}</td>
                      <td className="px-6 py-4 text-slate-600">{o.customer}</td>
                      <td className="px-6 py-4 text-slate-600">{o.phone}</td>
                      <td className="px-6 py-4 font-bold text-slate-800 text-right">{Number(o.price || 0).toLocaleString()}</td>
                      <td className="px-6 py-4 text-slate-500">{formatDateDMY(o.createdAt)}</td>
                  ...
                  <tr>
                    <td colSpan="11" className="px-6 py-12 text-center text-slate-500">
                      <div className="flex flex-col items-center justify-center">
                        ...
                        <p className="text-lg font-medium text-slate-600 uppercase tracking-widest text-sm">No orders found</p>
