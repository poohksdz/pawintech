...
import { FaArrowLeft, FaSave, FaTrash, FaCloudUploadAlt, FaFileArchive, FaUser, FaTruck, FaFileInvoiceDollar, FaRegCreditCard } from 'react-icons/fa';
import {
...
const CustomPCBOrderEditScreen = () => {
  ...
  const [updateCustomPCB, { isLoading: updateLoading }] = useUpdateCustomPCBMutation();
  ...
  const removeImage = (index) => setDiagramImages((prev) => prev.filter((_, i) => i !== index));
  ...
  const uploadDiagramImages = async () => {
    ...
    return (res?.images || []).map(img => typeof img === 'string' ? img : (img.path || img.url));
  };
  ...
  const submitHandler = async (e) => {
    e.preventDefault();
    if (!projectname.trim()) return toast.error(language === 'thai' ? 'กรุณาระบุชื่อโปรเจกต์' : 'Please enter a project name.');

    try {
      setIsLoadingSubmit(true);
      const uploadedNewImages = await uploadDiagramImages();
      const existingImages = diagramImages.filter((img) => !img.file).map((img) => img.raw || img.url);
      ...
      toast.success(language === 'thai' ? 'อัปเดตข้อมูลสำเร็จ!' : 'Order updated successfully!');
      navigate('/admin/ordercustompcblist');
    } catch (err) {
      console.error(err);
      toast.error(err?.data?.message || (language === 'thai' ? 'อัปเดตข้อมูลล้มเหลว' : 'Failed to update order.'));
    } finally {
  ...
  useEffect(() => {
    if (data?.success && data.data) {
      ...
      for (let i = 1; i <= 10; i++) {
        ...
        if (order[key]) images.push({ url: getFullUrl(order[key]), raw: order[key] });
      }
  ...
  if (error) return <div className="p-6"><Message variant="danger">{error?.data?.message || error.message}</Message></div>;
    ...
    <div className="min-h-screen bg-slate-50/50 pb-24 font-sans selection:bg-indigo-100 selection:text-indigo-900">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-8">
        ...
        <div className="flex items-center gap-4 mb-8">
          <button onClick={() => navigate(-1)} className="w-10 h-10 bg-white rounded-full flex items-center justify-center text-slate-600 shadow-sm border border-slate-200 hover:bg-slate-50 hover:text-indigo-600 transition-colors">
            <FaArrowLeft />
          </button>
          <div>
            <h1 className="text-2xl font-bold text-slate-900 tracking-tight">
              {language === 'thai' ? 'แก้ไขใบสั่งผลิต Custom PCB' : 'Edit Custom PCB Order'}
            </h1>
        ...
        <form onSubmit={submitHandler} className="flex flex-col lg:flex-row gap-6">
          <div className="flex-1 space-y-6">
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-slate-200/60 overflow-hidden">
              <div className="p-6 border-b border-slate-100 bg-slate-50/30">
                <h2 className="text-lg font-bold text-slate-800 flex items-center gap-2">
                  <FaFileArchive className="text-indigo-500" /> Project Information
                </h2>
              </div>
              <div className="p-6">
                ...
                {/* Diagram Images */}
                <div className="mb-6">
                  <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Diagram Images (Max 10)</label>
                  <label className="cursor-pointer group flex flex-col items-center justify-center border-2 border-dashed border-slate-200 rounded-2xl p-8 bg-slate-50 hover:bg-slate-100 hover:border-indigo-300 transition-all">
                    ...
                    <span className="text-sm font-semibold text-slate-700 group-hover:text-indigo-600 transition-colors">Click to Upload Images</span>
                    <span className="text-xs text-slate-400 mt-1">Support JPG, PNG</span>
                    ...
                    <div className="flex flex-wrap gap-4 mt-4">
                      <AnimatePresence>
                        {diagramImages.map((img, idx) => (
                          ...
                          >
                            <img src={img.url} alt={`Diagram ${idx + 1}`} className="w-full h-full object-cover" />
                            <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                              <button type="button" onClick={() => removeImage(idx)} className="w-8 h-8 bg-rose-500 text-white rounded-full flex items-center justify-center hover:bg-rose-600 hover:scale-110 transition-all shadow-lg">
                                <FaTrash size={12} />
                              </button>
                            </div>
                          </motion.div>
                ...
                {/* Gerber ZIP */}
                <div className="mb-6">
                  <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Gerber / Design Files (ZIP)</label>
                  <div className="p-4 rounded-xl border border-slate-200 bg-slate-50">
                    <input type="file" accept=".zip,.rar" onChange={(e) => handleChange('zipFile', e.target.files[0])} className="text-sm text-slate-700 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-xs file:font-bold file:bg-indigo-50 file:text-indigo-700 hover:file:bg-indigo-100 transition-all cursor-pointer w-full focus:outline-none" />
                    {formData.zipFile ? (
                      ...
                      <div className="mt-3 text-sm flex items-center gap-2">
                        ...
                        <a href={getFullUrl(existingZip)} target="_blank" rel="noreferrer" className="font-bold text-indigo-600 hover:text-indigo-800 hover:underline flex items-center gap-1">
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-slate-200/60 overflow-hidden">
              <div className="p-6 border-b border-slate-100 bg-slate-50/30">
                <h2 className="text-lg font-bold text-slate-800 flex items-center gap-2">
                  <FaTruck className="text-indigo-500" /> Contact & Shipping Details
                </h2>
              </div>
              <div className="p-6 grid lg:grid-cols-2 gap-8">
                ...
                <div className="space-y-6">
                  <div>
                    ...
                    <InputField label="Name" value={formData.userName} onChange={(val) => handleChange('userName', val)} />
                    <InputField label="Email" type="email" value={formData.userEmail} onChange={(val) => handleChange('userEmail', val)} />
                  </div>
                  <div>
                    <h3 className="text-sm font-bold text-indigo-600 uppercase tracking-wider mb-4 border-b border-indigo-100 pb-2 flex items-center gap-2">
                      <FaFileInvoiceDollar /> Billing Info
                    </h3>
                    <InputField label="Name" value={formData.billingName} onChange={(val) => handleChange('billingName', val)} />
                    <InputField label="Phone" value={formData.billingPhone} onChange={(val) => handleChange('billingPhone', val)} />
                    <InputField label="Address" value={formData.billinggAddress} onChange={(val) => handleChange('billinggAddress', val)} />
                    <InputField label="Tax ID" value={formData.tax} onChange={(val) => handleChange('tax', val)} />
                  </div>
                </div>
                ...
                <div>
                  ...
                  <InputField label="Name" value={formData.shippingName} onChange={(val) => handleChange('shippingName', val)} />
                  <InputField label="Phone" value={formData.shippingPhone} onChange={(val) => handleChange('shippingPhone', val)} />
                  <InputField label="Address" value={formData.shippingAddress} onChange={(val) => handleChange('shippingAddress', val)} />
                  <div className="grid grid-cols-2 gap-4">
                    <InputField label="City" value={formData.shippingCity} onChange={(val) => handleChange('shippingCity', val)} />
                    <InputField label="Postal Code" value={formData.shippingPostalCode} onChange={(val) => handleChange('shippingPostalCode', val)} />
                  </div>

                  <div className="mt-2">
                    <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Receive Place</label>
                    <select
                      value={formData.receivePlace}
                      onChange={(e) => handleChange('receivePlace', e.target.value)}
          ...
          <div className="w-full lg:w-80 shrink-0">
            <div className="bg-white rounded-2xl shadow-sm border border-slate-200/60 p-6 sticky top-6">
              <h2 className="text-lg font-bold text-slate-800 mb-6 flex items-center gap-2 border-b border-slate-100 pb-4">
                <FaRegCreditCard className="text-indigo-500" /> Order Configuration
              </h2>
              ...
              <div className="space-y-5 mb-8">
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => handleChange('pcbQty', parseInt(e.target.value) || 1)}
              ...
              <div className="space-y-3">
                ...
                >
                  {isLoadingSubmit || updateLoading ? <Loader /> : (
                    <>
