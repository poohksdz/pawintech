...
const CustomPCBDetailScreen = () => {
  ...
  const t = {
    en: {
      ...
      customerselectedtoreceiveatcompanyLbl: 'Customer selected to receive at company',
    },
    thai: {
  ...
  const downloadAllImages = async (e) => {
    ...
    saveAs(content, `${orderData.projectname || 'images'}-images-pawin-tech.zip`);
  };
  ...
  const handleDownloadAll = async () => {
    ...
    if (orderData.dirgram_zip) {
      ...
      try {
        ...
        if (response.ok) {
          const blob = await response.blob();
          zip.file(orderData.dirgram_zip.split('/').pop() || 'technical-files.zip', blob);
        }
      } catch (err) {
    ...
    const content = await zip.generateAsync({ type: 'blob' });
    saveAs(content, `${orderData.projectname || 'project'}-project-pawin-tech.zip`);
  };
          ...
          <ListGroup variant="flush">
            {(orderData.receivePlace === 'bysending') ? (
              <ListGroup.Item>
                ...
                <p>
                  <strong>{t.addressLbl}:</strong> {orderData.shippingAddress}, {orderData.shippingCity},{' '}
                  {orderData.shippingPostalCode}, {orderData.shippingCountry}
                </p>
              </ListGroup.Item>
            ) : (
              <ListGroup.Item>
            ...
            <ListGroup.Item>
              ...
              <p>
                <strong>{t.addressLbl}:</strong> {orderData.billingAddress}, {orderData.billingCity},{' '}
                {orderData.billingPostalCode}, {orderData.billingCountry}
              </p>
              <p>
            ...
            <ListGroup.Item>
              ...
              {orderData.isDelivered ? (
                <Message variant="success">
                  {t.deliveredOnLbl}{' '}
                  {orderData.deliveryOn ? format(new Date(orderData.deliveryOn), 'PPpp') : '-'}, {t.deliveryID}:
                  ...
                  <table className="table table-bordered">
                    ...
                    <tbody>
                      <tr>
                        ...
                        <td>{orderData.confirmed_price && orderData.pcb_qty ? (orderData.confirmed_price / orderData.pcb_qty).toFixed(2) : '-'} ฿</td>
                  ...
                  <Row className="align-items-center">
                    <Col md={6}>
                      ...
                      {orderData.transferedDate ? (
                        <h5>{format(new Date(orderData.transferedDate), 'PPpp')}</h5>
                      ) : (
                        <h5>-</h5>
                      )}
                    </Col>
