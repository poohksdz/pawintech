...
const CustomPCBShippingScreen = () => {
    ...
    const t = {
        en: {
            ...
            receiptShort: 'Short Receipt', receiptFull: 'Full Tax Invoice',
            noAddress: 'No address set in profile', orderIdNotFound: 'Error: Order ID not found',
        ...
        thai: {
            ...
            receiptShort: 'ใบเสร็จรับเงินย่อ', receiptFull: 'ใบกำกับภาษีเต็มรูป',
            noAddress: 'ไม่พบที่อยู่ในโปรไฟล์', orderIdNotFound: 'ข้อผิดพลาด: ไม่พบ ID คำสั่งซื้อ',
                ...
                <form onSubmit={submitHandler} className="space-y-6">
                    <motion.div layout className="bg-white rounded-[2rem] shadow-sm border border-slate-100 overflow-hidden">
                        <div className="bg-slate-50/50 px-8 py-5 border-b border-slate-100 flex items-center gap-3 text-start">
                            <FaTruck className="text-blue-600" /> <h3 className="font-bold text-slate-800">{t.receiveMethod}</h3>
                        </div>
                    ...
                    <AnimatePresence mode="wait">
                        {!isReceiveCompleteSelected ? (
                                        ...
                                        {addressSource === 'profile' ? (
                                            <div className="bg-blue-50/50 border border-blue-100 rounded-2xl p-6 relative overflow-hidden group text-start">
                                                <div className="relative z-10 space-y-3 font-medium text-start">
                                                    <p className="text-xs font-black text-blue-400 uppercase tracking-widest">{t.profileAddrTitle}</p>
                                                    ...
                                                    <div className="flex items-start gap-3 text-slate-500 text-sm leading-relaxed text-start">
                                                        ...
                                                        {userInfo.shippingAddress?.address ? `${userInfo.shippingAddress.address}, ${userInfo.shippingAddress.city}, ${userInfo.shippingAddress.postalCode}` : `⚠️ ${t.noAddress}`}
                                                    </div>
                                                </div>
                                            </div>
                                        ) : (
                            ...
                            <motion.div key="pickup" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="bg-white rounded-[2rem] shadow-sm border border-slate-100 p-8 space-y-6 text-start">
                                <div className="flex items-center gap-3 p-4 bg-blue-50 border border-blue-100 text-blue-700 rounded-2xl text-sm font-bold animate-pulse text-start"><FaInfoCircle /> {t.pickupInfo}</div>
                    ...
                    <motion.div layout className="bg-white rounded-[2rem] shadow-sm border border-slate-100 overflow-hidden">
                        ...
                        <div className="p-8 space-y-6 text-start">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <SelectionCard icon={<FaFileInvoice />} title={t.receiptShort} active={!isBillingCompleteSelected} onClick={() => setIsBillingCompleteSelected(false)} />
                                <SelectionCard icon={<FaBuilding />} title={t.receiptFull} active={isBillingCompleteSelected} onClick={() => setIsBillingCompleteSelected(true)} />
                            </div>
                            <AnimatePresence>
                                {isBillingCompleteSelected && (
                                    <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="pt-6 border-t border-slate-100 space-y-6 overflow-hidden text-start">
                                        <h4 className="text-[10px] font-black text-blue-600 uppercase tracking-[0.2em] pt-2 text-start">{t.fillBilling}</h4>
