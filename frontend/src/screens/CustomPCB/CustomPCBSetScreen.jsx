...
const CustomPCBSetScreen = () => {
  ...
  const translations = {
    en: {
      ...
      phNotes: 'Describe functionality, board size constraints, or specific components needed...',
      qty: 'Quantity (pcs)',
      submit: 'Send Request',
    ...
    thai: {
      ...
      subtitle: 'เปลี่ยนไอเดียของคุณให้เป็นบอร์ดจริง พร้อมคำแนะนำจากผู้เชี่ยวชาญ',
  ...
  const orderNowHandler = async (e) => {
    e.preventDefault();
    if (!userInfo) { toast.error(t.errLogin); return navigate('/login'); }
    ...
    <div className="min-h-screen bg-white pb-16 font-sans text-slate-900 antialiased selection:bg-slate-900 selection:text-white">
      ...
      {isSubmitting && (
        <div className="fixed inset-0 z-[9999] bg-white/90 backdrop-blur-sm flex flex-col items-center justify-center">
          ...
          <h5 className="mt-4 text-lg font-bold text-black animate-pulse">{t.submitting}</h5>
        </div>
      )}
        ...
        <form onSubmit={orderNowHandler}>
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8 items-start">
            ...
            <div className="lg:col-span-8 space-y-6">
              ...
              <div className="bg-white rounded-3xl border border-slate-100 overflow-hidden shadow-[0_1px_3px_rgba(0,0,0,0.02)]">
                <div className="bg-slate-50/50 border-b border-slate-100 px-8 py-5 flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <h3 className="font-black text-black text-xs uppercase tracking-widest m-0">Project Information</h3>
                  </div>
                </div>
                <div className="p-8">
                  ...
                  <div>
                    <label className="block text-sm font-bold text-slate-700 mb-2">{t.qty}</label>
                    <div className="flex items-center gap-4">
                      <div className="flex items-center bg-slate-50 border border-slate-100 rounded-2xl p-1.5">
                        <button type="button" onClick={() => changeQty(-1)} disabled={pcbQty <= 5} className="w-11 h-11 flex items-center justify-center text-slate-400 hover:text-black hover:bg-white hover:shadow-sm rounded-xl transition-all disabled:opacity-20">
                          <FaMinus className="text-xs" />
                        </button>
                        <div className="w-20 text-center font-black text-xl text-black">
                          {pcbQty}
                        </div>
                        <button type="button" onClick={() => changeQty(1)} className="w-11 h-11 flex items-center justify-center text-slate-400 hover:text-black hover:bg-white hover:shadow-sm rounded-xl transition-all">
                          <FaPlus className="text-xs" />
                        </button>
                      </div>
                      <span className="text-slate-400 text-xs font-bold uppercase tracking-wider">({t.minQty})</span>
                    </div>
                  </div>
              ...
              <div className="bg-white rounded-3xl border border-slate-100 overflow-hidden shadow-[0_1px_3px_rgba(0,0,0,0.02)]">
                <div className="bg-slate-50/50 border-b border-slate-100 px-8 py-5 flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <h3 className="font-black text-black text-xs uppercase tracking-widest m-0">Attachments</h3>
                  </div>
                </div>
                <div className="p-8">
                  ...
                  <div className="mb-6">
                    <div className="flex justify-between items-center mb-2">
                      ...
                      <span className="text-xs text-slate-400">{t.supportedImg}</span>
                    </div>
                    ...
                    >
                      ...
                      <div className="pointer-events-none flex flex-col items-center">
                        ...
                        <h6 className="font-black text-black text-sm uppercase tracking-widest mb-2">{t.dragDrop}</h6>
                        <p className="text-xs text-slate-400 font-medium">{t.supportedImg}</p>
                      </div>
                    </div>
                  ...
                  <div>
                    <div className="flex justify-between items-center mb-2">
                      <label className="text-sm font-bold text-slate-700 m-0">{t.diagramZip}</label>
                      <span className="text-xs text-slate-400">{t.supportedZip}</span>
                    </div>
                    ...
                    <div className="relative">
                      <input
                        ...
                        className="hidden"
                      />
                      <label htmlFor="zip-upload" className={`flex items-center w-full border ${zipFile ? 'border-black bg-slate-50' : 'border-slate-100 bg-slate-50/50 hover:bg-slate-50 hover:border-slate-200'} rounded-2xl p-2.5 cursor-pointer transition-all`}>
                        ...
                        <span className={`text-xs truncate ${zipFile ? 'text-black font-black uppercase tracking-tight' : 'text-slate-300'}`}>
              ...
              <div className="bg-white rounded-3xl border border-slate-100 overflow-hidden shadow-[0_1px_3px_rgba(0,0,0,0.02)]">
                <div className="p-8">
                  <label className="block text-xs font-black text-black uppercase tracking-widest mb-4">Requirements & Descriptions</label>
                  <textarea
                    rows={5}
            ...
            <div className="lg:col-span-4">
              <div className="sticky top-24 bg-black rounded-3xl shadow-2xl overflow-hidden border border-white/10 ring-1 ring-white/5">
                ...
                <div className="bg-white/5 border-b border-white/10 p-8">
                  <h3 className="text-white font-black text-xs uppercase tracking-widest m-0">{t.summary}</h3>
                </div>

                <div className="p-8 flex flex-col gap-6">
                  {/* Details List */}
                  <div className="flex justify-between items-start gap-4">
                    <span className="text-slate-500 text-[10px] font-black uppercase tracking-widest">{t.projectName}:</span>
                  ...
                  <div className="flex justify-between items-center text-[10px] font-black uppercase tracking-widest">
                    ...
                    <span className="text-white">{diagramImages.length + (zipFile ? 1 : 0)} Files</span>
                  </div>
                  ...
                  <div className="bg-white/5 rounded-2xl p-6 text-center border border-white/5">
                    <span className="text-slate-500 text-[9px] font-black uppercase tracking-[0.2em] block mb-2">{t.estPrice}</span>
                    <h4 className="text-white font-black text-2xl tracking-tighter m-0 uppercase">Waiting Quote</h4>
                  </div>
                  ...
                  <div className="mt-2 space-y-3">
                    <div className="flex items-center text-slate-400 text-[9px] font-black uppercase tracking-widest">
                      <BsShieldCheck className="mr-3 text-white" size={14} /> {t.secure}
                    </div>
                    <div className="flex items-center text-slate-400 text-[9px] font-black uppercase tracking-widest">
                      <BsClockHistory className="mr-3 text-white" size={14} /> {t.fastReview}
                    </div>
                  </div>
                </div>
              </div>
