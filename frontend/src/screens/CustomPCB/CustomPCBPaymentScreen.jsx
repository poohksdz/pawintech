...
const CustomPCBPaymentScreen = () => {
  ...
  const { data: cartData, isLoading: isCartLoading } =
    useGetCustomcartByIdQuery(orderId, {
      skip: !orderId,
    });
  ...
  const submitHandler = async (e) => {
    ...
    if (!orderId)
      return toast.error("ไม่สามารถชำระเงินได้เนื่องจากไม่พบรหัสคำสั่งซื้อ");
    if (finalAmount <= 0)
              ...
              >
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6 text-start">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <div className="w-full bg-slate-100 border border-slate-200 rounded-2xl px-4 py-3 text-sm font-black text-blue-600">
                      {Number(finalAmount).toLocaleString()} ฿
                    </div>
                  </div>
                  <div className="space-y-1.5">
                ...
                <div className="mt-4 flex justify-center items-center gap-2 text-slate-400 text-[10px] font-black uppercase tracking-[0.2em]">
                  <FaShieldAlt className="text-blue-500" /> Secure Payment
                  Processing
                </div>
              </form>
            </div>
