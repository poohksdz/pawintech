...
import {
  FaCheck, FaEdit, FaSearch, FaEye, FaMicrochip,
  FaTimes, FaFilter, FaSync, FaSave,
  FaTrashAlt, FaExclamationTriangle, FaChevronRight,
  FaBoxOpen, FaClock
} from 'react-icons/fa';
...
const CustomPCBCartListScreen = () => {
  ...
  // 1. Fetch Data
  const { data, isLoading, error, refetch } = useGetAllCustomcartsQuery(userInfo?._id, {
    skip: !userInfo?._id,
  });

  // 2. Mutations
  const [updateStatus, { isLoading: isStatusUpdating }] = useUpdateCustomcartComfirmStatusMutation();
  ...
  // State forms
  const [editFormData, setEditFormData] = useState({ confirmed_price: 0, status: 'pending' });
  ...
  const translations = {
    en: {
      ...
      headers: { id: 'ID', project: 'Project', qty: 'Qty', price: 'Price', date: 'Date', status: 'Status', actions: 'Actions' },
      status: { pending: 'Pending', accepted: 'Approved', rejected: 'Rejected' },
      noData: 'No requests found.',
      btn: { confirm: 'Approve', reject: 'Reject', edit: 'Edit', detail: 'View Details', save: 'Save Price', cancel: 'Cancel', confirmAction: 'Confirm Approval', delete: 'Delete' },
      ...
      approveDesc: 'Verify the price before approval. Client will be notified to proceed with payment.'
    },
    thai: {
      ...
      statusAll: 'ทุกสถานะ',
      headers: { id: 'ID', project: 'โปรเจกต์', qty: 'จำนวน', price: 'ราคา', date: 'วันที่', status: 'สถานะ', actions: 'จัดการ' },
      status: { pending: 'รอตรวจสอบ', accepted: 'อนุมัติแล้ว', rejected: 'ปฏิเสธ' },
      noData: 'ไม่พบรายการคำขอ',
      btn: { confirm: 'อนุมัติ', reject: 'ปฏิเสธ', edit: 'แก้ไข', detail: 'ดูข้อมูล', save: 'บันทึกราคา', cancel: 'ยกเลิก', confirmAction: 'ยืนยันการอนุมัติ', delete: 'ลบ' },
      ...
      approveDesc: 'กรุณาตรวจสอบราคาก่อนอนุมัติ ลูกค้าจะสามารถชำระเงินได้ทันทีหลังจากการอนุมัติ'
    }
  };
  ...
  const handleDeleteAction = async () => {
    try {
      ...
      toast.success(language === 'thai' ? 'ลบรายการเรียบร้อยแล้ว' : 'Record deleted');
      refetch();
  ...
  const handleApprove = async () => {
    try {
      ...
      toast.success(language === 'thai' ? 'อนุมัติรายการเรียบร้อย' : 'Quote Approved');
      refetch();
  ...
  const handleSaveEdit = async () => {
    try {
      ...
      toast.success(language === 'thai' ? 'บันทึกราคาเรียบร้อย' : 'Price updated');
      refetch();
  ...
  const filteredOrders = useMemo(() => {
    const ordersRaw = data?.data || data?.orders || (Array.isArray(data) ? data : []) || [];
  ...
  if (isLoading && !data) return <div className="min-h-screen flex items-center justify-center bg-[#fcfdfe]"><Loader /></div>;
    ...
    <div className="min-h-screen bg-[#fcfdfe] pb-24 font-prompt antialiased">
      <div className="px-4 lg:px-12 py-10 max-w-[1600px] mx-auto">
        ...
        >
          ...
          <div className="flex flex-wrap items-center gap-4 bg-white/60 backdrop-blur-xl p-4 rounded-3xl border border-slate-200/50 shadow-xl shadow-slate-100">
            <div className="relative group min-w-[300px]">
              <FaSearch className="absolute left-5 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-black transition-colors" size={14} />
              <input
                type="text"
            ...
            <div className="relative group min-w-[180px]">
              <FaFilter className="absolute left-5 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-black transition-colors" size={12} />
              <select
        ...
        {error ? (
          <div className="bg-white rounded-[3rem] p-12 shadow-xl border border-rose-100">
            <Message variant="danger">{error?.data?.message || 'Failed to load quotes'}</Message>
          </div>
          ...
          >
            ...
            <h3 className="text-2xl font-black text-slate-400 uppercase tracking-widest">{t.noData}</h3>
          </motion.div>
        ) : (
              ...
              <table className="w-full border-collapse">
                <thead>
                  <tr className="bg-slate-50/80 border-b border-slate-100">
                    <th className="py-8 px-10 text-[10px] font-black text-slate-400 uppercase tracking-[0.3em] text-center w-24">#</th>
                    <th className="py-8 px-6 text-[10px] font-black text-slate-400 uppercase tracking-[0.3em]">{t.headers.project}</th>
                    <th className="py-8 px-6 text-[10px] font-black text-slate-400 uppercase tracking-[0.3em] text-center">{t.headers.qty}</th>
                    <th className="py-8 px-6 text-[10px] font-black text-slate-400 uppercase tracking-[0.3em] text-right">{t.headers.price}</th>
                    <th className="py-8 px-6 text-[10px] font-black text-slate-400 uppercase tracking-[0.3em] text-center">{t.headers.date}</th>
                    <th className="py-8 px-6 text-[10px] font-black text-slate-400 uppercase tracking-[0.3em] text-center">{t.headers.status}</th>
                    <th className="py-8 px-10 text-[10px] font-black text-slate-400 uppercase tracking-[0.3em] text-center">{t.headers.actions}</th>
                  </tr>
                </thead>
                <motion.tbody
                    ...
                    >
                      ...
                      <td className="py-8 px-6 text-center">
                        <div className="inline-flex flex-col items-center">
                          ...
                          <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest opacity-50">PCS</span>
                        </div>
                      </td>
                      <td className="py-8 px-6 text-right">
                        <div className="flex flex-col items-end">
                          <span className={`text-[19px] font-black leading-none ${order.status === 'accepted' ? 'text-emerald-600' : 'text-slate-300'}`}>
                            {formatPrice(order.confirmed_price)}
                          </span>
                          {order.confirmed_price > 0 && (
                            <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest mt-2 opacity-50">Incl. VAT</span>
                          )}
                        </div>
                      </td>
                      <td className="py-8 px-6 text-center">
                        <div className="flex flex-col items-center">
                          <span className="text-[13px] font-black text-slate-600 leading-none mb-1">
                            {new Date(order.created_at || order.createdAt).toLocaleDateString(language === 'thai' ? 'th-TH' : 'en-US', { day: '2-digit', month: 'short' })}
                          </span>
                          <span className="text-[10px] font-bold text-slate-300 uppercase tracking-tighter">
                            {new Date(order.created_at || order.createdAt).getFullYear()}
                          </span>
                        </div>
                      </td>
                      <td className="py-8 px-6 text-center">
                      ...
                      <td className="py-8 px-10 text-center">
                        <div className="flex items-center justify-center gap-3">
                          <AnimatePresence mode="wait">
                            {order.status === 'pending' && (
                              ...
                              >
                                ...
                                <button
                                  onClick={() => { setSelectedOrderId(order.id || order._id); setShowRejectModal(true); }}
                          ...
                          <button
                            onClick={() => { setSelectedOrderId(order.id || order._id); setShowDeleteModal(true); }}
      ...
      {showConfirmModal && createPortal(
        <div className="fixed inset-0 z-[10000] flex items-center justify-center p-4">
          ...
          <motion.div initial={{ scale: 0.9, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.9, opacity: 0, y: 20 }} className="relative bg-white rounded-[3.5rem] shadow-3xl w-full max-w-lg p-14 text-center border border-white/20 font-prompt overflow-hidden z-10">
            ...
            <h2 className="text-3xl font-black text-slate-900 mb-4 tracking-tight uppercase leading-none">{t.approveModalTitle}</h2>
            <p className="text-slate-500 font-bold mb-10 text-sm leading-relaxed max-w-[90%] mx-auto opacity-70 uppercase tracking-widest">{t.approveDesc}</p>

            <div className="bg-slate-50/80 rounded-[2.5rem] p-10 border border-slate-100 mb-12 text-start">
              <label className="text-[11px] font-black text-slate-400 uppercase tracking-[0.3em] block mb-6 px-1">{t.form.priceLabel}</label>
            ...
            <div className="flex gap-4">
              <button onClick={() => setShowConfirmModal(false)} className="flex-1 py-5 rounded-2xl bg-slate-100 text-slate-500 font-black text-[11px] uppercase tracking-[0.2em] hover:bg-slate-200 transition-all border-none">
                {t.btn.cancel}
              </button>
              <button onClick={handleApprove} disabled={isStatusUpdating} className="flex-[1.8] py-5 rounded-2xl bg-black text-white font-black text-[11px] uppercase tracking-[0.2em] shadow-2xl hover:bg-slate-900 active:scale-95 transition-all flex items-center justify-center gap-4 border-none">
                {isStatusUpdating ? <Loader /> : <>{t.btn.confirmAction} <FaChevronRight size={10} /></>}
              </button>
            </div>
          </motion.div>
        </div>,
      ...
      {showEditModal && createPortal(
        <div className="fixed inset-0 z-[10000] flex items-center justify-center p-4">
          <AnimatePresence>
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setShowEditModal(false)} className="absolute inset-0 bg-slate-900/60 backdrop-blur-md" />
          </AnimatePresence>
          <motion.div initial={{ scale: 0.9, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.9, opacity: 0, y: 20 }} className="relative bg-white rounded-[3.5rem] shadow-3xl w-full max-w-lg p-14 text-center border border-white/20 font-prompt overflow-hidden z-10">
            ...
            <h2 className="text-3xl font-black text-slate-900 mb-4 tracking-tight uppercase leading-none">{t.editModalTitle}</h2>
            <p className="text-slate-500 font-bold mb-10 text-xs uppercase tracking-[0.2em]">{language === 'thai' ? 'ปรับเปลี่ยนราคาสำหรับรายการคำขอนี้' : 'ADJUST PRICE FOR THIS REQUEST'}</p>
            ...
            <div className="bg-slate-50/80 rounded-[2.5rem] p-10 border border-slate-100 mb-12 text-start">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  onChange={(e) => setEditFormData({ ...editFormData, confirmed_price: e.target.value })}
                />
              </div>
            </div>

            <div className="flex gap-4">
              <button onClick={() => setShowEditModal(false)} className="flex-1 py-5 rounded-2xl bg-slate-100 text-slate-500 font-black text-[11px] uppercase tracking-[0.2em] hover:bg-slate-200 transition-all border-none">
                {t.btn.cancel}
              </button>
              <button onClick={handleSaveEdit} className="flex-[1.8] py-5 rounded-2xl bg-black text-white font-black text-[11px] uppercase tracking-[0.2em] shadow-2xl hover:bg-slate-900 transition-all flex items-center justify-center gap-4 border-none">
      ...
      {showDeleteModal && createPortal(
        <div className="fixed inset-0 z-[10000] flex items-center justify-center p-4">
          ...
          <motion.div initial={{ scale: 0.9, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.9, opacity: 0, y: 20 }} className="relative bg-white rounded-[3rem] shadow-3xl w-full max-w-sm p-12 text-center border border-white/20 font-prompt z-10">
            ...
            <h3 className="text-2xl font-black text-slate-900 mb-3 uppercase tracking-tight leading-none">{t.deleteModalTitle}</h3>
            <p className="text-sm text-slate-500 mb-10 font-medium leading-relaxed">{t.deleteConfirm}</p>
            <div className="flex gap-4">
              <button onClick={() => setShowDeleteModal(false)} className="flex-1 py-4 bg-slate-100 text-slate-500 font-black text-[11px] uppercase tracking-widest hover:bg-slate-200 transition-all rounded-2xl border-none">{t.btn.cancel}</button>
              <button onClick={handleDeleteAction} className="flex-1 py-4 bg-rose-600 text-white font-black text-[11px] uppercase tracking-widest hover:bg-rose-700 transition-all shadow-xl shadow-rose-200 rounded-2xl border-none">{t.btn.delete}</button>
            </div>
          </motion.div>
        </div>,
        document.body
      ...
      {showRejectModal && createPortal(
        <div className="fixed inset-0 z-[10000] flex items-center justify-center p-4">
          <AnimatePresence>
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setShowRejectModal(false)} className="absolute inset-0 bg-slate-900/60 backdrop-blur-md" />
          </AnimatePresence>
          <motion.div initial={{ scale: 0.9, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.9, opacity: 0, y: 20 }} className="relative bg-white rounded-[3rem] shadow-3xl w-full max-w-sm p-12 text-center border border-white/20 font-prompt z-10">
            ...
            <h3 className="text-2xl font-black text-slate-900 mb-3 uppercase tracking-tight leading-none">{t.rejectModalTitle}</h3>
            <p className="text-sm text-slate-500 mb-10 font-medium leading-relaxed">{t.rejectConfirm}</p>
            <div className="flex gap-4">
              <button onClick={() => setShowRejectModal(false)} className="flex-1 py-4 bg-slate-100 text-slate-500 font-black text-[11px] uppercase tracking-widest hover:bg-slate-200 transition-all rounded-2xl border-none">{t.btn.cancel}</button>
              <button onClick={handleRejectAction} className="flex-1 py-4 bg-rose-500 text-white font-black text-[11px] uppercase tracking-widest hover:bg-rose-600 transition-all shadow-xl shadow-rose-100 rounded-2xl border-none">{t.btn.reject}</button>
            </div>
          </motion.div>
        </div>,
        document.body
