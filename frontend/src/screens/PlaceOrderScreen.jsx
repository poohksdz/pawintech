...
import {
  FaMapMarkerAlt, FaFileInvoice, FaBoxOpen, FaMoneyBillWave,
...
const PlaceOrderScreen = () => {
  ...
  }, [cart.paymentTransfer, cart.shippingAddress.address, cart.billingAddress.billinggAddress, navigate]);
  ...
  const AddressCard = ({ title, icon, data, isPickup = false }) => (
    <div className="bg-white rounded-3xl p-6 border border-slate-100 shadow-sm flex flex-col h-full hover:shadow-md transition-shadow duration-300">
      <div className="flex items-center gap-3 mb-4">
        <div className={`p-2 rounded-lg ${title === t.shipping ? 'bg-slate-100 text-black' : 'bg-slate-100 text-black'}`}>
          {icon}
        </div>
      ...
      {isPickup ? (
        ...
        <div className="space-y-2">
          <p className="text-slate-900 font-bold">{data.shippingname || data.billingName}</p>
          <p className="text-slate-500 text-sm leading-relaxed">
            {data.address || data.billinggAddress}<br />
            {data.city || data.billingCity}, {data.postalCode || data.billingPostalCode}<br />
          ...
          <div className="flex items-center gap-2 text-xs font-bold text-slate-400 mt-3 pt-3 border-t border-slate-50">
            ...
            {data.tax && data.tax !== 'N/A' && <span className="ml-auto text-black">Tax: {data.tax}</span>}
          </div>
        </div>
      )}
    </div>
            ...
            <div className="bg-white rounded-[2.5rem] shadow-sm border border-slate-100 overflow-hidden">
              ...
              <div className="p-0">
                {cart.cartItems.length === 0 ? (
                  <div className="p-12 text-center text-slate-400 font-medium">{t.emptyCart}</div>
                ) : (
                  <div className="divide-y divide-slate-50">
                    {cart.cartItems.map((item, index) => (
                      <div key={index} className="p-6 flex items-center gap-6 hover:bg-slate-50/50 transition-colors group">
                        <div className="w-20 h-20 shrink-0 rounded-2xl overflow-hidden border border-slate-100 bg-white p-1">
                          <img src={item.image} alt={item.name} className="w-full h-full object-contain" />
                        </div>
                        <div className="flex-grow min-w-0">
                          <Link to={`/product/${item.product}`} className="block text-slate-900 font-bold hover:text-black transition-colors truncate">
                            {item.name}
                          </Link>
                          <p className="text-xs font-bold text-slate-400 mt-1 uppercase tracking-tighter">REF: {item.product}</p>
                        </div>
                        <div className="text-right whitespace-nowrap">
                          <p className="text-xs font-bold text-slate-400">{item.qty} x {safeFixed(item.price)} ฿</p>
                          <p className="text-sm font-black text-slate-900 mt-0.5">{safeFixed(item.qty * item.price)} ฿</p>
                        </div>
                      </div>
                    ))}
                  </div>
            ...
            <div className="bg-slate-900 rounded-[2.5rem] text-white shadow-2xl overflow-hidden border border-white/10 ring-1 ring-white/5">
              <div className="p-8">
                <div className="flex items-center gap-3 mb-8">
                  <div className="p-2 bg-black rounded-xl shadow-lg shadow-black/30"><FaMoneyBillWave size={18} /></div>
                  <h2 className="text-xl font-black tracking-tight uppercase">{t.summary}</h2>
                </div>

                <div className="space-y-4 mb-10 text-sm font-medium">
                  <div className="flex justify-between items-center text-slate-400">
                    <span>{t.itemsCost}</span>
                    <span className="text-white text-lg font-bold">{safeFixed(cart.itemsPrice)} ฿</span>
                  </div>
                  <div className="flex justify-between items-center text-slate-400">
                    <span>{t.shippingCost}</span>
                    <span className={`text-lg font-bold ${Number(cart.shippingPrice) === 0 ? 'text-emerald-400' : 'text-white'}`}>
                      {Number(cart.shippingPrice) === 0 ? 'FREE' : `${safeFixed(cart.shippingPrice)} ฿`}
                    </span>
                  </div>
                  <div className="flex justify-between items-center text-slate-400">
                    <span>{t.vat}</span>
                    <span className="text-white text-lg font-bold">{safeFixed(cart.vatPrice)} ฿</span>
                  </div>
                  <div className="pt-6 border-t border-white/10 mt-6 flex justify-between items-end">
                    <span className="text-sm font-black text-white/60 uppercase tracking-widest">{t.total}</span>
                    <div className="text-right">
                      <span className="text-4xl font-black">{safeFixed(cart.totalPrice)}</span>
                      <span className="text-xs ml-1 text-slate-400 font-bold uppercase tracking-widest">THB</span>
                    </div>
                  </div>
                </div>
