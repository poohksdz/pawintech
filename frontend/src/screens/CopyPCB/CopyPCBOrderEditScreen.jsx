...
const CopyPCBOrderEditScreen = () => {
  ...
  const [updatecopyPCB, { isLoading: loading }] = useUpdatecopyPCBMutation();
  ...
  const handleImageUpload = (e, type) => {
    ...
    if (type === "front") {
      setFormData((prev) => ({
        ...prev,
        frontImages: [...prev.frontImages, ...files],
      }));
    } else {
      setFormData((prev) => ({
        ...prev,
        backImages: [...prev.backImages, ...files],
      }));
    }
  };
            ...
            <Card className="p-3 my-3">
              ...
              {existingZip && !formData.zipFile && (
                <div className="text-muted mt-2">
                  Existing ZIP: {existingZip.split("/").pop()}
                </div>
              )}
            </Card>
