...
import { FaSearch, FaFilter, FaCheck, FaEdit, FaEye, FaSync, FaLayerGroup, FaChevronRight, FaBox, FaClock, FaCheckCircle, FaTimesCircle } from 'react-icons/fa';
...
const CopyPCBCartListScreen = () => {
  ...
  const { data: rawData, isLoading, error, refetch } = useGetAllcopycartsQuery();
  ...
  const StatusBadge = ({ status }) => {
    const configs = {
      pending: { bg: 'bg-amber-50', text: 'text-amber-600', icon: <FaClock className="animate-pulse" />, label: t.pending },
      accepted: { bg: 'bg-emerald-50', text: 'text-emerald-600', icon: <FaCheckCircle />, label: t.accepted },
      rejected: { bg: 'bg-rose-50', text: 'text-rose-600', icon: <FaTimesCircle />, label: t.rejected },
    };
    const config = configs[status] || configs.pending;

    return (
      <div className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-[10px] font-bold uppercase tracking-wider ${config.bg} ${config.text}`}>
        {config.icon}
        {config.label}
      </div>
    );
      ...
      <Container className="py-8">
            ...
            <Col lg={4}>
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-xs font-bold text-slate-400 mt-1 uppercase tracking-widest">Management Dashboard</p>
                </div>
              </div>
            </Col>
            <Col lg={8}>
              <div className="flex flex-wrap items-center justify-end gap-3">
                ...
                <div className="relative">
                  <FaFilter className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 z-10" size={12} />
                  <Form.Select
        ...
        {isLoading ? (
          ...
          <div className="bg-white rounded-[2.5rem] border-2 border-dashed border-slate-100 py-20 text-center">
            ...
            <h5 className="text-slate-400 font-bold uppercase tracking-widest m-0">{t.noData}</h5>
          </div>
        ) : (
          <>
            {/* Desktop Table */}
              ...
              <table className="w-full border-collapse">
                <thead>
                  <tr className="bg-slate-50/50">
                    <th className="px-8 py-5 text-left text-[10px] font-bold text-slate-400 uppercase tracking-widest border-b border-slate-50">Project Information</th>
                    <th className="px-6 py-5 text-center text-[10px] font-bold text-slate-400 uppercase tracking-widest border-b border-slate-50">{t.qty}</th>
                    <th className="px-6 py-5 text-end text-[10px] font-bold text-slate-400 uppercase tracking-widest border-b border-slate-50">{t.price}</th>
                    <th className="px-6 py-5 text-center text-[10px] font-bold text-slate-400 uppercase tracking-widest border-b border-slate-50">{t.status}</th>
                    <th className="px-8 py-5 text-end text-[10px] font-bold text-slate-400 uppercase tracking-widest border-b border-slate-50">{t.actions}</th>
                  </tr>
                </thead>
                    ...
                    <tr key={order.id} className="group hover:bg-slate-50/30 transition-colors">
                      <td className="px-8 py-6">
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            <div className="font-bold text-slate-900 mb-0.5">{order.projectname}</div>
                            <div className="text-[10px] font-bold text-slate-400 uppercase tracking-widest flex items-center gap-2">
                              ID: {order.id} <span className="text-slate-200">|</span>
                              {new Date(order.created_at).toLocaleDateString('th-TH', { year: 'numeric', month: 'short', day: 'numeric' })}
                            </div>
                          </div>
                        </div>
                      </td>
                      ...
                      <td className="px-6 py-6 text-end font-bold text-slate-900">
                        {order.confirmed_price ? (
                          <span className="text-blue-600">฿{parseFloat(order.confirmed_price).toLocaleString()}</span>
                        ) : (
                          <span className="text-slate-300 italic font-medium">Pending Quote</span>
                        )}
                      </td>
                      <td className="px-6 py-6 text-center">
            ...
            <div className="lg:hidden space-y-4">
              {filteredData.map((order) => (
                <div key={order.id} className="bg-white rounded-[2.5rem] shadow-sm border border-slate-100 p-6 overflow-hidden relative">
                  <div className="flex justify-between items-start mb-6">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <h5 className="font-bold text-slate-900 m-0">{order.projectname}</h5>
                  ...
                  <div className="grid grid-cols-2 gap-4 mb-6">
                    <div className="p-4 bg-slate-50 rounded-3xl border border-slate-100">
                      <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1.5">{t.qty}</p>
                      <p className="text-sm font-bold text-slate-900 m-0">{order.pcb_qty} Pcs</p>
                    </div>
                    <div className="p-4 bg-slate-50 rounded-3xl border border-slate-100">
                      <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1.5">{t.price}</p>
                      <p className="text-sm font-bold text-blue-600 m-0">
                        {order.confirmed_price ? `฿${parseFloat(order.confirmed_price).toLocaleString()}` : '-'}
                      </p>
                    </div>
                  </div>
