...
import { FaDownload, FaFileArchive, FaImages, FaArrowLeft, FaMicrochip, FaBox, FaQuoteRight, FaCheckCircle, FaTimesCircle, FaClock, FaCreditCard, FaSearchPlus, FaInfoCircle } from 'react-icons/fa';
...
const CopyPCBCartDetailScreen = () => {
  ...
  if (isLoading) return <div className="min-h-screen flex items-center justify-center bg-slate-50/50"><Loader /></div>;
  if (isError) return <div className="min-h-screen p-8 bg-slate-50/50"><Message variant="danger">{error.message}</Message></div>;
  ...
  const downloadAllImages = async (e) => {
    ...
    for (let i = 1; i <= 10; i++) {
      if (order[`front_image_${i}`]) imagesToFetch.push({ url: getFullUrl(order[`front_image_${i}`]), filename: `front-${i}.png` });
      if (order[`back_image_${i}`]) imagesToFetch.push({ url: getFullUrl(order[`back_image_${i}`]), filename: `back-${i}.png` });
    }

    if (imagesToFetch.length === 0) return toast.error('No images found.');

    try {
      await Promise.all(imagesToFetch.map(async ({ url, filename }) => {
        try {
  ...
  const handleDownloadAll = async (e) => {
    ...
    try {
      if (order.copypcb_zip) {
        ...
        if (resp.ok) {
          folder.file(order.copypcb_zip.split('/').pop() || 'source.zip', await resp.blob());
        } else {
  ...
  const imageGroups = {
    front: Array.from({ length: 10 }, (_, i) => order[`front_image_${i + 1}`]).filter(Boolean),
    back: Array.from({ length: 10 }, (_, i) => order[`back_image_${i + 1}`]).filter(Boolean),
  };
  ...
  return (
    <div className="min-h-screen bg-slate-50/50 pb-12 font-prompt">
      <Container className="py-6">
        ...
        <div className="mb-8 font-prompt">
          <div className="flex items-center gap-4 mb-4">
            <Link to="/cart/copypcbcart" className="w-10 h-10 rounded-2xl bg-white shadow-sm border border-slate-100 flex items-center justify-center text-slate-400 hover:text-blue-600 hover:border-blue-100 transition-all">
              <FaArrowLeft size={16} />
            </Link>
            <div>
              <nav className="flex mb-1" aria-label="Breadcrumb">
                <ol className="flex items-center space-x-2 text-[10px] font-bold uppercase tracking-widest text-slate-400">
                  <li><Link to="/cart/copypcbcart" className="hover:text-blue-500 transition-colors">Cart</Link></li>
                  <li>/</li>
            ...
            <div className="space-y-6">
                ...
                <div className="p-8 border-b border-slate-50 flex items-center justify-between bg-slate-50/30">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest m-0">Project ID: #{order.id}</p>
                    </div>
                  </div>
                ...
                <div className="p-8">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
                    <div className="flex items-start gap-4">
                      ...
                      <div>
                        <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">{t.quantityLbl}</p>
                    ...
                    <div className="flex items-start gap-4">
                      ...
                      <div>
                        <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">Estimated Price</p>
                        <p className="text-xl font-bold text-blue-600 m-0">
                          {order.confirmed_price && order.confirmed_price > 0 ? (
                            ...
                            order.status === 'accepted' ? '฿0.00' : t.waitQuotation
                          )}
                        </p>
                      </div>
                    </div>
              ...
              <div className="bg-white rounded-[2.5rem] shadow-sm border border-slate-100 p-8">
                <div className="flex items-center justify-between mb-8">
                  ...
                  <button onClick={downloadAllImages} className="text-[10px] font-bold text-blue-600 uppercase tracking-widest hover:text-blue-700 transition-colors border-none bg-transparent">
                    {t.downloadAllImg}
                  </button>
                </div>
                ...
                {imageGroups.front.length > 0 && (
                  <div className="mb-8">
                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-4 px-1">Front Surface</p>
                    <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-3">
                      {imageGroups.front.map((img, idx) => (
                        <div key={`front-${idx}`} className="group relative aspect-square rounded-2xl overflow-hidden border border-slate-100 shadow-sm transition-all hover:border-blue-200">
                          <Image src={getFullUrl(img)} className="w-full h-full object-contain p-2 bg-slate-50 transition-transform duration-500 group-hover:scale-110" />
                          <div className="absolute inset-0 bg-slate-900/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center backdrop-blur-[1px]">
                            <button onClick={() => setZoomedImage(getFullUrl(img))} className="w-8 h-8 rounded-xl bg-white text-slate-900 flex items-center justify-center shadow-lg border-none hover:bg-blue-600 hover:text-white transition-all">
                ...
                {imageGroups.back.length > 0 && (
                  <div>
                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-4 px-1">Back Surface</p>
                    <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-3">
                      {imageGroups.back.map((img, idx) => (
                        <div key={`back-${idx}`} className="group relative aspect-square rounded-2xl overflow-hidden border border-slate-100 shadow-sm transition-all hover:border-blue-200">
                          <Image src={getFullUrl(img)} className="w-full h-full object-contain p-2 bg-slate-50 transition-transform duration-500 group-hover:scale-110" />
                          <div className="absolute inset-0 bg-slate-900/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center backdrop-blur-[1px]">
                            <button onClick={() => setZoomedImage(getFullUrl(img))} className="w-8 h-8 rounded-xl bg-white text-slate-900 flex items-center justify-center shadow-lg border-none hover:bg-blue-600 hover:text-white transition-all">
                              <FaSearchPlus size={12} />
                ...
                {imageGroups.front.length === 0 && imageGroups.back.length === 0 && (
                  <div className="py-12 text-center bg-slate-50 rounded-3xl border-2 border-dashed border-slate-100">
                    <FaImages size={32} className="mx-auto text-slate-200 mb-3" />
                    <p className="text-xs font-bold text-slate-400 uppercase tracking-widest m-0">{t.noneLbl}</p>
                  </div>
                )}
              </div>
            </div>
          ...
          <Col lg={4}>
            <div className="sticky top-6 space-y-6">
              ...
              <div className="bg-white rounded-[2.5rem] shadow-sm border border-slate-100 p-8 text-center">
                ...
                <h5 className="text-lg font-bold text-slate-900 mb-2">{t.CopyZip}</h5>
                <p className="text-xs text-slate-400 mb-8 px-4 leading-relaxed">Technical project files required for manufacturing.</p>
                ...
                <div className="space-y-3">
                  ...
                  <button onClick={handleDownloadAll} className="flex items-center justify-center gap-3 w-full py-4 rounded-2xl bg-white text-slate-900 border-2 border-slate-100 font-bold text-sm hover:bg-slate-50 transition-all shadow-sm">
              ...
              <div className="bg-gradient-to-br from-blue-600 to-indigo-700 rounded-[2.5rem] shadow-xl shadow-blue-100 p-8 text-white relative overflow-hidden">
                ...
                <div className="relative z-10">
                  ...
                  <p className="text-white/80 text-xs leading-relaxed mb-6">If you have any questions regarding this order, please contact the engineering team.</p>
      ...
      {/* Zoom Modal */}
      {zoomedImage && (
        <div onClick={() => setZoomedImage(null)} className="fixed inset-0 z-[1050] bg-slate-950/95 backdrop-blur-sm flex justify-center items-center p-4 cursor-zoom-out animate-in fade-in duration-300">
          <img src={zoomedImage} alt="Zoomed" className="max-w-full max-h-full object-contain rounded-2xl shadow-2xl animate-in zoom-in-95 duration-300" />
