...
import {
  FaTrash, FaClone, FaMinus, FaPlus, FaImages, FaInfoCircle, FaArrowLeft, FaFileArchive, FaRegFileCode
} from 'react-icons/fa';
...
const CopyPCBSetScreen = () => {
  ...
  const translations = {
    en: {
      ...
      addFilesDesc: 'If you have schematics or specific files, attach them here.',
      changeFile: 'Change File',
      browseFiles: 'Browse Files',
      noFileChosen: 'No file chosen (Optional)',
      modifications: 'Modifications / Notes',
      phModifications: "Describe what you want to copy or change (e.g., 'Copy exactly as is' or 'Change connector to USB-C')...",
    ...
    thai: {
      ...
      phModifications: "อธิบายสิ่งที่คุณต้องการ (เช่น 'ก็อปปี้ให้เหมือนเดิม 100%' หรือ 'เปลี่ยนจุดเชื่อมต่อเป็น USB-C')...",
  ...
  const [uploadMultipleCopyPCBImages] = useUploadMultipleCopyPCBImagesMutation();
  ...
  const uploadFrontImages = async () => {
    ...
    return (Array.isArray(result) ? result : [result]).map(img => typeof img === 'string' ? img : (img.path || img.url));
  };
  ...
  const uploadcopypcbZipHandler = async () => {
    ...
    const path = res.file || res.zipPath || res.url || res.copypcbZip || res.path || res;
    return typeof path === 'string' ? path : (path.path || path.url || path.filename);
  };
    ...
    <div className="min-h-screen bg-white pb-16 font-sans text-slate-900 antialiased selection:bg-black selection:text-white">
      ...
      {isLoading && (
        <div className="fixed inset-0 z-[9999] bg-white/90 backdrop-blur-sm flex flex-col items-center justify-center text-center">
          ...
          <h5 className="mt-6 text-xl font-black text-black animate-pulse uppercase tracking-tighter">{t.processing}</h5>
          <p className="text-slate-400 text-xs font-bold uppercase tracking-widest mt-2">{t.uploadingFiles}</p>
        </div>
      )}
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex items-center gap-6">
            ...
            <div>
              ...
              <p className="text-sm md:text-base text-slate-500 font-medium tracking-wide m-0">{t.subtitle}</p>
            </div>
          </div>
        </div>
      </div>
            ...
            <div className="lg:col-span-8 space-y-6">
              ...
              <div className="bg-white rounded-3xl border border-slate-100 overflow-hidden shadow-[0_1px_3px_rgba(0,0,0,0.02)]">
                <div className="bg-slate-50/50 border-b border-slate-100 px-8 py-5 flex items-center gap-3">
                  ...
                  <h3 className="font-black text-black text-xs uppercase tracking-widest m-0">{t.projectDetails}</h3>
                </div>
                <div className="p-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="bg-white rounded-3xl border border-slate-100 overflow-hidden flex flex-col shadow-[0_1px_3px_rgba(0,0,0,0.02)]">
                  <div className="bg-slate-50 border-b border-slate-100 px-6 py-4 flex items-center justify-between">
                    <span className="font-black text-black text-[10px] uppercase tracking-widest flex items-center gap-2"><BsMotherboard className="text-black text-sm" /> {t.frontSide}</span>
                    <span className="text-[9px] font-black border border-black text-black px-2 py-0.5 rounded-full uppercase tracking-tighter">{t.required}</span>
                  </div>
                  <div className="p-8 flex-grow flex flex-col">
                    ...
                    >
                      <input type="file" id="front-upload" accept="image/*" multiple className="absolute inset-0 w-full h-full opacity-0 cursor-pointer z-10" onChange={(e) => handleImageUpload(e.target.files, 'front')} />
                      <div className="pointer-events-none flex flex-col items-center text-center">
                        <FaImages className="text-3xl text-slate-200 mb-4" />
                        <span className="font-black text-black text-[10px] uppercase tracking-widest">{t.clickToUpload}</span>
                        <span className="text-[10px] text-slate-300 font-bold uppercase tracking-widest mt-1">{t.dragPhotosHere}</span>
                      </div>
                    </div>
                    ...
                    {frontImages.length > 0 && (
                      <div className="flex flex-wrap gap-3 mt-auto pt-4">
                        {frontImages.map((img, idx) => (
                          <div key={idx} className="relative w-14 h-14 rounded-xl overflow-hidden border border-slate-100 group">
                            <img src={img.url} alt="front" className="w-full h-full object-cover" />
                            <button type="button" className="absolute inset-0 bg-black/40 text-white opacity-0 group-hover:opacity-100 transition-all flex items-center justify-center backdrop-blur-[1px]" onClick={() => removeImage(idx, 'front')}>
                ...
                <div className="bg-white rounded-3xl border border-slate-100 overflow-hidden flex flex-col shadow-[0_1px_3px_rgba(0,0,0,0.02)]">
                  <div className="bg-slate-50 border-b border-slate-100 px-6 py-4 flex items-center justify-between">
                    <span className="font-black text-black text-[10px] uppercase tracking-widest flex items-center gap-2"><BsCpu className="text-black text-sm" /> {t.backSide}</span>
                    <span className="text-[9px] font-black border border-black text-black px-2 py-0.5 rounded-full uppercase tracking-tighter">{t.required}</span>
                  </div>
                  <div className="p-8 flex-grow flex flex-col">
                    <div
                    ...
                    >
                      <input type="file" id="back-upload" accept="image/*" multiple className="absolute inset-0 w-full h-full opacity-0 cursor-pointer z-10" onChange={(e) => handleImageUpload(e.target.files, 'back')} />
                      <div className="pointer-events-none flex flex-col items-center text-center">
                        <FaImages className="text-3xl text-slate-200 mb-4" />
                        <span className="font-black text-black text-[10px] uppercase tracking-widest">{t.clickToUpload}</span>
                        <span className="text-[10px] text-slate-300 font-bold uppercase tracking-widest mt-1">{t.dragPhotosHere}</span>
                    ...
                    {backImages.length > 0 && (
                      <div className="flex flex-wrap gap-3 mt-auto pt-4">
                        {backImages.map((img, idx) => (
                          <div key={idx} className="relative w-14 h-14 rounded-xl overflow-hidden border border-slate-100 group">
                            <img src={img.url} alt="back" className="w-full h-full object-cover" />
                            <button type="button" className="absolute inset-0 bg-black/40 text-white opacity-0 group-hover:opacity-100 transition-all flex items-center justify-center backdrop-blur-[1px]" onClick={() => removeImage(idx, 'back')}>
                              <FaTrash className="text-xs" />
                            </button>
                          </div>
                        ))}
              ...
              <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
                <div className="p-6 space-y-6">
                  ...
                  {/* Zip Upload */}
                  <div>
                    <label className="block text-[10px] font-black text-black uppercase tracking-widest mb-3">{t.addFiles}</label>
                    <p className="text-xs text-slate-400 font-medium tracking-wide mb-4 italic">{t.addFilesDesc}</p>

                    <div className="relative">
                      <input type="file" id="zip-upload" accept=".zip,.rar" onChange={(e) => handleChange('zipFile', e.target.files[0])} className="hidden" />
                      <label htmlFor="zip-upload" className={`flex items-center w-full border ${formData.zipFile ? 'border-black bg-slate-50' : 'border-slate-100 bg-slate-50/50 hover:bg-slate-50 hover:border-slate-200'} rounded-2xl p-3 cursor-pointer transition-all`}>
                        <span className="bg-black text-white text-[9px] font-black px-5 py-3 rounded-xl mr-4 shrink-0 shadow-sm uppercase tracking-widest flex items-center gap-2">
                          {formData.zipFile ? <FaFileArchive /> : <FaRegFileCode />}
                          {formData.zipFile ? t.changeFile : t.browseFiles}
                        </span>
                        <span className={`text-xs truncate font-black uppercase tracking-tight ${formData.zipFile ? 'text-black' : 'text-slate-300'}`}>
                          {formData.zipFile ? formData.zipFile.name : t.noFileChosen}
                        </span>
                      </label>
                    </div>
                  </div>
                  ...
                  {/* Notes */}
                  <div className="pt-2">
                    <label className="block text-[10px] font-black text-black uppercase tracking-widest mb-3">{t.modifications}</label>
                    <textarea
                      rows={5}
            ...
            <div className="lg:col-span-4">
              <div className="sticky top-24 bg-black rounded-[2.5rem] shadow-2xl overflow-hidden border border-white/10 ring-1 ring-white/5">
                ...
                <div className="bg-white/5 border-b border-white/10 p-8 text-center">
                  <h3 className="text-white font-black text-xs uppercase tracking-[0.2em] m-0">{t.orderSummary}</h3>
                </div>
                ...
                <div className="p-8">
                  ...
                  <div className="mb-8">
                    <div className="flex justify-between items-center mb-3">
                      <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest m-0">{t.quantity}</label>
                      <span className="text-[9px] font-black border border-white/20 text-slate-400 px-2 py-0.5 rounded-full uppercase tracking-tighter">{t.min5}</span>
                    </div>
                    <div className="flex items-center justify-between bg-white/5 border border-white/10 rounded-2xl p-2 shadow-inner">
                      <button type="button" onClick={() => changeQty(-1)} disabled={formData.pcbQty <= 5} className="w-11 h-11 flex items-center justify-center text-slate-400 bg-black/40 rounded-xl shadow-lg border border-white/10 hover:text-white hover:border-white/30 transition-all disabled:opacity-20 active:scale-95">
                        <FaMinus size={12} />
                      </button>
                      <span className="font-black text-2xl text-white tracking-tighter">{formData.pcbQty}</span>
                      <button type="button" onClick={() => changeQty(1)} className="w-11 h-11 flex items-center justify-center text-slate-400 bg-black/40 rounded-xl shadow-lg border border-white/10 hover:text-white hover:border-white/30 transition-all active:scale-95">
                        <FaPlus size={12} />
                      </button>
                    </div>
                  </div>
                  ...
                  <div className="space-y-4 mb-8">
                    <div className="flex justify-between items-center text-[10px] font-black uppercase tracking-widest">
                      ...
                      <span className={`px-2.5 py-1 rounded-full text-[9px] font-black border ${frontImages.length > 0 ? 'bg-white text-black border-white' : 'bg-transparent text-slate-600 border-white/10'}`}>
                        {frontImages.length}
                      </span>
                    </div>
                    <div className="flex justify-between items-center text-[10px] font-black uppercase tracking-widest">
                      <span className="text-slate-500">{t.backPhotos}</span>
                      <span className={`px-2.5 py-1 rounded-full text-[9px] font-black border ${backImages.length > 0 ? 'bg-white text-black border-white' : 'bg-transparent text-slate-600 border-white/10'}`}>
                        {backImages.length}
                      </span>
                    </div>
                    <div className="flex justify-between items-center text-[10px] font-black uppercase tracking-widest">
                      <span className="text-slate-500">{t.zipFile}</span>
                      <span className={`px-2.5 py-1 rounded-full text-[9px] font-black border ${formData.zipFile ? 'bg-white text-black border-white' : 'bg-transparent text-slate-600 border-white/10'}`}>
