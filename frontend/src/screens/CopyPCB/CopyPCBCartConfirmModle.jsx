...
const CopyPCBCartConfirmModle = ({ show, handleClose, pcborderId, onConfirm }) => {
  const [updatecopycartComfirmStatus] = useUpdatecopycartComfirmStatusMutation();
  ...
  return (
    <Modal show={show} onHide={handleClose} animation={true} centered contentClassName="rounded-[2.5rem] border-none shadow-2xl font-prompt">
      <Modal.Header closeButton className="border-b border-slate-50 px-8 py-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <Modal.Title className="text-xl font-bold text-slate-900">{t.ConfirmOrdersLbl}</Modal.Title>
            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-0.5">Admin Review Session</p>
          </div>
        </div>
      </Modal.Header>
      <Modal.Body className="p-8">
        <Form>
          <div className="space-y-6">
            <div>
              <Form.Label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-3 px-1">{t.StatusLbl}</Form.Label>
              <div className="flex gap-3">
                <button
                  type="button"
                  onClick={() => setStatus('accepted')}
                  className={`flex-grow py-4 px-4 rounded-2xl border-2 transition-all flex items-center justify-center gap-2 font-bold text-sm ${status === 'accepted'
                ...
                <button
                  ...
                  className={`flex-grow py-4 px-4 rounded-2xl border-2 transition-all flex items-center justify-center gap-2 font-bold text-sm ${status === 'rejected'
            ...
            {status === 'accepted' && (
              <div className="animate-in fade-in slide-in-from-top-2 duration-300">
                <Form.Label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-3 px-1">{t.ConfirmedPriceLbl}</Form.Label>
                <div className="relative">
                  ...
                  />
                  <div className="absolute right-4 top-1/2 -translate-y-1/2 text-[10px] font-bold text-slate-400 uppercase tracking-widest">THB</div>
                </div>
              </div>
            )}
          </div>
      ...
      <Modal.Footer className="border-t border-slate-50 p-8 flex gap-3">
        ...
        <button
          ...
          className={`flex-grow py-4 rounded-2xl font-bold text-sm shadow-xl transition-all disabled:opacity-50 disabled:shadow-none ${status === 'accepted'
