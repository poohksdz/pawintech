...
import {
  FaCheck, FaCloudUploadAlt, FaFileArchive, FaMicrochip, FaReceipt,
  FaRulerCombined, FaLayerGroup, FaPalette, FaWeightHanging, FaShieldAlt,
  FaChevronRight, FaExclamationCircle, FaPlus, FaMinus, FaArrowLeft
} from 'react-icons/fa';
...
const GerberViewerScreen = () => {
  ...
  const translations = {
    en: {
      ...
      subtitle: 'Professional Manufacturing • Quality Guaranteed • Fast Delivery',
    ...
    thai: {
      ...
      successCart: 'เพิ่มลงตะกร้าเรียบร้อยแล้ว!'
    }
  };
  ...
  const extraServiceFee = Number(data?.defaultPricing?.extra_service_fee ?? 795); // ค่าธรรมเนียมคงที่
  ...
  const surfaceFinishes = data?.surfaceFinishes || [{ name: 'HASL(With lead)', price: 0 }];
  ...
  const getEmsDeliveryPrice = () => {
    ...
    if (emsRates.length === 0) return 50.00; // ค่าจัดส่ง EMS แบบ fallback เป็น 50 หรือง่ายๆ
  ...
  const getBaseMaterialPrice = () => (Number(baseMaterials.find(m => m.name === formData.baseMaterial)?.price) || 0);
  const getColorPrice = () => (Number(pcbColors.find(i => i.name === formData.pcbColor)?.price) || 0);
  const getSurfaceFinishPrice = () => (Number(surfaceFinishes.find(s => s.name === formData.surfaceFinish)?.price) || 0);
  const getCopperWeightPrice = () => (Number(copperWeights.find(c => c.name === formData.copperWeight)?.price) || 0);
  ...
  const handleZipUpload = async (e) => {
    ...
    if (!file || !/\.(zip|rar)$/i.test(file.name)) {
      toast.error(language === 'thai' ? 'กรุณาอัปโหลดไฟล์ .zip หรือ .rar ที่ถูกต้อง' : 'Please upload a valid .zip or .rar file.');
      return;
    }
    ...
    try {
      const response = await fetch('/api/gerber/upload-zip', { method: 'POST', body: formDataUpload });
      const result = await response.json();
      if (response.ok) {
        setUploadSuccess(true);
        setUploadMessage(language === 'thai' ? 'อัปโหลดไฟล์สำเร็จ!' : 'File uploaded successfully!');
        setGerberFiles([`${result.path}`]);
      } else {
        setUploadSuccess(false);
        setUploadMessage(language === 'thai' ? 'อัปโหลดไม่สำเร็จ' : 'Upload failed');
      }
    } catch (err) {
      setUploadSuccess(false);
  ...
  const orderNowHandler = (e) => {
    ...
    if (!zipFileName && (!gerberFiles || gerberFiles.length === 0)) return toast.error(t.errGerber);
    if (formData.dimensions.x <= 0 || formData.dimensions.y <= 0) return toast.error(t.errDim);
    ...
    localStorage.setItem('pcbcard', JSON.stringify(orderDetails));
  ...
  const renderOptionTiles = (field, options) => {
    ...
    const colorMap = {
      'Green': 'bg-green-600', 'Red': 'bg-red-600', 'Blue': 'bg-blue-600',
      'Yellow': 'bg-yellow-400', 'Black': 'bg-gray-900', 'White': 'bg-gray-100', 'Purple': 'bg-purple-600'
    };
        ...
        {options.map((option, index) => {
          ...
          return (
            <button
              ...
              className={`relative flex flex-col items-center justify-center p-3 rounded-xl border transition-all duration-200 ${isDisabled ? 'opacity-40 cursor-not-allowed bg-slate-50 border-slate-100' :
                isSelected ? 'border-black bg-black text-white shadow-lg scale-[1.02]' : 'border-slate-100 bg-slate-50 text-slate-400 hover:border-slate-200'
                }`}
            >
              {isColorField && (
                <div className={`w-6 h-6 rounded-full mb-2 shadow-inner border border-black/10 ${colorMap[name] || 'bg-gray-400'}`} />
              )}
              <span className={`text-[10px] font-black uppercase tracking-widest ${isSelected ? 'text-white' : 'text-slate-400'}`}>{name}</span>
              {isSelected && !isDisabled && <FaCheck className="absolute top-2 right-2 text-white text-[8px]" />}
            </button>
          );
        })}
      </div>
          ...
          <div className="lg:col-span-8 space-y-8">
            ...
            <section className="bg-white rounded-3xl border border-slate-100 overflow-hidden shadow-[0_1px_3px_rgba(0,0,0,0.02)]">
              <div className="px-8 py-5 border-b border-slate-100 flex items-center gap-4 bg-slate-50/50">
                <div className="text-black"><FaCloudUploadAlt size={20} /></div>
                <div>
                  <h2 className="text-xs font-black text-black uppercase tracking-widest leading-none">{t.gerberTitle}</h2>
                  <p className="text-[10px] text-slate-400 mt-1 uppercase tracking-widest font-black">{t.gerberDesc}</p>
                </div>
              </div>
              <div className="p-8">
                <div className={`relative group border-2 border-dashed rounded-2xl p-12 text-center transition-all ${uploadSuccess ? 'border-black bg-slate-50' : 'border-slate-100 hover:border-slate-200 bg-slate-50/50'
                  }`}>
                  <input type="file" accept=".zip,.rar" onChange={handleZipUpload} className="absolute inset-0 w-full h-full opacity-0 cursor-pointer z-10" />
                  <div className="space-y-4">
                    {uploadSuccess ? <FaCheck className="mx-auto text-black text-5xl" /> : <FaFileArchive className="mx-auto text-slate-200 group-hover:text-black text-5xl transition-colors" />}
                    <div>
                      <h3 className="text-sm font-black text-black uppercase tracking-widest">{zipFileName || t.uploadHint}</h3>
                      <p className="text-[10px] text-slate-400 font-black uppercase tracking-widest">{t.uploadSupport}</p>
                    </div>
                    {uploadMessage && <span className={`text-[10px] font-black px-4 py-1.5 rounded-full uppercase tracking-widest border ${uploadSuccess ? 'bg-black text-white border-black' : 'bg-white text-black border-black'}`}>{uploadMessage}</span>}
                  </div>
                </div>
              </div>
            </section>
            ...
            <section className="bg-white rounded-3xl border border-slate-100 overflow-hidden shadow-[0_1px_3px_rgba(0,0,0,0.02)]">
              <div className="px-8 py-5 border-b border-slate-100 flex items-center gap-4 bg-slate-50/50">
                <div className="text-black"><FaMicrochip size={20} /></div>
                <div>
                  <h2 className="text-xs font-black text-black uppercase tracking-widest leading-none">{t.pcbSpecs}</h2>
                  <p className="text-[10px] text-slate-400 mt-1 uppercase font-black tracking-widest">{t.specsDesc}</p>
                </div>
              </div>
              <div className="p-8 space-y-12">
                <div className="p-8 bg-slate-50/50 border border-slate-100 rounded-3xl">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                    <div>
                      <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3 block">{t.boardSize}</label>
                      <div className="flex items-center gap-4">
                        <div className="relative flex-1">
                          <input type="number" placeholder="W" value={formData.dimensions.x} onChange={(e) => setFormData({ ...formData, dimensions: { ...formData.dimensions, x: parseInt(e.target.value) || 0 } })} className="w-full bg-white border border-slate-200 rounded-xl px-4 py-4 font-black uppercase tracking-tighter focus:ring-1 focus:ring-black focus:border-black focus:outline-none text-center" />
                          <span className="absolute right-4 top-1/2 -translate-y-1/2 text-[9px] font-black text-slate-300 uppercase">mm</span>
                        </div>
                        <span className="text-slate-200 font-black">×</span>
                        <div className="relative flex-1">
                          <input type="number" placeholder="H" value={formData.dimensions.y} onChange={(e) => setFormData({ ...formData, dimensions: { ...formData.dimensions, y: parseInt(e.target.value) || 0 } })} className="w-full bg-white border border-slate-200 rounded-xl px-4 py-4 font-black uppercase tracking-tighter focus:ring-1 focus:ring-black focus:border-black focus:outline-none text-center" />
                          <span className="absolute right-4 top-1/2 -translate-y-1/2 text-[9px] font-black text-slate-300 uppercase">mm</span>
                        </div>
                      </div>
                    </div>
                    <div>
                      <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3 block">{t.qty}</label>
                      <div className="flex items-center bg-white border border-slate-200 rounded-xl p-1.5 shadow-sm">
                        <button type="button" onClick={() => changeQty(-5)} disabled={formData.pcbQty <= 5} className="w-12 h-11 flex items-center justify-center text-slate-400 hover:text-black hover:bg-slate-50 rounded-lg transition-all disabled:opacity-20">
                          <FaMinus className="text-[10px]" />
                        </button>
                        <div className="flex-1 text-center font-black text-xl text-black tracking-tighter">
                          {formData.pcbQty}
                        </div>
                        <button type="button" onClick={() => changeQty(5)} className="w-12 h-11 flex items-center justify-center text-slate-400 hover:text-black hover:bg-slate-50 rounded-lg transition-all">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                  <div><label className="text-xs font-black text-slate-400 uppercase tracking-widest mb-2 block">{t.layers}</label>{renderOptionTiles('layers', [1, 2])}</div>
                  <div><label className="text-xs font-black text-slate-400 uppercase tracking-widest mb-2 block">{t.thickness}</label>{renderOptionTiles('thickness', ['0.8mm', '1.6mm'])}</div>
                </div>

                <div className="pt-6 border-t border-slate-100">
                  <label className="text-xs font-black text-slate-400 uppercase tracking-widest mb-2 block">{t.material}</label>
                  {renderOptionTiles('baseMaterial', materialOptions.length ? materialOptions : ['FR-4', 'Aluminum'])}
                </div>

                <div className="space-y-8 pt-6 border-t border-slate-100">
                  <div><label className="text-xs font-black text-slate-400 uppercase tracking-widest mb-2 block">{t.solderMask}</label>{renderOptionTiles('pcbColor', colorOptions.length ? colorOptions : ['Green', 'Purple', 'Red', 'Yellow', 'Blue', 'White', 'Black'])}</div>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                    <div><label className="text-xs font-black text-slate-400 uppercase tracking-widest mb-2 block">{t.silkscreen}</label>{renderOptionTiles('silkscreen', ['White', 'Black'])}</div>
                    <div><label className="text-xs font-black text-slate-400 uppercase tracking-widest mb-2 block">{t.finish}</label>{renderOptionTiles('surfaceFinish', finishOptions.length ? finishOptions : ['HASL(With lead)', 'LeadFree HASL', 'ENIG'])}</div>
                  </div>
                </div>

                <div className="pt-6 border-t border-slate-100">
                  <label className="text-xs font-black text-slate-400 uppercase tracking-widest mb-2 block">{t.copper}</label>
                  {renderOptionTiles('copperWeight', copperOptions.length ? copperOptions : ['1 oz', '2 oz'])}
                </div>
              </div>
            </section>
          </div>
            ...
            <div className="sticky top-24 space-y-6">
              ...
              <div className="bg-white rounded-3xl border border-slate-100 shadow-sm overflow-hidden text-start">
                <div className="px-6 py-4 border-b border-slate-100 bg-slate-50/50">
                  <h2 className="text-[10px] font-black tracking-widest uppercase text-slate-400 mb-0">Summary Selected</h2>
                </div>
                <div className="p-6 space-y-4">
                  {[
                    { label: 'Base Material', value: formData.baseMaterial },
                    { label: 'Layers', value: formData.layers },
                    { label: 'Dimensions', value: `${formData.dimensions.x} × ${formData.dimensions.y} mm` },
                    ...
                    <div key={i} className="flex justify-between items-center text-[10px] font-bold">
                      ...
                      <span className="text-black uppercase tracking-tighter">{row.value}</span>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="bg-white rounded-3xl border border-slate-200 shadow-xl overflow-hidden text-start ring-1 ring-black/5">
                <div className="px-6 py-4 border-b border-slate-100 bg-black text-white">
                  <h2 className="text-[10px] font-black tracking-widest uppercase mb-0">Charge Details</h2>
                </div>
                <div className="p-6 space-y-5">
                  {[
                    { label: 'Price', value: getSubPriceRaw().toFixed(2) },
                    { label: 'Base Material', value: getBaseMaterialPrice().toFixed(2) },
                    { label: 'Surface Finish', value: getSurfaceFinishPrice().toFixed(2) },
                    { label: 'Solder Mark Color', value: getColorPrice().toFixed(2) },
                    { label: 'Copper Weight', value: getCopperWeightPrice().toFixed(2) },
                    { label: 'Build Time', value: `${buildTime} days` },
                    { label: 'Delivery', value: getEmsDeliveryPrice().toFixed(2) },
                    { label: 'Vat 7%', value: calculateVatAmount().toFixed(2) },
                  ].map((row, i) => (
                    <div key={i} className="flex justify-between items-center text-[11px] font-black">
                      <span className="text-slate-400 uppercase tracking-widest">{row.label}</span>
                      <span className="text-black tracking-tighter">{row.value} {typeof row.value === 'string' && row.value.includes('days') ? '' : '฿'}</span>
                    </div>
                  ))}

                  <div className="pt-4 border-t border-slate-100 flex justify-between items-baseline">
                    <span className="text-xs font-black uppercase text-black">Total Price</span>
                    <div className="text-right">
                      <span className="text-3xl font-black text-black tracking-tighter">{calculateTotalPriceRaw().toFixed(2)}</span>
                      <span className="text-[10px] ml-1 font-black text-slate-400 uppercase">฿</span>
                    </div>
                  </div>
                  ...
                  <div className="pt-2">
                    <p className="text-[9px] text-red-500 font-bold uppercase tracking-wide leading-relaxed italic">
                      * If your order is lower than the minimum {basePrice.toLocaleString()} ฿ charge, the minimum charge will apply.
                    </p>
                  </div>
                </div>
              </div>
