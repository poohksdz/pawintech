...
import { FaMicrochip, FaRocket, FaShieldAlt, FaEdit, FaServer, FaCogs, FaUsers } from 'react-icons/fa';
...
const AboutScreen = () => {
  ...
  const content = about ? (language === "thai" ? about.aboutContentThai : about.aboutContentEng) : "";
  ...
  const processContent = (html) => {
    ...
    return html
      ...
      .replace(/<img /g, '<img class="w-full h-auto rounded-3xl shadow-xl my-8 object-cover object-center max-h-[500px]" ')
      .replace(/<iframe /g, '<iframe class="w-full max-w-4xl aspect-video rounded-3xl shadow-xl my-8 mx-auto" ')
      .replace(
  ...
  if (!processedContent || processedContent.trim().length === 0) {
    processedContent = language === "thai" ? processContent(fallbackOverviewTh) : processContent(fallbackOverviewEn);
  }

  const t = {
    en: {
      heroTitle: "Expert End-to-End PCB Solutions",
      heroSubtitle: "Pawin Technology Co., Ltd. is committed to providing excellent electronic manufacturing services, from PCB design and component sourcing to high-standard PCBA assembly.",
      stats: [
      ...
      values: [
        { icon: <FaMicrochip />, title: "PCB & Product Design", desc: "Professional schematic and layout design services tailored to your specific product requirements." },
        { icon: <FaRocket />, title: "PCB Clone Service", desc: "High-precision PCB cloning and reverse engineering to restore or upgrade legacy systems." },
        { icon: <FaServer />, title: "Component Sourcing", desc: "Reliable global procurement of electronic components, ensuring authentic parts at optimal pricing." },
        { icon: <FaCogs />, title: "PCBA Manufacturing", desc: "State-of-the-art Surface Mount Technology (SMT) and through-hole assembly services." },
        { icon: <FaShieldAlt />, title: "Testing & Inspection", desc: "Rigorous quality control, functional testing, and inspection at every manufacturing stage." },
        { icon: <FaUsers />, title: "Expert Consultation", desc: "Personalized consulting and deep-dive analysis for Bill of Materials (BOM) and risk mitigation." },
      ],
      editButton: "Edit Database Content",
    },
    thai: {
      heroTitle: "ผู้เชี่ยวชาญการผลิตและออกแบบ PCB ครบวงจร",
      heroSubtitle: "บริษัท ภาวินท์เทคโนโลยี จำกัด มุ่งมั่นให้บริการด้านอิเล็กทรอนิกส์ด้วยโซลูชันที่ทันสมัย ตั้งแต่การออกแบบ จัดหาอุปกรณ์ ไปจนถึงการประกอบ PCBA ที่ได้มาตรฐานสูงสุด",
      stats: [
      ...
      values: [
        { icon: <FaMicrochip />, title: "ออกแบบ PCB และผลิตภัณฑ์", desc: "บริการออกแบบวงจรและจัดวาง Layout ระดับมืออาชีพ ปรับแต่งให้ตรงกับความต้องการเฉพาะของคุณ" },
        { icon: <FaRocket />, title: "บริการคัดลอกวงจร (Clone)", desc: "วิศวกรรมย้อนกลับและคัดลอกแผ่น PCB ด้วยความแม่นยำสูง เพื่อซ่อมบำรุงหรืออัปเกรดระบบเดิม" },
        { icon: <FaServer />, title: "จัดหาอุปกรณ์อิเล็กทรอนิกส์", desc: "จัดหาชิ้นส่วนและอะไหล่จากเครือข่ายระดับโลก มั่นใจได้ในของแท้และราคาที่เหมาะสม" },
        { icon: <FaCogs />, title: "ประกอบแผงวงจร (PCBA)", desc: "บริการประกอบ SMT และ Through-hole ด้วยเครื่องจักรที่ทันสมัย รองรับการผลิตทุกระดับ" },
        { icon: <FaShieldAlt />, title: "ทดสอบและตรวจสอบวงจร", desc: "ผ่านการทดสอบฟังก์ชันและควบคุมคุณภาพอย่างเข้มงวดในทุกขั้นตอนการผลิต" },
        { icon: <FaUsers />, title: "บริการให้คำปรึกษา", desc: "วิเคราะห์ความเสี่ยง BOM ประเมินอุปกรณ์ที่ล้าสมัย และให้คำปรึกษาด้านเส้นทางวิกฤตของโปรดักส์" },
      ],
  ...
  return (
    <div className="bg-slate-50 min-h-screen font-sans pb-40">
      ...
      <section className="relative pt-32 pb-40 lg:pt-40 lg:pb-48 overflow-hidden bg-slate-900">
        ...
        <div className="relative z-10 max-w-5xl mx-auto px-6 lg:px-8 text-center text-white">
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-slate-800/80 border border-slate-700/50 backdrop-blur-md mb-8 shadow-2xl">
            ...
            <span className="text-xs font-bold tracking-widest uppercase text-slate-300">About PAWIN Technology</span>
          </div>
      ...
      <section className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 relative z-20 -mt-20 md:-mt-24 mb-32">
        <div className="bg-white rounded-[2.5rem] shadow-[0_20px_60px_-15px_rgba(0,0,0,0.1)] border border-slate-100/60 p-8 md:p-14 overflow-hidden backdrop-blur-xl bg-white/90">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 md:gap-4 divide-y md:divide-y-0 md:divide-x divide-slate-100">
            {t.stats.map((stat, i) => (
              <div key={i} className={`pt-8 md:pt-0 ${i === 0 ? 'pt-0' : ''} flex flex-col items-center justify-center group`}>
                <div className="text-5xl md:text-6xl font-black mb-2 tracking-tight text-slate-900 group-hover:scale-105 transition-transform duration-300">
                  {stat.value}
                </div>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mb-32">
        <div className="text-center mb-16 md:mb-20">
          <h2 className="text-3xl md:text-5xl font-black text-slate-900 mb-6 tracking-tight">{t.valuesTitle}</h2>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
          {t.values.map((v, i) => (
            <TrackVisibility once partialVisibility key={i}>
              {({ isVisible }) => (
                <div
                  className={`bg-white rounded-3xl p-8 hover:bg-slate-900 hover:-translate-y-2 group transition-all duration-500 shadow-sm border border-slate-100 hover:shadow-2xl h-full flex flex-col ${isVisible ? 'animate-[slideUpFade_0.8s_ease-out_forwards]' : 'opacity-0'}`}
                ...
                >
                  ...
                  <h3 className="text-xl font-bold text-slate-900 group-hover:text-white mb-4 transition-colors duration-500">{v.title}</h3>
                  <p className="text-slate-500 group-hover:text-slate-400 leading-relaxed font-medium transition-colors duration-500">{v.desc}</p>
                </div>
              )}
            </TrackVisibility>
          ))}
      ...
      <section className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-10 gap-4">
          <div className="flex items-center gap-4">
            ...
            <h2 className="text-3xl md:text-4xl font-black text-slate-900 tracking-tight">Company Overview</h2>
          </div>

          {about && (
            <Link
              to={`/admin/about/${aboutId}/edit`}
              className="inline-flex items-center gap-2 px-5 py-2.5 bg-white border border-slate-200 hover:border-blue-200 hover:bg-blue-50 text-slate-700 hover:text-blue-700 font-bold rounded-xl transition-all shadow-sm cursor-pointer"
            >
              <FaEdit /> {t.editButton}
            </Link>
          )}
        ...
        <div className="bg-white p-8 sm:p-12 md:p-16 rounded-[2.5rem] shadow-sm border border-slate-100 min-h-[300px] flex flex-col justify-center">
          <div
            className="prose prose-lg md:prose-xl prose-slate max-w-none 
                          prose-headings:font-black prose-headings:text-slate-900 prose-headings:tracking-tight
                          prose-p:text-slate-600 prose-p:leading-loose
                          prose-a:text-blue-600 hover:prose-a:text-blue-700 prose-a:font-semibold prose-a:no-underline hover:prose-a:underline
