...
const CustomerListScreen = () => {
  ...
  const translations = {
    ...
    thai: {
      ...
      headers: {
        name: "ชื่อลูกค้า",
        contact: "ผู้ติดต่อ / เลขผู้เสียภาษี",
        address: "ที่อยู่",
        actions: "จัดการ",
      },
      modal: {
        title: "ยืนยันการลบ",
        body: "คุณแน่ใจหรือไม่ที่จะลบข้อมูลลูกค้ารายนี้? การกระทำนี้ไม่สามารถย้อนกลับได้",
        cancel: "ยกเลิก",
        confirm: "ลบข้อมูล",
      },
  ...
  const handleDeleteConfirm = async () => {
    try {
      await deleteCustomer(
        customerToDelete.id || customerToDelete._id,
      ).unwrap();
    ...
    if (searchTerm) {
      ...
      processed = processed.filter(
        (c) =>
          ...
          (c.customer_vat &&
            c.customer_vat.toLowerCase().includes(lowerTerm)) ||
          (c.customer_present_name &&
            c.customer_present_name.toLowerCase().includes(lowerTerm)),
      );
    }
    ...
    <Container fluid className="py-4 font-prompt">
            ...
            <Col lg={7}>
              <div className="d-flex flex-column flex-md-row gap-2 justify-content-lg-end">
                ...
                >
                  <FaSync
                    className={
                      isLoading ? "fa-spin text-success" : "text-success"
                    }
                  />
                </Button>
              </div>
      ...
      {isLoading ? (
        ...
        <Message variant="danger">
          {error?.data?.message || error.message || "Error loading customers"}
        </Message>
          ...
          <Card className="shadow-sm border-0 rounded-4 d-none d-lg-block overflow-hidden">
            <Table hover responsive className="mb-0 align-middle">
              <thead className="bg-light">
                <tr>
                  <th
                    className="py-3 ps-4 text-secondary text-uppercase small border-0 fw-bold"
                    style={{ width: "25%" }}
                  >
                    {t.headers.name}
                  </th>
                  <th
                    className="py-3 text-secondary text-uppercase small border-0 fw-bold"
                    style={{ width: "20%" }}
                  >
                    {t.headers.contact}
                  </th>
                  ...
                  >
                    ...
                    <td>
                      <div className="d-flex flex-column small">
                        {customer.customer_present_name && (
                          <span className="mb-1 text-dark fw-semibold">
                            <FaUserTie className="me-1 text-primary opacity-50" />{" "}
                            {customer.customer_present_name}
                          </span>
                        )}
                        {customer.customer_vat && (
                          <span className="text-muted font-monospace">
                            <FaIdCard className="me-1 text-success opacity-50" />{" "}
                            {customer.customer_vat}
                          </span>
                        )}
                      </div>
                    ...
                    <td>
                      <div className="text-muted small d-flex">
                        ...
                        <span className="text-truncate-2-lines">
