...
import { useCreateOrderMutation, useUploadPaymentSlipImageMutation } from '../slices/ordersApiSlice';
...
import {
    FaUniversity, FaMoneyBillWave, FaFileUpload, FaReceipt,
    FaQrcode, FaCopy, FaTimes, FaShieldAlt, FaChevronRight, FaTags, FaClock, FaCalendarAlt
} from 'react-icons/fa';
...
const PaymentScreen = () => {
    ...
    // ✅ เรียกใช้ API Hooks
    const [createOrder, { isLoading: isCreatingProductOrder }] = useCreateOrderMutation();
    ...
    // 🔥 กำหนดยอดเงิน
    const transferedAmount = orderType === 'product' ? cart.totalPrice : urlAmount;
    ...
    useEffect(() => {
        ...
        if (amountToPay > 0) {
            const payload = generatePayload(SHOP_CONFIG.promptPayID, { amount: amountToPay });
    ...
    const submitHandler = async (e) => {
        e.preventDefault();
        if (!image) {
            toast.error(language === 'thai' ? 'กรุณาแนบสลิปโอนเงิน' : 'Please upload payment slip');
            return;
        }
        if (!transferedName) {
            toast.error(language === 'thai' ? 'กรุณาเลือกบัญชีธนาคาร' : 'Please select a bank account');
            return;
        }
        ...
        if (orderType === 'product') {
            ...
            try {
                const cleanedOrderItems = cart.cartItems.map((item) => ({
                    name: item.name, qty: Number(item.qty), image: item.image, price: Number(item.price), product: item.product,
                }));
                ...
                const res = await createOrder({
                    ...
                    paymentResult: { paymentComfirmID, transferedName, transferedAmount: Number(transferedAmount), transferedDate, image, status: 'COMPLETED', email_address: userInfo.email },
    ...
    const t = {
        en: { stepTitle: 'Confirm Payment', scanQR: 'PromptPay QR', bankTransfer: 'Bank Account', amount: 'Amount to Transfer', date: 'Transaction Date', customer: 'Payer Name', upload: 'Upload Slip', confirm: 'Confirm Payment', summary: 'Payment Info', total: 'Net Amount', selectMethod: 'Choose Method', scanHint: 'Scan to pay exact amount', account: 'Transfer Account' },
        thai: { stepTitle: 'ยืนยันการชำระเงิน', scanQR: 'สแกนจ่าย (QR)', bankTransfer: 'โอนผ่านบัญชี', amount: 'ยอดเงินที่ต้องโอน', date: 'วันเวลาที่โอน', customer: 'ชื่อผู้โอน', upload: 'แนบหลักฐานการโอน (สลิป)', confirm: 'ยืนยันการชำระเงิน', summary: 'สรุปข้อมูลการชำระ', total: 'ยอดรวมสุทธิ', selectMethod: 'เลือกวิธีชำระเงิน', scanHint: 'สแกนเพื่อจ่ายยอดที่ถูกต้อง', account: 'โอนเข้าบัญชีธนาคาร' },
    }[language || 'en'];

    const getCategoryName = (type) => {
        switch (type) {
            case 'product': return { name: 'สินค้าทั่วไป (Cart)', color: 'text-black', bg: 'bg-slate-50' };
            case 'custom': return { name: 'สั่งทำ Custom PCB', color: 'text-black', bg: 'bg-slate-50' };
            case 'pcb': return { name: 'ก๊อปปี้ Copy PCB', color: 'text-black', bg: 'bg-slate-50' };
            case 'assembly': return { name: 'งานประกอบ (Assembly)', color: 'text-black', bg: 'bg-slate-50' };
            case 'orderpcb': return { name: 'ออเดอร์แผ่น PCB', color: 'text-black', bg: 'bg-slate-50' };
            default: return { name: 'รายการอื่นๆ', color: 'text-slate-600', bg: 'bg-slate-50' };
        }
    };
    const categoryInfo = getCategoryName(orderType);
    const isProcessing = isCreatingProductOrder || isCreatingCustom || isImageUploading;
        ...
        <div className="bg-[#fcfdfe] min-h-screen py-10 px-4 font-sans antialiased">
            <div className="max-w-6xl mx-auto">
                ...
                <div className="flex justify-center mb-8 md:mb-12 px-2">
                    <div className={`inline-flex items-center gap-2 px-4 md:px-5 py-2 rounded-full border border-slate-100 shadow-sm ${categoryInfo.bg}`}>
                        <FaTags className={categoryInfo.color} size={14} />
                        <span className={`text-[10px] md:text-sm font-black uppercase tracking-wider ${categoryInfo.color}`}>
                                ...
                                <AnimatePresence mode="wait">
                                    {paymentMethod === 'promptpay' ? (
                                        <motion.div key="qr" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }}>
                                            ...
                                            <h3 className="text-3xl font-black text-slate-900">{Number(transferedAmount).toLocaleString()} ฿</h3>
                                            <p className="text-slate-400 text-sm font-medium mt-2">{t.scanHint}</p>
                                        </motion.div>
                                    ) : (
                                        <motion.div key="bank" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-4">
                                            <div className="bg-slate-50 border border-slate-100 p-6 rounded-3xl text-start">
                                                <div className="flex items-center gap-4 mb-4">
                                                    <div className="w-12 h-12 bg-black rounded-2xl flex items-center justify-center text-white font-bold text-xl shadow-lg">K</div>
                                                    <div>
                                                        <p className="text-xs font-black text-slate-400 uppercase tracking-widest mb-1">Bank Name</p>
                                                ...
                                                <div className="bg-white p-4 rounded-2xl flex items-center justify-between border border-slate-100">
                                                    ...
                                                    <button type="button" onClick={() => copyToClipboard(SHOP_CONFIG.accNo)} className="p-2 bg-slate-50 text-slate-400 hover:text-black rounded-xl transition-all"><FaCopy /></button>
                                                </div>
                    ...
                    <div className="lg:col-span-7 text-start">
                        <div className="bg-white rounded-[2.5rem] shadow-sm border border-slate-100 overflow-hidden h-full flex flex-col relative z-10">
                            <div className="bg-slate-50/50 px-6 md:px-8 py-4 md:py-5 border-b border-slate-100 flex items-center gap-3">
                                <FaReceipt className="text-black" /> <h3 className="font-bold text-slate-800 text-sm md:text-base">{t.summary}</h3>
                            </div>

                            <form onSubmit={submitHandler} className="p-6 md:p-8 flex-grow flex flex-col gap-6">
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-start">
                                    <div className="md:col-span-2 space-y-1.5">
                                        <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">{t.customer}</label>
                                    ...
                                    <div className="space-y-1.5">
                                        ...
                                        <div className="w-full bg-slate-100 border border-slate-200 rounded-2xl px-4 py-3 text-sm font-black text-black">{Number(transferedAmount).toLocaleString()} ฿</div>
                                    </div>
                                    <div className="space-y-1.5">
                                ...
                                <div className="space-y-1.5 text-start">
                                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">{t.upload} <span className="text-rose-500">*</span></label>
                                ...
                                >
                                    {isProcessing ? <div className="w-6 h-6 border-4 border-white/30 border-t-white rounded-full animate-spin"></div> : <>{t.confirm} <FaChevronRight /></>}
                                </button>
                                <div className="mt-4 flex justify-center items-center gap-2 text-slate-400 text-[10px] font-black uppercase tracking-[0.2em]">
                                    <FaShieldAlt className="text-black" /> Secure Payment Processing
                                </div>
                            </form>
                        </div>
                    </div>
