...
const ServiceAllScreen = () => {
  ...
  if (isLoading) return <div className="min-h-screen flex justify-center items-center bg-[#FAFAFB]"><Loader /></div>;
  if (error) return <div className="max-w-7xl mx-auto px-4 py-12"><Message variant='danger'>{error?.data?.message || "Error"}</Message></div>;

  return (
    <div className="bg-[#FAFAFB] min-h-screen py-6 md:py-16 font-sans selection:bg-indigo-500 selection:text-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-8 md:mb-16 text-center md:text-left">
          <Link
            to='/'
            className="inline-flex items-center gap-2 text-sm font-medium text-slate-500 hover:text-indigo-600 mb-4 md:mb-6 transition-colors group"
          >
            <FaArrowLeft className="group-hover:-translate-x-1 transition-transform" />
            {t.goBackLbl}
          </Link>
          <h2 className="text-3xl md:text-4xl lg:text-5xl font-extrabold text-slate-900 tracking-tight mb-2 md:mb-4">
            <span className="bg-clip-text text-transparent bg-gradient-to-r from-slate-900 to-indigo-900">
              {t.ServicePosts}
            </span>
          </h2>
          <p className="text-slate-500 text-sm md:text-lg max-w-2xl mx-auto md:mx-0 leading-relaxed font-normal">
            {t.Subtitle}
          </p>
        </div>
          ...
          {data?.services?.map((service) => (
            <motion.div variants={itemVariants} key={service.ID} className="h-full">
              <Link
                ...
                className="group block h-full bg-white rounded-2xl md:rounded-[1.5rem] p-2.5 sm:p-4 border border-slate-50 shadow-sm hover:shadow-[0_12px_30px_rgba(79,70,229,0.1)] transition-all duration-300 flex flex-col"
              >
                ...
                <div className="flex flex-col flex-grow px-1">
                  ...
                  <div className="flex items-center gap-1.5 mb-1.5 sm:mb-3">
                    ...
                    <span className="text-indigo-600 font-bold text-[9px] sm:text-xs uppercase tracking-wider truncate">
                      {service.deploymentTypes || 'SERVICE'}
                    </span>
                  </div>

                  {/* Title - ปรับให้เล็กลงบนมือถือ */}
                  <h3 className="font-bold text-slate-900 text-sm sm:text-lg line-clamp-2 mb-1 sm:mb-2 leading-snug group-hover:text-indigo-600 transition-colors">
                    {language === 'thai' ? service.headerThaiOne : service.headerTextOne}
                  </h3>

                  {/* Description - ซ่อนบนจอมือถือ (hidden) และโชว์เฉพาะจอ sm ขึ้นไป */}
                  <p className="hidden sm:line-clamp-2 md:line-clamp-3 text-slate-500 text-sm mb-4 leading-relaxed flex-grow font-normal">
