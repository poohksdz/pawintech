...
const OrderassemblyPaymentScreen = () => {
  ...
  const { assemblyOrderDetails, shippingAddress, billingAddress } = assemblycart;
  ...
  // 3. ดึงข้อมูลจาก Redux / Context
  const { userInfo } = useSelector((state) => state.auth);

  // 4. ดึงข้อมูลตะกร้าจาก Database (กรณี Single ID)
  ...
  if (orderId) {
    ...
    totalCalculatedAmount = urlAmount > 0 ? urlAmount : Number(dbPrice);
  } else if (assemblyOrderDetails) {
    totalCalculatedAmount = assemblyOrderDetails.reduce((acc, item) => {
      const price = parseFloat(item.confirmed_price) || parseFloat(item.estimatedCost) || 0;
      return acc + price;
    }, 0);
  }

  const finalAmount = totalCalculatedAmount;
  ...
  const submitHandler = async (e) => {
    ...
    const payload = {
      orderItems: orderId ? [{ cartId: orderId }] : assemblyOrderDetails,
      userId: userInfo?._id,
    ...
    try {
      await createAssemblyPCB(payload).unwrap();

      // Clear localStorage used for checkout
      if (!orderId) {
        localStorage.removeItem("assemblycart");
      }
            ...
            <div className="bg-white rounded-[2.5rem] shadow-sm border border-slate-100 overflow-hidden h-full flex flex-col relative z-10">
              <div className="bg-slate-50/50 px-8 py-5 border-b border-slate-100 flex items-center justify-between gap-3">
                ...
                <span className="text-xs font-bold text-slate-500 bg-white px-3 py-1 rounded-lg border border-slate-200 shadow-sm truncate max-w-[200px]">
                  Project: {orderId
                    ? (fetchedData.projectname || "Checking...")
                    : (assemblyOrderDetails?.length > 1
                      ? `${assemblyOrderDetails[0].projectname} (+${assemblyOrderDetails.length - 1})`
                      : (assemblyOrderDetails?.[0]?.projectname || "Assembly Projects"))}
                </span>
              </div>
