...
const OrderassemblyPaymentScreen = () => {
    ...
    // 3. ดึงข้อมูลจาก Redux
    const { userInfo } = useSelector((state) => state.auth);
    const cart = useSelector((state) => state.cart);
    const { shippingAddress, receivePlace } = cart; 

    // 4. ดึงข้อมูลตะกร้าจาก Database
    const { data: cartData, isLoading: isCartLoading } = useGetAssemblycartByIdQuery(orderId, { 
        skip: !orderId 
    });

    // 🔥🔥 จุดแก้ไขสำคัญ: คำนวณยอดเงิน (URL > Confirmed Price > Estimated Cost) 🔥🔥
    // ถ้า Admin ยังไม่ confirm ราคา ให้ใช้ estimatedCost แทน
    ...
    const [createAssemblyPCB, { isLoading: isCreatingAssembly }] = useCreateAssemblyPCBMutation();
                                        ...
                                        <motion.div key="qr" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }}>
                                            {isCartLoading ? (
                                                ...
                                                <div className="p-10 border-2 border-dashed border-red-200 text-red-400 rounded-3xl mb-6 font-bold">
                                                    ไม่สามารถสร้าง QR ได้<br/>(ไม่พบยอดเงิน)
                                                </div>
                                            )}
                        ...
                        <div className="bg-white rounded-[2.5rem] shadow-sm border border-slate-100 overflow-hidden h-full flex flex-col relative z-10">
                            <div className="bg-slate-50/50 px-8 py-5 border-b border-slate-100 flex items-center justify-between gap-3">
                                <div className="flex items-center gap-3">
                                    <FaReceipt className="text-orange-600" /> <h3 className="font-bold text-slate-800">สรุปข้อมูลการชำระ</h3>
                                </div>
                            ...
                            <form onSubmit={submitHandler} className="p-8 flex-grow flex flex-col gap-6">
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-start">
                                    <div className="md:col-span-2 space-y-1.5">
                                        ...
                                        <input type="text" value={customerName} onChange={(e) => setCustomerName(e.target.value)} required className="w-full bg-slate-50 border border-slate-200 rounded-2xl px-4 py-3 text-sm font-bold focus:ring-4 focus:ring-orange-500/10 focus:border-orange-500 outline-none transition-all" />
                                    </div>
                                    ...
                                    <div className="space-y-1.5">
                                        ...
                                        <div className="w-full bg-slate-100 border border-slate-200 rounded-2xl px-4 py-3 text-sm font-black text-orange-600">{Number(finalAmount).toLocaleString()} ฿</div>
                                    </div>
                                    <div className="space-y-1.5">
                                ...
                                <div className="space-y-1.5 text-start">
                                    ...
                                    <div className={`relative group border-2 border-dashed rounded-3xl p-8 text-center transition-all duration-300 ${image ? 'border-green-400 bg-green-50/30' : 'border-slate-200 bg-slate-50/50 hover:border-orange-400'}`}>
                                ...
                                <div className="mt-4 flex justify-center items-center gap-2 text-slate-400 text-[10px] font-black uppercase tracking-[0.2em]">
                                    <FaShieldAlt className="text-orange-500" /> Secure Payment Processing
                                </div>
                            </form>
                        </div>
                    </div>
