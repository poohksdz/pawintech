...
const OrderAssemblyCartScreen = () => {
  ...
  const { data, isLoading, error, refetch } = useGetAssemblyCartByUserIDQuery(userInfo?._id, {
    skip: !userInfo?._id,
  ...
  const validOrders = useMemo(() => data?.data?.filter((order) => order.pcb_qty > 0) || [], [data]);
  ...
  const subTotal = selectedItems.reduce((acc, item) => {
    const price = item.status === 'accepted' ? (parseFloat(item.confirmed_price) || 0) : (parseFloat(item.estimatedCost) || 0);
    return acc + price;
  }, 0);
  const acceptedItemsCount = selectedItems.filter(o => o.status === 'accepted').length;
      ...
      default:
        return (
          <span className="inline-flex items-center gap-1.5 text-gray-500 bg-gray-50 px-2.5 py-1 rounded-md text-[9px] font-black uppercase tracking-widest border border-gray-100 animate-pulse whitespace-nowrap">
            <FaInfoCircle /> {language === 'thai' ? 'รอตรวจสอบ' : 'Pending Review'}
          </span>
        );
    }
  };
  ...
  if (error && error.status !== 404) return <div className="max-w-4xl mx-auto p-10"><Message variant="danger">{error?.data?.message || error.error}</Message></div>;
        ...
        <div className="flex-1 p-0 md:p-8 bg-white text-start">
          <div className="hidden md:grid grid-cols-12 pb-3 mb-6 border-b border-slate-200 text-[13px] uppercase tracking-wide">
            ...
            <div className="col-span-11 grid grid-cols-11 pl-4 gap-3 md:gap-4">
              <div className="col-span-1 md:col-span-4 font-semibold text-slate-900">{language === 'thai' ? 'รายการประกอบแผงวงจร' : 'Assembly Project'}</div>
              <div className="col-span-1 md:col-span-1 text-right font-semibold pr-4 text-slate-900">{language === 'thai' ? 'จำนวน' : 'Qty'}</div>
              <div className="col-span-1 md:col-span-3 text-center font-semibold text-slate-900">{language === 'thai' ? 'สถานะ' : 'Status'}</div>
              <div className="col-span-1 md:col-span-3 text-right font-semibold text-slate-900">{language === 'thai' ? 'ราคา' : 'Price'}</div>
            </div>
          </div>
            ...
            <AnimatePresence>
              {validOrders.length === 0 ? (
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="py-20 text-center flex flex-col items-center justify-center">
                  ...
                  <h3 className="text-xl font-bold text-gray-800 mb-2">{language === 'thai' ? 'ไม่มีสินค้าในตะกร้าของคุณ' : 'Your assembly cart is empty'}</h3>
                  <Link to="/assemblypcb" className="text-blue-500 hover:text-black hover:underline mt-4 tracking-wide uppercase font-bold text-[12px] transition-colors">{language === 'thai' ? 'เริ่มสั่งผลิตใหม่' : 'Start New Order'}</Link>
                </motion.div>
              ) : (
                          ...
                          <div className="flex gap-3 md:gap-5 items-start md:items-center w-full">
                            ...
                            <div className="w-16 h-16 md:w-24 md:h-24 bg-slate-50 shrink-0 flex items-center justify-center overflow-hidden rounded-xl border border-slate-100 shadow-sm group-hover:shadow-md transition-shadow">
                              {order.image_top_1 ? (
                                ...
                                <FaRobot size={24} className="text-gray-400 md:hidden" />
                              )}
                              <div className="hidden md:block">
                                {!order.image_top_1 && <FaRobot size={36} className="text-gray-400" />}
                              </div>
                            </div>

                            {/* Details Context */}
                            <div className="flex flex-col flex-1 min-w-0 py-0.5 md:py-0 text-start md:pr-6 relative">
                              <Link to={`/assemblycartpcb/${order.id}`} className="text-[13px] md:text-[14px] font-black text-slate-900 uppercase hover:text-blue-600 transition-colors leading-snug mb-0.5 truncate block w-full pr-6 md:pr-0">
                          ...
                          <div className="flex md:hidden items-center justify-between w-full bg-slate-50 border border-slate-200 px-3 py-1.5 rounded-xl">
                            <div className="flex flex-col min-w-0">
                              <span className="text-[8px] font-black tracking-[0.15em] uppercase text-slate-400 mb-0.5">
                                {order.status === 'accepted' ? (language === 'thai' ? 'ยอดที่อนุมัติ' : 'Confirmed') : (language === 'thai' ? 'ประมาณการ' : 'Estimate')}
                              </span>
                              <div className="flex items-center gap-1.5">
                                ...
                                <span className="text-[9px] font-bold text-slate-400">/ {order.pcs_qty || order.qty || 0} PCS</span>
                              </div>
                            </div>
                        ...
                        <div className="hidden md:flex col-span-3 items-center justify-end gap-3 text-right">
                          <div className="flex flex-col items-end mr-2">
                            ...
                            <span className="text-[9px] text-gray-400 font-bold uppercase tracking-tighter mt-0.5">
                              {order.status === 'accepted' ? 'Confirmed' : 'Estimate'}
                            </span>
                          </div>
                          {order.status !== 'paid' && (
                            <button
          ...
          <div className="mt-8 pt-4">
            <Link to="/assemblypcb" className="text-[11px] font-bold text-gray-400 hover:text-black transition-colors uppercase tracking-[0.2em] flex items-center gap-2">
            ...
            <div className="mt-4">
              ...
              <div className="bg-gray-50 border border-dashed border-gray-200 rounded p-6 mb-6">
                <p className="text-[11px] text-gray-500 font-medium leading-relaxed mb-4">
                  <strong className="text-gray-900 uppercase tracking-widest block mb-1 text-[10px]">Assembly evaluation</strong>
                  Our technicians must manually verify the BOM and CPL files to provide a final accurate quotation.
                </p>
        ...
        {showConfirmModal && createPortal(
          <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 overflow-hidden" style={{ width: '100vw', height: '100vh', top: 0, left: 0 }}>
            ...
            <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }} className="relative bg-white rounded shadow-2xl w-full max-w-sm p-8 text-center border-t-4 border-black font-sans z-10">
              <h3 className="text-[16px] font-bold text-gray-900 mb-2 uppercase tracking-tight">Remove Assembly?</h3>
              <p className="text-[14px] text-gray-600 mb-8 font-medium">Are you sure you want to remove this assembly order? This action cannot be undone.</p>
              <div className="flex gap-3">
                ...
                <button onClick={handleCancel} className="flex-1 py-3 bg-black text-white font-bold text-[11px] uppercase tracking-widest hover:bg-gray-800 transition-colors shadow-lg">Remove</button>
              </div>
            </motion.div>
          </div>,
          document.body
