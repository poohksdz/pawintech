...
import {
  Container, Row, Col, Form, Button, Card, ListGroup, Image, ButtonGroup, ToggleButton, Badge, InputGroup
} from 'react-bootstrap';
import { toast } from 'react-toastify';
import { useSelector } from 'react-redux';
import { useNavigate, useParams } from 'react-router-dom';
import { FaSave, FaCheckCircle, FaTimesCircle, FaFileArchive, FaImages, FaCalculator } from 'react-icons/fa';
...
const OrderassemblyOrderEditScreen = () => {
  ...
  // API Hooks
  const [updateAssemblyPCB, { isLoading: isUpdating }] = useUpdateAssemblyPCBMutation();
  ...
  const { data: configData, isLoading: isFetchingConfig } = useGetAssemblyPCBByIdQuery(id);
  ...
  const totalCost = qty * (smdCost + thtCost) + stencilPrice + (parseFloat(formData.setup_price) || 0) + (parseFloat(formData.delievery_price) || 0);
  ...
  useEffect(() => {
    if (configData?.success && configData.data) {
      ...
      const mapImages = (prefix) => {
        ...
        for (let i = 1; i <= 10; i++) {
          if (d[`${prefix}_${i}`]) {
            imgs.push({ url: getFullUrl(d[`${prefix}_${i}`]), raw: d[`${prefix}_${i}`], file: null });
          }
        }
        return imgs;
      };
  ...
  const uploadImages = async (images) => {
    ...
    const oldPaths = images.filter(img => !img.file).map(img => img.raw || img.url);
  ...
  const submitAction = async (newStatus) => {
    ...
    try {
      ...
      if (newStatus === 'rejected') {
        if (!formData.confirmed_reason || formData.confirmed_reason.trim() === '') {
      ...
      const payload = {
        ...
        confirmed_price: newStatus === 'rejected' ? 0 : formData.confirmed_price,
      };

      await updateAssemblyPCB({ id, updatedData: payload }).unwrap();
      toast.success(`Order ${newStatus === 'accepted' ? 'Approved' : newStatus === 'rejected' ? 'Rejected' : 'Saved'} Successfully!`);
  ...
  return (
    <Container className="py-4 font-prompt">
      <div className="d-flex justify-content-between align-items-center mb-4">
        ...
        <Badge bg={formData.status === 'accepted' ? 'success' : formData.status === 'rejected' ? 'danger' : 'warning'} className="fs-6">
        ...
        <Row className="g-4">
          ...
          <Col lg={8}>
            ...
            <Card className="shadow-sm border-0 rounded-4 mb-4">
              <Card.Header className="bg-white border-bottom-0 pt-4 px-4"><h5 className="fw-bold">{t.projectInfo}</h5></Card.Header>
              <Card.Body className="px-4 pb-4">
                <Row className="g-3">
                  <Col md={8}>
                    <Form.Group>
                      <Form.Label className="text-muted small mb-1">{t.projectnamelbl}</Form.Label>
                      <Form.Control type="text" name="projectname" value={formData.projectname} onChange={handleChange} required className="fw-bold" />
                    </Form.Group>
                  </Col>
                  <Col md={4}>
                    <Form.Group>
                      <Form.Label className="text-muted small mb-1">{t.qtylbl}</Form.Label>
                      <Form.Control type="number" name="pcb_qty" value={formData.pcb_qty} onChange={handleChange} min="1" required />
                    </Form.Group>
                  </Col>
                </Row>
              </Card.Body>
            ...
            {/* 2. Technical Specs */}
            <Card className="shadow-sm border-0 rounded-4 mb-4">
              <Card.Header className="bg-white border-bottom-0 pt-4 px-4"><h5 className="fw-bold">{t.specs}</h5></Card.Header>
              <Card.Body className="px-4 pb-4">
                <Row className="g-3 mb-3">
                  <Col md={6}>
                    <Form.Group>
                      <Form.Label className="text-muted small mb-1">{t.width}</Form.Label>
                      <InputGroup>
                  ...
                  <Col md={6}>
                    <Form.Group>
                      <Form.Label className="text-muted small mb-1">{t.height}</Form.Label>
                      <InputGroup>
                  ...
                  {showSMDFields && (
                    <div className="bg-light p-3 rounded-3">
                      <Row className="g-3">
                        <Col md={4}>
                          ...
                          <ButtonGroup className="w-100">
                            {['Top', 'Bottom', 'Both'].map((side, idx) => (
                              <ToggleButton key={idx} id={`smd-${side}`} type="radio" variant="outline-primary" name="smd_side" value={side} checked={formData.smd_side === side} onChange={handleChange} size="sm">
                                {side}
                              </ToggleButton>
                            ))}
                          </ButtonGroup>
                        ...
                        <Col md={4}>
                          <Form.Label className="small">{t.smdCount}</Form.Label>
                          <Form.Control type="number" name="count_smd" value={formData.count_smd} onChange={handleChange} size="sm" />
                        </Col>
                        <Col md={4}>
                          <Form.Label className="small">{t.smdPins}</Form.Label>
                          <Form.Control type="number" name="total_point_smd" value={formData.total_point_smd} onChange={handleChange} size="sm" />
                        </Col>
                      </Row>
                    </div>
                  )}
                    ...
                    <div className="bg-light p-3 rounded-3">
                      <Row className="g-3">
                        <Col md={4}>
                          ...
                          <ButtonGroup className="w-100">
                            {['Top', 'Bottom', 'Both'].map((side, idx) => (
                              <ToggleButton key={idx} id={`tht-${side}`} type="radio" variant="outline-primary" name="tht_side" value={side} checked={formData.tht_side === side} onChange={handleChange} size="sm">
                                {side}
                              </ToggleButton>
                            ))}
                          </ButtonGroup>
                        ...
                        <Col md={4}>
                          <Form.Label className="small">{t.thtCount}</Form.Label>
                          <Form.Control type="number" name="count_tht" value={formData.count_tht} onChange={handleChange} size="sm" />
                        </Col>
                        <Col md={4}>
                          <Form.Label className="small">{t.thtPins}</Form.Label>
                          <Form.Control type="number" name="total_point_tht" value={formData.total_point_tht} onChange={handleChange} size="sm" />
                        </Col>
                      </Row>
                    </div>
                  )}
            ...
            {/* 3. Files */}
            <Card className="shadow-sm border-0 rounded-4 mb-4">
              <Card.Header className="bg-white border-bottom-0 pt-4 px-4"><h5 className="fw-bold">{t.files}</h5></Card.Header>
              <Card.Body className="px-4 pb-4">
                <Row className="g-4">
                  <Col md={6}>
                    <Form.Group>
                      <Form.Label className="fw-bold small"><FaImages className="me-1" /> {t.topImages}</Form.Label>
                      <Form.Control type="file" multiple accept="image/*" onChange={(e) => handleImageUpload(e, 'top')} size="sm" />
                      <div className="d-flex flex-wrap mt-2 gap-2">
                        {topImages.map((img, idx) => (
                          <div key={idx} className="position-relative">
                            <Image src={img.url} thumbnail style={{ width: '60px', height: '60px', objectFit: 'cover' }} />
                            <FaTimesCircle className="text-danger position-absolute top-0 end-0 bg-white rounded-circle" style={{ cursor: 'pointer' }} onClick={() => removeImage(idx, 'top')} />
                          </div>
                        ))}
                      </div>
                    </Form.Group>
                  ...
                  <Col md={6}>
                    <Form.Group>
                      <Form.Label className="fw-bold small"><FaImages className="me-1" /> {t.bottomImages}</Form.Label>
                      <Form.Control type="file" multiple accept="image/*" onChange={(e) => handleImageUpload(e, 'bottom')} size="sm" />
                      <div className="d-flex flex-wrap mt-2 gap-2">
                        {bottomImages.map((img, idx) => (
                          <div key={idx} className="position-relative">
                            <Image src={img.url} thumbnail style={{ width: '60px', height: '60px', objectFit: 'cover' }} />
                            <FaTimesCircle className="text-danger position-absolute top-0 end-0 bg-white rounded-circle" style={{ cursor: 'pointer' }} onClick={() => removeImage(idx, 'bottom')} />
                          </div>
                        ))}
                      </div>
                    </Form.Group>
                  ...
                  <Col md={12}>
                    <Form.Group>
                      <Form.Label className="fw-bold small"><FaFileArchive className="me-1" /> {t.zipFile}</Form.Label>
                      <Form.Control type="file" accept=".zip,.rar" onChange={handleChange} size="sm" />
                      {formData.gerber_zip && (
                        <div className="mt-1 small">
                          Current: <a href={getFullUrl(typeof formData.gerber_zip === 'string' ? formData.gerber_zip : formData.gerber_zip.name)} target="_blank" rel="noreferrer" className="text-decoration-none">
                            {typeof formData.gerber_zip === 'string' ? formData.gerber_zip.split('/').pop() : formData.gerber_zip.name}
                          </a>
                        </div>
                      )}
                    </Form.Group>
            ...
            <div className="sticky-top" style={{ top: '20px', zIndex: 10 }}>
                ...
                <Card.Body className="p-4">
                  ...
                  <ListGroup variant="flush" className="bg-transparent">
                    ...
                    <ListGroup.Item className="d-flex justify-content-between bg-white rounded-3 px-3 py-3 mt-2 shadow-sm">
                      ...
                      <strong className="fs-5">{totalCost.toLocaleString()} ฿</strong>
                    </ListGroup.Item>
                  </ListGroup>
                </Card.Body>
              </Card>
              ...
              <Card className="shadow border-0 rounded-4 border-top border-5 border-primary">
                <Card.Body className="p-4">
                  ...
                  {/* Approve Section */}
                  <div className="mb-4">
                    <Form.Label className="fw-bold text-success small">{t.lblConfirmPrice}</Form.Label>
                  ...
                  {/* Reject Section */}
                  <div className="mb-4">
                    <Form.Label className="fw-bold text-danger small">{t.lblReason}</Form.Label>
                    <Form.Control
                      as="textarea"
                      rows={3}
                  ...
                  {/* Save Draft */}
                  <Button variant="light" className="w-100 rounded-pill text-muted small" onClick={() => submitAction(formData.status)}>
