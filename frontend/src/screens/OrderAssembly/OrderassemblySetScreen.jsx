          ...
          {options.map((opt) => {
            ...
            return (
              ...
              >
                ...
                {/* ไอคอนและข้อความ */}
                <span
                  className={`text-xl md:text-2xl mb-1 relative z-10 transition-colors duration-300 ${isActive ? "text-white" : "text-slate-200"}`}
                >
                  {opt.icon}
                </span>
                <span
                  className={`text-[10px] md:text-[10px] font-black uppercase tracking-widest relative z-10 transition-colors duration-300 ${isActive ? "text-white" : "text-slate-400"}`}
                >
                  {opt.label}
                </span>
              </button>
          ...
          <AnimatePresence mode="wait">
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <span className="text-xs md:text-xs font-black text-black uppercase tracking-widest whitespace-nowrap">
                  {options.find((o) => o.id === activeCart).full}
                </span>
              </div>
              <span className="text-xs md:text-xs font-black text-slate-300 uppercase tracking-widest">
                {options.find((o) => o.id === activeCart).desc}
              </span>
            </motion.div>
          </AnimatePresence>
        </div>
        ...
        <div className="bg-white rounded-[2.5rem] shadow-2xl shadow-black/5 border border-slate-100 min-h-[500px] overflow-hidden relative">
          ...
          <div className="p-4 md:p-8 lg:p-12">
            <AnimatePresence mode="wait">
              ...
              >
                {activeCart === "youpart" && (
                  <OrderassemblyCartYouSupplyPartScreen />
                )}
                {activeCart === "wepart" && (
                  <OrderassemblyCartWeSupplyPartScreen />
                )}
                {activeCart === "both" && (
                  <OrderassemblyCartYouSupplySomePartWedotherestScreen />
                )}
              </motion.div>
            </AnimatePresence>
          </div>
