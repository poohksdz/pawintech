...
import {
  FaCheck, FaDownload, FaImage, FaArrowLeft, FaHashtag,
  FaLayerGroup, FaRulerCombined, FaMicrochip, FaRobot, FaCheckCircle, FaTimesCircle, FaInfoCircle
} from 'react-icons/fa';
...
const OrderassemblyCartDetailScreen = () => {
  ...
  const handleDownloadAll = async (e) => {
    ...
    try {
      if (order.gerber_zip) {
        ...
        folder.file(order.gerber_zip.split('/').pop() || 'gerber.zip', gerberBlob);
      }
  ...
  if (isError) return <div className="p-10"><Message variant="danger">{error.message}</Message></div>;
  ...
  const SpecItem = ({ label, value, confirmed, icon: Icon }) => (
    <div className="flex items-center justify-between p-4 bg-white border border-slate-100 rounded-2xl hover:border-indigo-100 transition-colors">
      <div className="flex items-center gap-3">
        {Icon && <div className="w-8 h-8 rounded-lg bg-slate-50 flex items-center justify-center text-slate-400"><Icon size={14} /></div>}
        <div>
          <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest leading-none mb-1">{label}</p>
          <p className="text-[14px] font-bold text-slate-900 leading-tight">{value || 'N/A'}</p>
        </div>
      </div>
      <div>
        {confirmed === 1 ? (
          <div className="w-6 h-6 rounded-full bg-emerald-50 text-emerald-500 flex items-center justify-center border border-emerald-100"><FaCheck size={10} /></div>
        ) : (
          <div className="w-6 h-6 rounded-full bg-rose-50 text-rose-500 flex items-center justify-center border border-rose-100"><TbExclamationMark size={14} /></div>
        )}
      </div>
    </div>
  );
      ...
      <div className="max-w-6xl mx-auto space-y-12">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div className="space-y-4">
            <Link to="/cart/assemblycart" className="inline-flex items-center gap-2 text-slate-400 hover:text-indigo-600 transition-colors font-black text-[11px] uppercase tracking-widest">
              <FaArrowLeft size={10} /> Back to Cart
            </Link>
            <h1 className="text-4xl font-black text-slate-900 tracking-tight leading-tight uppercase">
              {order.projectname}
              <span className="block text-lg font-bold text-slate-400 tracking-normal capitalize mt-1">Assembly Project Details</span>
            </h1>
          </div>
          <div className="flex items-center gap-3">
            <button
          ...
          <div className="lg:col-span-8 space-y-10">
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <SpecItem label={t.ProjectName} value={order.projectname} confirmed={order.confirmProjectName} icon={FaHashtag} />
                <SpecItem label={t.Quantity} value={`${order.pcb_qty} PCS`} confirmed={order.confirmQty} icon={FaLayerGroup} />
                <SpecItem label={t.BoardType} value={order.board_types} confirmed={order.confirmBoardType} icon={FaLayerGroup} />
                <SpecItem label={t.Dimensions} value={`${order.width_mm} x ${order.high_mm} mm`} confirmed={order.confirmWidth && order.confirmHeight} icon={FaRulerCombined} />
                <SpecItem label={t.SMDDetails} value={`${order.count_smd} Parts / ${order.total_point_smd} Pins (${order.smd_side})`} confirmed={order.confirmCountSmd} icon={FaMicrochip} />
                <SpecItem label={t.THTDetails} value={`${order.count_tht} Parts / ${order.total_point_tht} Pins (${order.tht_side})`} confirmed={order.confirmCountTht} icon={FaRobot} />
              </div>
            </div>
            ...
            <div className="space-y-6">
              ...
              <div className="bg-white rounded-[2rem] border border-slate-200/60 p-8 shadow-sm space-y-8">
                <div>
                  <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-3">Customer Note</h4>
                ...
                {order.confirmed_reason && (
                  <div className="pt-8 border-t border-slate-100">
                    <h4 className="text-[10px] font-black text-indigo-400 uppercase tracking-[0.2em] mb-4">Engineer's Response</h4>
          ...
          <div className="lg:col-span-4 space-y-10">
            ...
            <div className="bg-slate-900 rounded-[2.5rem] p-8 text-white shadow-2xl shadow-slate-200/50 space-y-8">
              <div className="space-y-1 border-b border-white/5 pb-4">
                <h3 className="text-[11px] font-black uppercase tracking-[0.2em] text-indigo-400">{t.Status}</h3>
                <p className="text-xl font-black capitalize">{order.status || 'Pending'}</p>
              </div>

              <div className="space-y-6">
                <div>
                  <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">{t.Estimated}</p>
                  <p className="text-2xl font-black tabular-nums">{order.estimatedCost || '-'}</p>
                </div>
                <div className="pt-4 border-t border-white/5">
                  <p className="text-[10px] font-black text-emerald-400 uppercase tracking-widest mb-1">{t.Confirmed}</p>
                  <p className="text-3xl font-black tabular-nums text-white">
                    {order.confirmed_price ? `฿${parseFloat(order.confirmed_price).toLocaleString()}` : 'Wait Quote'}
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                {[...Array(10)].flatMap((_, i) => [`image_top_${i + 1}`, `image_bottom_${i + 1}`])
        ...
        {zoomedImage && (
          ...
          >
            ...
            <div className="absolute top-8 right-8 text-white/50 font-black text-[11px] uppercase tracking-widest flex items-center gap-3">
              Press ESC to close <div className="w-10 h-10 rounded-full border border-white/20 flex items-center justify-center">?</div>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
