...
import {useCreateQuotationMutation, useGetQuotationDetailsQuery, useUploadQuotationImageMutation, useUpdateQuotationByQuotationNoMutation, useUploadQuotationPDFMutation, useGetQuotationByQuotationNoQuery} from "../../slices/quotationApiSlice"; 
...
const QuotationSetSelectedCustomerScreen = () => {
  ...
  const { data, isLoading: isLoadingData, isError: isErrorData, error: errorData } = useGetQuotationDetailsQuery(id);
 ...
 const [today, setToday] = useState(() => {
  ...
  const { data: defaultData, isLoading, isError, error } = useGetDefaultQuotationUsedQuery();
  const defaultSelected = defaultData?.quotations?.[0] || null;

  const [createQuotation, { isLoading: isLoadingCreateQuotation, isError: isErrorCreateQuotation, error: errorCreateQuotation }] = useCreateQuotationMutation();
  const [uploadQuotationPDF, {isLoading: isLoadingUploadQuotationPDF, isError: isErrorUploadQuotationPDF, error: errorUploadQuotationPDF  }] = useUploadQuotationPDFMutation(); 
  const [updateQuotationByQuotationNo, {isLoading: isLoadingUpdateQuotationByQuotationNo, isError: isErrorUpdateQuotationByQuotationNo, error: errorUpdateQuotationByQuotationNo  }] = useUpdateQuotationByQuotationNoMutation();
...
const handleCustomerChange = (field, value) => {
  ...
  const handleChange = (index, field, value) => {
    ...
    const hasData = Object.values(lastRow).some(val => val !== "" && val !== 0);
    if (hasData) {
      setRows(prev => [
        ...prev,
  ...
  if (isError) return <p>Error: {error?.message || "Failed to load default quotation"}</p>;
...
const uploadPDF = async (quotation_no) => {
  ...
  formData.append("quotationPDF", pdfBlob, `Quotation_${quotation_no || Date.now()}.pdf`);
  ...
  grand_total: grandTotal,
    total: grandTotal,
      ...
      signatures: {
          ...
          sales_person_signature_date: defaultSelected?.sales_person_signature_date || null,
          sales_manager_signature: defaultSelected?.sales_manager || null,
          sales_manager_signature_date: defaultSelected?.sales_manager_signature_date || null,
      },
    };
...
const handleCreateQuotation = async () => {
  ...
  return (
    <>
    {isLoading || isLoadingData || isLoadingCreateQuotation || isLoadingUploadQuotationPDF || isLoadingUpdateQuotationByQuotationNo ? (
        <Loader />
      ) :
    <>
      <div style={{ display: "flex", justifyContent: "center", width: "100%", backgroundColor: "#f0f0f0", padding: "5px 0" }}>
        <div
          ref={componentRef}
          style={{
            width: "210mm",
        ...
        >
          ...
          <div style={{ textAlign: "center", position: "relative" }}>
            <div style={{ position: "absolute", top: 30, left: 0 }}>
              <img src={defaultSelected?.logo} alt="Logo" style={{ maxHeight: "35px", objectFit: "contain" }} />
            </div>
              ...
              <p style={{ lineHeight: 2 }}>
                 <strong>ติดต่อจาก:</strong>
                  <input
                    ...
                    style={{ border: "none", marginLeft: "5px", width: "370px" }}
                  /><br />
                  ...
                  <input
                    ...
                    style={{ border: "none", marginLeft: "5px", width: "400px" }}
                  /><br />
                  ...
                  <input
                    ...
                    style={{ border: "none", marginLeft: "5px", width: "300px" }}
                  /><br /> 
                <input type="checkbox" className="form-check-input" style={{ marginTop: "7px" }} /> <strong>สำนักงานใหญ่</strong>
                <input type="checkbox" className="form-check-input" style={{ marginLeft: "12px", marginTop: "7px"  }} /> <strong>สาขา</strong>
...
/>
  ...
  <br />
    ...
    <input
      ...
      style={{ width: "40px", border: "none", outline: "none", marginLeft: "5px", backgroundColor: "transparent", lineHeight: "2"  }}
    /> วัน<br />
    ...
    /> วัน
          ...
          <div style={{ flexGrow: 1 }}>
            <div style={{ display: "grid", gridTemplateColumns: "0.5fr 1fr 2.7fr 0.7fr 0.7fr 1fr 1fr", border: "1px solid #dee2e6", width: "100%", maxWidth: "180mm" }}>
              {["ลำดับ", "รหัสสินค้า", "รายละเอียด", "จำนวน", "หน่วย", "ราคาต่อหน่วย", "จำนวนเงิน"].map((header, i) => (
                <div key={i} style={{ textAlign: "center", padding: "2px 4px", fontWeight: "bold", borderRight: i === 6 ? "none" : "1px solid #dee2e6", fontSize: "12px" }}>
                  {header}
                </div>
              ))}
              {rows.map((row, index) => (
                <React.Fragment key={index}>
                  <div style={{ gridColumn: "1", textAlign: "center", alignContent: "start", borderTop: "1px solid #dee2e6", borderRight: "1px solid #dee2e6", padding: "2px 4px", fontSize: "12px" }}>{index + 1}</div>
                <div
  style={{
    gridColumn: "2",
...
>
  ...
  />
                  ...
                  <div style={{ gridColumn: "7", textAlign: "right",  alignContent: "start", borderTop: "1px solid #dee2e6", padding: "2px 3px", fontSize: "12px" }}>
                    {row.qty && row.unit_price ? (parseFloat(row.qty) * parseFloat(row.unit_price)).toFixed(2) : ""}
                  </div>
                </React.Fragment>
              ))}
              ...
              {/* Summary Rows */}
              {[
                [`*** มัดจำ ${defaultSummary.deposit} % ***`, `ส่วนลด ${defaultSummary.discount || ""} %`, totalDiscount],
                [`${defaultSummary.bank_account_name || ""}\n${defaultSummary.bank_account_number || ""}`, "มูลค่าสินค้าหลังหักส่วนลด", totalAfterDiscount],
                ["", `ภาษีมูลค่าเพิ่ม ${defaultSummary.vat || ""} %`, totalVat],
                ["", "ยอดรวมสุทธิ", grandTotal],
              ].map(([note, label, value], i) => (
                <React.Fragment key={i}>
                  <div style={{ gridColumn: "1 / span 4", textAlign: "center", padding: "2px 4px", borderTop: i === 0 ? "1px solid #dee2e6" : "none", borderRight: "1px solid #dee2e6", fontWeight: "normal", fontSize: "12px" }}>{note}</div>
                  <div style={{ gridColumn: "5 / span 2", textAlign: "left", padding: "2px 4px", borderTop: "1px solid #dee2e6", borderRight: "1px solid #dee2e6", fontWeight: "bold", fontSize: "12px" }}>{label}</div>
                  <div style={{ gridColumn: "7", textAlign: "right", padding: "2px 4px", borderTop: "1px solid #dee2e6", fontWeight: "bold", fontSize: "12px" }}>{parseFloat(value).toFixed(2)}</div>
                </React.Fragment>
              ))}
            </div>
          </div>
...
>
  ...
  <div
    style={{
      ...
      marginLeft: role === "buyer_approves" ? "-10px" : "0", // shift only buyer_approves
    }}
  >
    {role !== "buyer_approves" && (
      <img
      ...
      <div style={{ display: "flex", alignItems: "center", width: "100%", marginTop: "2px" }}>
        <span style={{ flex: "0 0 25%", textAlign: "left" }}>ลงวันที่</span>
        <div style={{
          flex: "1",
          borderBottom: "1px solid #7b8a8b",
          height: "25px",
