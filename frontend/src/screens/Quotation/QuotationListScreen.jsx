...
import { FaTrash, FaEdit, FaSearch, FaPlus, FaFilePdf, FaUser, FaCalendarAlt, FaFileInvoiceDollar, FaFilter, FaSync } from 'react-icons/fa';
import { toast } from 'react-toastify';
import { Link } from 'react-router-dom';
import Loader from '../../components/Loader';
import Message from '../../components/Message';
import { useGetQuotationsQuery, useDeleteQuotationByQuotationNoMutation } from '../../slices/quotationApiSlice';
...
const QuotationListScreen = () => {
  ...
  const [deleteQuotationByQuotationNo, { isLoading: isDeleting }] = useDeleteQuotationByQuotationNoMutation();
  ...
  const handleDelete = async () => {
    try {
      await deleteQuotationByQuotationNo(selectedQuotation.quotation_no).unwrap();
      toast.success(`Quotation ${selectedQuotation.quotation_no} deleted successfully`);
  ...
  const uniqueQuotations = useMemo(() => {
    return data?.quotations?.filter(
      (q, index, self) =>
        index === self.findIndex((item) => item.quotation_no === q.quotation_no)
    ) || [];
  }, [data]);
  ...
  const displayedQuotations = useMemo(() => {
    return uniqueQuotations.filter(q => {
      const matchCustomer = !filterCustomer || q.customer_name === filterCustomer;
      const matchPresenter = !filterPresenter || q.customer_present_name === filterPresenter;
      const matchDate = !filterDate || new Date(q.date).toLocaleDateString() === new Date(filterDate).toLocaleDateString();
  ...
  }, [uniqueQuotations, filterCustomer, filterPresenter, filterDate, searchQuery]);

  // Get Lists for Dropdowns
  const customerList = useMemo(() => Array.from(new Set(uniqueQuotations.map(q => q.customer_name))), [uniqueQuotations]);
  const presenterList = useMemo(() => Array.from(new Set(uniqueQuotations.map(q => q.customer_present_name))), [uniqueQuotations]);

  if (isLoading) return <div className="min-h-screen flex justify-center items-center"><Loader /></div>;
  if (isError) return <div className="p-4"><Message variant="danger">{error?.data?.message || error.message}</Message></div>;
  ...
  return (
    <div className="py-8 px-4 sm:px-6 lg:px-8 bg-slate-50 min-h-screen font-sans">
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="bg-white shadow-sm border border-slate-200 rounded-3xl p-6 sm:p-8 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h4 className="text-2xl font-black text-slate-900 tracking-tight">Quotations</h4>
              <p className="text-slate-500 font-medium mt-1">Manage your sales quotations</p>
            </div>
          </div>
        ...
        <div className="bg-white shadow-sm border border-slate-200 rounded-3xl p-6">
          <div className="grid grid-cols-1 md:grid-cols-12 gap-4 items-end">
            ...
            <div className="md:col-span-4 lg:col-span-3">
              <label className="block text-xs font-bold text-slate-500 uppercase tracking-widest mb-2">Customer</label>
              <select
            ...
            <div className="md:col-span-4 lg:col-span-2">
              <label className="block text-xs font-bold text-slate-500 uppercase tracking-widest mb-2">Presenter</label>
              <select
              ...
              >
                ...
                {presenterList.map(name => <option key={name} value={name}>{name}</option>)}
              </select>
            </div>
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-slate-100">
                {displayedQuotations.length === 0 ? (
                  <tr><td colSpan="6" className="px-6 py-12 text-center text-slate-500 font-medium">No quotations found.</td></tr>
                ) : (
                    ...
                    <tr key={q.id} className="hover:bg-slate-50/50 transition-colors group">
                      ...
                      <td className="px-6 py-4">
                        ...
                        <div className="text-xs text-slate-500 flex items-center gap-1.5 mt-1 font-medium">
                          <FaUser className="text-slate-400" /> {q.customer_present_name}
                        </div>
                      </td>
                      ...
                      <td className="px-6 py-4 text-right font-black text-slate-900 text-lg tracking-tight">
                        {parseFloat(q.grand_total).toLocaleString(undefined, { minimumFractionDigits: 2 })} ฿
                      </td>

                      {/* Actions */}
                      <td className="px-6 py-4">
                        <div className="flex justify-center items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                          <Link to={`/admin/customers/selectedcustomer/${q.id}/set`} title="Duplicate / Create New">
                          ...
                          <Link to={`/admin/quotations/${q.quotation_no}/edit`} title="Edit">
        ...
        <div className="lg:hidden grid grid-cols-1 sm:grid-cols-2 gap-4">
          {displayedQuotations.length === 0 ? (
            <div className="col-span-full bg-white p-8 rounded-3xl text-center text-slate-500 font-medium shadow-sm border border-slate-200">No quotations found.</div>
          ) : (
            displayedQuotations.map((q) => (
              <div key={q.id} className="bg-white rounded-3xl shadow-sm border border-slate-200 p-5 flex flex-col hover:border-blue-200 transition-colors">
                <div className="flex justify-between items-start mb-4">
                  <div className="flex items-center gap-3">
                    <span className="bg-blue-100 text-blue-800 font-bold uppercase tracking-wider rounded-md font-mono text-sm px-2.5 py-1">{q.quotation_no}</span>
                    {q.quotation_pdf && (
                      <button onClick={() => handleViewPdf(q.quotation_pdf)} className="text-rose-500 p-1 hover:text-rose-600 focus:outline-none">
                        <FaFilePdf size={18} />
                      </button>
                    )}
                  </div>
                  <span className="text-xs font-semibold text-slate-500 flex items-center gap-1.5 bg-slate-50 px-2 py-1 rounded-md border border-slate-100">
                    ...
                    {new Date(q.date).toLocaleDateString('th-TH', { day: '2-digit', month: '2-digit', year: '2-digit' })}
                  </span>
                </div>

                <div className="mb-4">
                  <h6 className="font-bold text-slate-900 text-lg leading-tight mb-1">{q.customer_name}</h6>
                ...
                <div className="mt-auto border-t border-slate-100 pt-4 flex flex-col gap-4">
                  ...
                  <div className="flex gap-2">
                    <Link to={`/admin/customers/selectedcustomer/${q.id}/set`} className="flex-1">
      ...
      <Modal
        ...
        title={<span className="flex items-center gap-2 text-rose-600"><FaTrash /> Delete Quotation</span>}
      >
        <div className="py-4 text-slate-600">
          <p className="mb-2 text-lg">Are you sure you want to delete <strong className="text-slate-900 font-mono">{selectedQuotation?.quotation_no}</strong>?</p>
          <p className="text-sm text-slate-500 font-medium">This action cannot be undone and will permanently remove this quotation from the system.</p>
        </div>
