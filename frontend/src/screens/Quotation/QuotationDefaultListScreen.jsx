...
const QuotationDefaultListScreen = () => {
  ...
  const [deleteQuotation, { isLoading: isDeleting }] = useDeleteDefaultQuotationMutation();
  ...
  if (error) return <div className="p-4"><Message variant="danger">{error?.data?.message || error.error}</Message></div>;
  ...
  return (
    <div className="py-8 px-4 sm:px-6 lg:px-8 bg-slate-50 min-h-screen font-sans">
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="bg-white shadow-sm border border-slate-200 rounded-3xl p-6 sm:p-8 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h4 className="text-2xl font-black text-slate-900 tracking-tight">Default Quotations</h4>
              <p className="text-slate-500 font-medium mt-1">Manage quotation templates and settings</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {data.quotations && data.quotations.length > 0 ? (
              data.quotations.map((q) => (
                <div key={q.id} className={`relative flex flex-col p-6 rounded-2xl border ${q.is_used === 1 ? 'border-green-500 bg-green-50/30 shadow-md ring-1 ring-green-500/50' : 'border-slate-200 bg-white shadow-sm hover:border-blue-300'} transition-all`}>

                  {/* Default Badge/Checkbox */}
                  <div className="absolute top-4 right-4">
                    <label className="flex items-center cursor-pointer group" title={q.is_used === 1 ? "Current Default" : "Set as Default"}>
                      <input
                        type="checkbox"
                        checked={q.is_used === 1}
                        ...
                        className="sr-only"
                      />
                      <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center transition-colors ${q.is_used === 1 ? 'bg-green-500 border-green-500 text-white shadow-sm shadow-green-500/30' : 'bg-slate-100 border-slate-300 text-transparent group-hover:border-green-400'}`}>
                        <FaCheckCircle size={14} className={q.is_used === 1 ? 'block' : 'hidden'} />
                      </div>
                    </label>
                  </div>

                  <div className="mb-4 pr-8">
                    <h5 className="text-lg font-bold text-slate-900 leading-tight">{q.company_name}</h5>
                  </div>

                  <div className="space-y-2 mb-6 flex-grow text-sm font-medium">
                    <div className="flex items-start gap-2 text-slate-600">
                      <span className="font-bold text-slate-400 min-w-[60px] uppercase text-[10px] tracking-wider mt-1">Branch</span>
                      <span className="text-slate-900">{q.branch_name}</span>
                    </div>
                    <div className="flex items-start gap-2 text-slate-600">
                      <span className="font-bold text-slate-400 min-w-[60px] uppercase text-[10px] tracking-wider mt-1">Contact</span>
                  ...
                  <div className="flex items-center gap-2 pt-4 border-t border-slate-100/60 mt-auto">
                    <button
                      onClick={() => navigate(`/admin/defaultquotations/${q.id}`)}
                    ...
                    <button
                      onClick={() => navigate(`/admin/defaultquotations/${q.id}/edit`)}
              ...
              <div className="col-span-full py-12 text-center text-slate-500 font-medium">
                No default quotations found. Create your first template to get started.
              </div>
            )}
          </div>
        </div>
      ...
      <Modal
        ...
        title={<span className="flex items-center gap-2 text-rose-600"><FaTrash /> Confirm Delete</span>}
      >
        <div className="py-4 text-slate-600">
          <p className="mb-2 text-lg">Are you sure you want to delete this default quotation?</p>
          <p className="text-sm text-slate-500 font-medium">This action cannot be undone and will permanently remove this template from the system.</p>
        </div>
