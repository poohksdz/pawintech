...
const QuotationDefaultSetScreen = () => {
  const navigate = useNavigate()
  const [createDefaultQuotation, { isLoading }] = useCreateDefaultQuotationMutation()
  ...
  const handleSubmit = async (e) => {
    ...
    const logoUrl = await handleUploadImage(logo, 'logo')
    const buyerUrl = await handleUploadImage(buyerApproves, 'buyer')
    const salesPersonUrl = await handleUploadImage(salesPerson, 'salesPerson')
    const salesManagerUrl = await handleUploadImage(salesManager, 'salesManager')

    try {
      await createDefaultQuotation({
      ...
      toast.success('Default Quotation created successfully')
      navigate('/admin/defaultquotations')
    } catch (error) {
      toast.error(error?.data?.message || 'Failed to create Default Quotation')
    }
  }
    ...
    <div className="py-8 px-4 sm:px-6 lg:px-8 bg-slate-50 min-h-screen font-sans">
      <div className="max-w-5xl mx-auto space-y-6">
        ...
        <div className="flex items-center gap-4 mb-8">
          <Link to="/admin/defaultquotations" className="p-2 bg-white text-slate-500 hover:text-blue-600 hover:bg-blue-50 rounded-xl transition-colors shadow-sm border border-slate-200">
            <FaArrowLeft className="w-5 h-5" />
          </Link>
          <h2 className="text-2xl font-black text-slate-800 tracking-tight">Create Default Quotation</h2>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6 lg:space-y-8">
          ...
          <div className="bg-white shadow-sm border border-slate-200 rounded-3xl p-6 sm:p-8">
            <h3 className="text-lg font-bold text-slate-800 mb-6 pb-2 border-b border-slate-100">Branding & Signatures</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              ...
              <div>
                <label className="block text-sm font-bold text-slate-700 mb-2">Buyer Approves</label>
                <input
                  type="file"
                  accept="image/*"
                  onChange={(e) => handleFileChange(e, setBuyerApproves, setBuyerApprovesPreview)}
                  className="block w-full text-sm text-slate-500 file:mr-4 file:py-2 file:px-4 file:rounded-xl file:border-0 file:text-sm file:font-semibold file:bg-blue-50 file:text-blue-600 hover:file:bg-blue-100 transition-colors cursor-pointer"
                />
                {buyerApprovesPreview && <Thumbnail src={buyerApprovesPreview} />}
              </div>
              <div>
                <label className="block text-sm font-bold text-slate-700 mb-2">Sales Person</label>
                <input
                  type="file"
                  accept="image/*"
                  onChange={(e) => handleFileChange(e, setSalesPerson, setSalesPersonPreview)}
                  className="block w-full text-sm text-slate-500 file:mr-4 file:py-2 file:px-4 file:rounded-xl file:border-0 file:text-sm file:font-semibold file:bg-blue-50 file:text-blue-600 hover:file:bg-blue-100 transition-colors cursor-pointer"
                />
                {salesPersonPreview && <Thumbnail src={salesPersonPreview} />}
              </div>
              <div>
                <label className="block text-sm font-bold text-slate-700 mb-2">Sales Manager</label>
                <input
                  type="file"
                  accept="image/*"
                  onChange={(e) => handleFileChange(e, setSalesManager, setSalesManagerPreview)}
                  className="block w-full text-sm text-slate-500 file:mr-4 file:py-2 file:px-4 file:rounded-xl file:border-0 file:text-sm file:font-semibold file:bg-blue-50 file:text-blue-600 hover:file:bg-blue-100 transition-colors cursor-pointer"
                />
          ...
          <div className="bg-white shadow-sm border border-slate-200 rounded-3xl p-6 sm:p-8">
            <h3 className="text-lg font-bold text-slate-800 mb-6 pb-2 border-b border-slate-100">Company Information</h3>
          ...
          <div className="bg-white shadow-sm border border-slate-200 rounded-3xl p-6 sm:p-8">
            <h3 className="text-lg font-bold text-slate-800 mb-6 pb-2 border-b border-slate-100">Financial & Accounting Settings</h3>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
              <Input
        ...
        {/* View Image Modal */}
        <Modal isOpen={modalShow} onClose={() => setModalShow(false)} title="Image Preview">
          <div className="p-4 flex justify-center bg-slate-100 rounded-lg">
            {modalImage && <img src={modalImage} alt="Preview Zoom" className="max-w-full max-h-[70vh] object-contain rounded-md shadow-sm" />}
          </div>
        </Modal>
      </div>
    </div>
...
export default QuotationDefaultSetScreen
