...
const ReorderPCBAdminCreateOrderPCBScreen = () => {
  ...
  const {
    ...
    error: orderError,
  } = useGetOrderPCBByorderIDQuery(orderID);
  ...
  useEffect(() => {
    if (orderData) {
      ...
      if (order) {
        ...
        const storedPrice =
          Number(order.quoted_price_to_customer) ||
          Number(order.total_amount_cost) ||
          0;
        if (storedPrice > 0) {
  ...
  const getkilogram = () => {
    const area =
      (Number(formData.dimensions.x) || 0) *
      (Number(formData.dimensions.y) || 0);
    const thickness = getThicknessPrice();
    const weight =
      area * 0.0000035 * (Number(formData.pcbQty) || 5) * thickness;
  ...
  const getEmsDeliveryPrice = () => {
    ...
    return emsRates.length > 0
      ? Number(emsRates[emsRates.length - 1].price)
      : 0;
  };
  ...
  const getDhlDeliveryPrice = () => {
    ...
    return dhlRates.length > 0
      ? Number(dhlRates[dhlRates.length - 1].price)
      : 0;
  };
  ...
  const getSurfaceFinishPrice = () => {
    ...
    const selected = surfaceFinishes.find(
      (s) => s.name === formData.surfaceFinish,
    );
    return Number(selected?.price) || 0;
  };
  ...
  const getCopperWeightPrice = () => {
    ...
    const selected = copperWeights.find(
      (c) => c.name === formData.copperWeight,
    );
    return Number(selected?.price) || 0;
  };
  ...
  const getBaseMaterialPrice = () => {
    ...
    const selected = baseMaterials.find(
      (m) => m.name === formData.baseMaterial,
    );
    return Number(selected?.price) || 0;
  };
  ...
  const pricing = useMemo(() => {
    ...
    const materialCost =
      area * pricePerCm * qty + extraFee + dhlRate + dhlService + addons;
  ...
  if (isOrderLoading || isShippingLoading)
    return (
  ...
  if (orderError)
    return (
      <Message variant="danger">
        {orderError?.data?.message || "Error loading order"}
      </Message>
    );
          ...
          <div className="space-y-6 lg:col-span-2">
            ...
            <div className="rounded-3xl border border-slate-200 bg-white p-8 shadow-sm">
              ...
              <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
                ...
                <InfoBlock
                  label="Quantity"
                  value={`${formData.pcbQty} Units`}
                  icon={<FaBoxOpen />}
                />
                <InfoBlock
                  label="Layers"
                  value={`${formData.layers} Layers`}
                  icon={<FaLayerGroup />}
                />
                <InfoBlock
                  label="Thickness"
                ...
                <div>
                  ...
                  <div className="rounded-2xl bg-slate-50 p-5">
                    ...
                    <p className="text-sm font-medium text-slate-500 mt-1">
                      {customerInfo.customerCompanyName || "Personal Account"}
                    </p>
                    <p className="text-sm text-slate-500 mt-1">
                      {customerInfo.customerPhoneNumber}
                    </p>
                    <p className="text-sm text-slate-500">
                      {customerInfo.customerEmailAddress}
                    </p>
                  </div>
                </div>
            ...
            <div className="sticky top-10 space-y-6">
                    ...
                    <>
                      <div className="flex justify-between text-sm text-slate-400">
                        ...
                        <span className="font-mono text-slate-300">
                          ฿{(pricing.subtotal + pricing.addons).toFixed(2)}
                        </span>
                      </div>
                      <div className="flex justify-between text-sm text-slate-400">
                        <span>Shipping</span>
              ...
              <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
                <h3 className="mb-4 flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-slate-400">
                  ...
                  Override{" "}
                  <span className="text-[9px] font-normal lowercase bg-slate-100 px-2 py-0.5 rounded-full text-slate-500 ml-auto">
                    (Optional)
                  </span>
                </h3>
