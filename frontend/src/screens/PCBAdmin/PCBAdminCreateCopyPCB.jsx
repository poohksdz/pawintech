...
const PCBAdminCreateCopyPCB = () => {
  ...
  const [formData, setFormData] = useState({ pcbQty: 5, projectname: '', zipFile: null });
  ...
  const [createcopyPCBbyAdmin, { isLoading: creating }] = useCreatecopyPCBbyAdminMutation();
        ...
        <Row className="mb-4">
          <Col xl={8}>
              ...
              <div className="d-flex flex-wrap mt-3 gap-2">
                {frontImages.map((img, idx) => (
                  <div key={img.url || idx} style={{ position: 'relative' }}>
                    <Image src={img.url} alt={`Front ${idx + 1}`} thumbnail style={{ width: 100, height: 100, objectFit: 'cover' }} />
                    <Button variant="danger" size="sm" style={{ position: 'absolute', top: 4, right: 4 }} onClick={() => removeImage(idx, 'front')}>×</Button>
                  </div>
                ))}
              </div>
            </Card>
            ...
            <Card className="p-3 mb-4">
              ...
              <div className="d-flex flex-wrap mt-3 gap-2">
                {backImages.map((img, idx) => (
                  <div key={img.url || idx} style={{ position: 'relative' }}>
                    <Image src={img.url} alt={`Back ${idx + 1}`} thumbnail style={{ width: 100, height: 100, objectFit: 'cover' }} />
                    <Button variant="danger" size="sm" style={{ position: 'absolute', top: 4, right: 4 }} onClick={() => removeImage(idx, 'back')}>×</Button>
                  </div>
                ))}
              </div>
            </Card>
            ...
            <Card className="p-3 mb-4">
              ...
              <Form.Control type="file" accept=".zip,.rar" onChange={(e) => handleChange('zipFile', e.target.files?.[0] || null)} />
              {formData.zipFile && <div className="mt-2 text-muted">Selected: {formData.zipFile.name}</div>}
            </Card>

            <Card className="p-3 mb-4">
              <Card.Title>Additional Notes</Card.Title>
              <Form.Control as="textarea" rows={7} placeholder="Enter any specific details or requirements..." value={notes} onChange={(e) => setNotes(e.target.value)} />
            </Card>
            ...
            <Form.Group className="my-2" controlId="ReceiveSelect">
              ...
              <div className="d-flex justify-content-between">
                <Form.Check type="radio" label={t('ReceiveProductBySendingLabel', 'Receive product by sending')} name="receiveAddressFormat" id="receiveBySending" value="bysending" onChange={handleRadioReceiveChange} checked={!isReceiveCompleteSelected} />
                <Form.Check type="radio" label={t('ReceiveProductAtCompanyLabel', 'Receive product at company')} name="receiveAddressFormat" id="receiveAtCompany" value="atcompany" onChange={handleRadioReceiveChange} checked={isReceiveCompleteSelected} />
              </div>
            </Form.Group>
            ...
            {!isReceiveCompleteSelected && (
              <>
                <Form.Group className="my-2" controlId="shippingname">
                  ...
                  <Form.Control type="text" placeholder={t('namePlaceholder', 'Enter name')} value={shippingname} onChange={(e) => setShippingname(e.target.value)} required />
                </Form.Group>

                <Form.Group className="my-2" controlId="address">
                  <Form.Label>{t('addressLabel', 'Address')}</Form.Label>
                  <Form.Control type="text" placeholder={t('address', 'Enter address')} value={address} onChange={(e) => setAddress(e.target.value)} required />
                </Form.Group>

                <Form.Group className="my-2" controlId="city">
                  <Form.Label>{t('cityLabel', 'City')}</Form.Label>
                  <Form.Control type="text" placeholder={t('city', 'Enter city')} value={city} onChange={(e) => setCity(e.target.value)} required />
                </Form.Group>

                <Form.Group className="my-2" controlId="postalCode">
                  <Form.Label>{t('postalCodeLabel', 'Postal Code')}</Form.Label>
                  <Form.Control type="text" placeholder={t('postalCode', 'Enter postal code')} value={postalCode} onChange={(e) => setPostalCode(e.target.value)} required />
                </Form.Group>

                <Form.Group className="my-2" controlId="country">
                  <Form.Label>{t('countryLabel', 'Country')}</Form.Label>
                  <Form.Control type="text" placeholder={t('country', 'Enter country')} value={country} onChange={(e) => setCountry(e.target.value)} required />
                </Form.Group>

                <Form.Group className="my-2" controlId="phone">
                  <Form.Label>{t('phoneLabel', 'Phone')}</Form.Label>
                  <Form.Control type="text" placeholder={t('phone', 'Enter phone')} value={phone} onChange={(e) => setPhone(e.target.value)} required />
                </Form.Group>
                ...
                <Form.Group className="my-2" controlId="BillingSelect">
                  ...
                  <div className="d-flex justify-content-between">
                    <Form.Check type="radio" label={t('ShortBillingAddressLabel', 'Short billing address')} name="billingAddressFormat" id="billingShort" value="short" onChange={handleRadioChange} checked={!isBillingCompleteSelected} />
                    <Form.Check type="radio" label={t('CompleteBillingAddressLabel', 'Complete billing address')} name="billingAddressFormat" id="billingComplete" value="complete" onChange={handleRadioChange} checked={isBillingCompleteSelected} />
                  </div>
                </Form.Group>
                ...
                {isBillingCompleteSelected && (
                  <>
                    ...
                    <Form.Group className="my-2" controlId="billingName">
                      <Form.Label>{t('nameLabel', 'Name')}</Form.Label>
                      <Form.Control type="text" placeholder={t('namePlaceholder', 'Enter name')} value={billingName} onChange={(e) => setBillingName(e.target.value)} required />
                    </Form.Group>

                    <Form.Group className="my-2" controlId="billinggAddress">
                      <Form.Label>{t('addressLabel', 'Address')}</Form.Label>
                      <Form.Control type="text" placeholder={t('address', 'Enter address')} value={billinggAddress} onChange={(e) => setBillinggAddress(e.target.value)} required />
                    </Form.Group>

                    <Form.Group className="my-2" controlId="billingCity">
                      <Form.Label>{t('cityLabel', 'City')}</Form.Label>
                      <Form.Control type="text" placeholder={t('city', 'Enter city')} value={billingCity} onChange={(e) => setBillingCity(e.target.value)} required />
                    </Form.Group>

                    <Form.Group className="my-2" controlId="billingPostalCode">
                      <Form.Label>{t('postalCodeLabel', 'Postal Code')}</Form.Label>
                      <Form.Control type="text" placeholder={t('postalCode', 'Enter postal code')} value={billingPostalCode} onChange={(e) => setBillingPostalCode(e.target.value)} required />
                    </Form.Group>

                    <Form.Group className="my-2" controlId="billingCountry">
                      <Form.Label>{t('countryLabel', 'Country')}</Form.Label>
                      <Form.Control type="text" placeholder={t('country', 'Enter country')} value={billingCountry} onChange={(e) => setBillingCountry(e.target.value)} required />
                    </Form.Group>

                    <Form.Group className="my-2" controlId="billingPhone">
                      <Form.Label>{t('phoneLabel', 'Phone')}</Form.Label>
                      <Form.Control type="text" placeholder={t('phone', 'Enter phone')} value={billingPhone} onChange={(e) => setBillingPhone(e.target.value)} required />
                    </Form.Group>

                    <Form.Group className="my-2" controlId="tax">
                      <Form.Label>{t('TaxLabel', 'Tax ID')}</Form.Label>
                      <Form.Control type="text" placeholder={t('Tax', 'Enter tax id')} value={tax} onChange={(e) => setTax(e.target.value)} required />
                    </Form.Group>
                  </>
                )}
              </>
          ...
          <Col xl={4}>
            <Card className="p-4 mb-3 align-items-center">
              ...
              <Form.Group as={Row} className="mb-3  w-100">
                ...
                <Form.Control type="number" placeholder={t('transferedAmountPlaceholder')} value={transferedAmount} onChange={(e) => setTransferedAmount(e.target.value)} required />
              ...
              <Form.Group as={Row} className="mb-3  w-100">
                ...
                <Form.Control type="datetime-local" value={transferedDate} onChange={(e) => setTransferedDate(e.target.value)} required />
              </Form.Group>
              ...
              {status === 'accepted' && (
                <Form.Group className="mb-3 w-100" controlId="confirmedPriceInput">
                  ...
                  <Form.Control className="w-100" type="number" min="0" step="0.01" value={confirmedPrice} onChange={(e) => setConfirmedPrice(e.target.value)} />
                </Form.Group>
              )}

              <Form.Group className="mb-3 w-100" controlId="confirmedReason">
                <Form.Label>{t('ConfirmedreasonLbl', 'Confirmed Reason')}</Form.Label>
                <Form.Control className="w-100" as="textarea" rows={5} value={confirmedReason} onChange={(e) => setConfirmedReason(e.target.value)} />
              </Form.Group>
              ...
              {slipImagePath && (
                <div style={{ position: 'relative', marginBottom: 12 }}>
                  ...
                  <Button variant="danger" size="sm" onClick={removeSlipImage} style={{ position: 'absolute', top: 8, right: 8 }}>Remove</Button>
                </div>
              )}

              <Form.Group className="mb-2 w-100">
                <Form.Label>PCB Quantity</Form.Label>
                <Form.Control type="number" min="5" value={formData.pcbQty} onChange={(e) => handleChange('pcbQty', Math.max(5, parseInt(e.target.value || 0, 10)))} />
                <Button type="submit" className="w-100 mt-3" disabled={isLoading || creating}>
