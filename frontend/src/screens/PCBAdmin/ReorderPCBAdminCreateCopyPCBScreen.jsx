...
const ReorderCopyPCBToCartScreen = () => {
  ...
  const { data, isLoading, error } =
    useGetcopyPCBByOrderIdQuery(orderID, { skip: !orderID })

  const [createcopycart, { isLoading: creating }] =
    useCreatecopycartMutation()
    ...
    if (data?.data?.length) {
      ...
      for (let i = 1; i <= 10; i++) {
        if (order[`front_image_${i}`])
          front.push({ url: getFullUrl(order[`front_image_${i}`]), raw: order[`front_image_${i}`] })
        if (order[`back_image_${i}`])
          back.push({ url: getFullUrl(order[`back_image_${i}`]), raw: order[`back_image_${i}`] })
      }
        ...
        <Row>
          <Col md={8}>
            ...
            {['front', 'back'].map(type => (
              <Card className="p-3 mb-3" key={type}>
                ...
                <Form.Control type="file" multiple onChange={e => handleUpload(e, type)} />
                <div className="d-flex flex-wrap gap-2 mt-2">
                  {(type === 'front' ? frontImages : backImages).map((img, i) => (
                    <Image key={i} src={img.url} thumbnail style={{ width: 100 }} />
                  ))}
                </div>
              </Card>
            ))}
            ...
            <Card className="p-3 mb-3">
              ...
              <Form.Control type="file" onChange={e => setZipFile(e.target.files[0])} />
            </Card>
          ...
          <Col md={4}>
            <Card className="p-3">
              ...
              <Form.Control type="number" min={1} value={pcbQty} onChange={e => setPcbQty(e.target.value)} />
