...
const createStockRequestgood = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    for (const item of items) {
      const {
        reqqty, img, electotronixPN, value, category, subcategory, footprint,
        weight, position, price: unitprice, manufacture, manufacturePN, supplier,
        supplierPN, moq, spq, link, process, description, alternative, note,
        important, important_message, username, _id: ID,
      } = item

      await db.pool.query(query, [
        requestno, requestdate, requesttime, reqqty, img, electotronixPN, value,
        category, subcategory, footprint, weight, position, unitprice, manufacture,
        manufacturePN, supplier, supplierPN, moq, spq, link, process, description,
        alternative, note, important, important_message, updateImportanceBy,
        username, requestedUser, userId, ID,
      ])
    }
...
const updateStockRequestgood = asyncHandler(async (req, res) => {
  const { id } = req.params
  const {
    requestno, requestdate, requesttime, requestqty, img, electotronixPN, value,
    footprint, category, subcategory, description, position,
    manufacturePN, manufacture, weight, alternative, process, note, unitprice,
    username, user_id,
  } = req.body
  ...
  try {
    ...
    res.status(200).json({
      ...
      requestgood: {
        ...
        value, footprint, category, subcategory, description, position,
        manufacturePN, manufacture, weight, alternative, process, note, unitprice,
        username, user_id,
      },
    })
...
const getStockRequestImportanceByUserId = asyncHandler(async (req, res) => {
  try {
    ...
    res.status(500).json({ message: 'Error fetching requestimportance by user' })
  }
})
