...
const createProduct = asyncHandler(async (req, res) => {
  ...
  const {
    ...
    description, datasheet, manual, price, countInStock,
    nameThai, descriptionThai, brandThai, categoryThai, videoLink,
  } = req.body
  ...
  try {
    ...
    const [result] = await pool.query(query, [
      productCode, name, image, mutipleImage || '', brand, category,
      description, datasheet, manual, 5, 5, price || 0,
      countInStock || 0, nameThai, descriptionThai, brandThai, categoryThai,
      5, videoLink
    ])
...
const updateProduct = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    await pool.query(query, [
      productCode, name, image, mutipleImage, brand, category,
      description, datasheet, manual, price, countInStock,
      nameThai, descriptionThai, brandThai, categoryThai, videoLink, id,
    ])
...
const deleteProduct = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    const [rows] = await pool.query('SELECT image, mutipleImage, datasheet, manual FROM products WHERE _id = ?', [id])
...
const updateReviewsProduct = asyncHandler(async (req, res) => {
  ...
  try {
    await pool.query('UPDATE products SET user = ?, reviews = ? WHERE _id = ?', [userID, reviews, id])
...
const updateShowFrontProduct = asyncHandler(async (req, res) => {
  ...
  try {
    await pool.query('UPDATE products SET showFront = ? WHERE _id = ?', [showFront, id])
...
const createProductReview = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    res.status(error.status || 500).json({ message: error.message || 'Error adding review' })
  }
})

module.exports = {
  getProducts, getProductById, createProduct, updateProduct, deleteProduct,
  updateReviewsProduct, updateShowFrontProduct, createProductReview,
}