...
const createService = asyncHandler(async (req, res) => {
  ...
  if (
    !headerTextOne && !bodyTextOne && !headerThaiOne && !bodyTextThaiOne &&
    !headerTextTwo && !bodyTextTwo && !headerThaiTwo && !bodyTextThaiTwo &&
    !headerTextThree && !bodyTextThree && !headerThaiThree && !bodyTextThaiThree &&
    !headerTextFour && !bodyTextFour && !headerThaiFour && !bodyTextThaiFour &&
    !headerTextFive && !bodyTextFive && !headerThaiFive && !bodyTextThaiFive &&
    !imageOne && !imageTwo && !imageThree && !imageFour && !imageFive
  ) {
    res.status(400)
  ...
  try {
    ...
    const [result] = await pool.query(query, [ // ✅ ใช้ pool.query
      headerTextOne, headerTextTwo, headerTextThree, headerTextFour, headerTextFive,
      bodyTextOne, bodyTextTwo, bodyTextThree, bodyTextFour, bodyTextFive,
      headerThaiOne, headerThaiTwo, headerThaiThree, headerThaiFour, headerThaiFive,
      bodyTextThaiOne, bodyTextThaiTwo, bodyTextThaiThree, bodyTextThaiFour, bodyTextThaiFive,
...
const updateService = asyncHandler(async (req, res) => {
  ...
  try {
    const [existingService] = await pool.query( // ✅ ใช้ pool.query
...
const updateShowFrontService = asyncHandler(async (req, res) => {
  ...
  try {
    const [service] = await pool.query( // ✅ ใช้ pool.query
      'SELECT * FROM services WHERE ID = ?',
      [id]
    )
    ...
    res.status(200).json({ message: 'Service show status updated successfully' })
