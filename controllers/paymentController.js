  ...
  try {
    ...
    const formatData = (items) => items.map(item => {
      ...
      if (slipPath) {
        ...
        if (!slipPath.startsWith('uploads/') && !slipPath.startsWith('http')) {
      ...
      return {
        ...
        isPaid: item.isPaid === 1 || item.status === 'Paid' || item.status === 'COMPLETED' || item.status === 'accepted' || item.status === 'paid',
        isDelivered: item.isDelivered === 1 || item.isDelivered === true,
        deliveryID: item.deliveryID || '',
        isManufacting: item.isManufacting === 1 || item.isManufacting === true,
...
const updatePaymentStatus = asyncHandler(async (req, res) => {
  ...
  try {
    if (orderType === 'product') {
      ...
      await pool.query(`UPDATE orders SET isPaid = ?, status = ?, updatedAt = NOW() WHERE id = ?`, [isPaid, status, id])
      ...
      await pool.query(`UPDATE order_pcbs SET status = ?, updated_at = NOW() ${extraFields} WHERE id = ?`, [dbStatus, ...extraValues, id])
    } else {
      // Tables: pcb_custom_orders, pcb_copy_carts, pcb_assembly_carts
      let table = ''
      switch (orderType) {
        case 'custom': table = 'pcb_custom_orders'; break;
        case 'pcb': table = 'pcb_copy_carts'; break;
        case 'assembly': table = 'pcb_assembly_carts'; break;
        default: return res.status(400).json({ message: 'Invalid order type' })
      }
      ...
      await pool.query(`UPDATE ${table} SET status = ?, updated_at = NOW() WHERE id = ?`, [dbStatus, id])

      // Custom PCB also has a relation to carts table
      if (orderType === 'custom') {
        await pool.query(`UPDATE pcb_custom_carts SET status = ?, updated_at = NOW() WHERE id = (SELECT cartId FROM pcb_custom_orders WHERE id = ?)`, [dbStatus, id])
      }
    }

    res.json({ message: `อัปเดตสถานะเป็น ${status} เรียบร้อยแล้ว`, status, id })
