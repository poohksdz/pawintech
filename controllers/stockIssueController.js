...
const createStockIssuegoods = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    for (const item of items) {
      const {
        ID, requestno, requestdate, requesttime, requestqty, img, electotronixPN,
        value, category, subcategory, footprint, weight, position, unitprice,
        manufacture, manufacturePN, supplier, supplierPN, moq, spq, link, process,
        description, alternative, note, username, requestedUser, user_id, product_id, issueqty,
      } = item
      ...
      await connection.query(insertQuery, [
        issueno, issuedate, issuetime, issueQtyNum, requestno, formattedRequestDate,
        requesttime, Number(requestqty), electotronixPN, value, footprint, category,
        subcategory, description, position, supplierPN, supplier, manufacturePN,
        manufacture, weight, Number(unitprice), moq, spq, link, alternative, process,
        note, img, username, requestedUser, user_id, issueUser, product_id,
      ])
    }
...
const updateStockIssuegoods = asyncHandler(async (req, res) => {
  const { id } = req.params
  const {
    issueno, issuedate, issuetime, issueqty, requestno, requestdate, requesttime,
    electotronixPN, value, footprint, category, subcategory, description, position,
    supplierPN, supplier, manufacturePN, manufacture, weight, unitprice, moq, spq,
    link, alternative, process, note, img, username, reciever,
  } = req.body
  ...
  try {
    ...
    const [result] = await db.pool.query(query, [
      ...
      electotronixPN, value, footprint, category, subcategory, description, position,
      supplierPN, supplier, manufacturePN, manufacture, weight, unitprice, moq, spq,
      link, alternative, process, note, img, username, reciever, id,
    ])
    ...
    res.status(200).json({
      ...
      issuegoods: {
        id, issueno, issuedate, issuetime, issueqty, requestno, requestdate,
        requesttime, electotronixPN, value, footprint, category, subcategory,
        description, position, supplierPN, supplier, manufacturePN, manufacture,
        weight, unitprice, moq, spq, link, alternative, process, note, img,
        reciever, username,
      },
    })
  } catch (error) {
