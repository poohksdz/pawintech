...
// ⚠️⚠️⚠️ สำคัญมาก: เปลี่ยนชื่อตารางตรงนี้ให้ตรงกับในฐานข้อมูลของคุณ
...
const createOrderPCB = asyncHandler(async (req, res) => {
    ...
    if (!paymentSlip) {
        return res.status(400).json({ success: false, message: '⚠️ กรุณาแนบรูปสลิปโอนเงิน' });
    }
    ...
    const paymentConfirmId = paymentResult?.paymentComfirmID || `PAY-${getTimestamp()}`;
    ...
    try {
        ...
        res.status(201).json({ success: true, message: 'บันทึกคำสั่งซื้อสำเร็จ', orderId: baseOrderId });
    } catch (error) {
...
const getOrderPCBByorderID = asyncHandler(async (req, res) => {
    const [rows] = await pool.query(`SELECT * FROM \`${tableName}\` WHERE orderID = ?`, [req.params.orderID]);
    if (rows.length === 0) return res.status(404).json({ success: false, message: 'Not found' });
    res.status(200).json({ success: true, data: rows[0] });
});
...
const createOrderPCBbyAdmin = asyncHandler(async (req, res) => {
    ...
    try {
        ...
        await pool.query(sql, values);
        res.status(201).json({ success: true, message: 'Admin order created successfully', orderId: baseOrderId });
    } catch (error) {
...
const verifyPaymentPCB = asyncHandler(async (req, res) => {
    ...
    const [result] = await pool.query(sql, [manufactureOrderNumber || '', req.params.id, req.params.id, req.params.id]);

    if (result.affectedRows > 0) {
        res.status(200).json({ success: true, message: 'ยืนยันยอดเงินเรียบร้อย ระบบเปลี่ยนสถานะเป็น กำลังผลิต' });
    } else {
...
const updateDeliveryPCB = asyncHandler(async (req, res) => {
    ...
    const [result] = await pool.query(sql, [transferedNumber, req.params.id, req.params.id, req.params.id]);

    if (result.affectedRows > 0) {
        res.status(200).json({ success: true, message: 'อัปเดตสถานะจัดส่ง พร้อมบันทึกเลขพัสดุเรียบร้อย' });
    } else {
        res.status(404).json({ success: false, message: 'ไม่พบคำสั่งซื้อนี้' });
    }
});
...
const deleteOrderPCB = asyncHandler(async (req, res) => {
    const { id } = req.params;
    try {
        const [rows] = await pool.query(`SELECT paymentSlip, gerberZip FROM \`${tableName}\` WHERE id = ?`, [id]);
        if (rows.length > 0) {
            const order = rows[0];
...
const getOwnShippingRates = asyncHandler(async (req, res) => {
    try {
        const [defaultPricing] = await pool.query('SELECT * FROM orderpcbdefaultprice ORDER BY created_at DESC LIMIT 1');
        const [shippingRates] = await pool.query('SELECT * FROM shipping_rates ORDER BY weight_kg ASC');
        const [baseMaterials] = await pool.query('SELECT * FROM base_materials');
        const [surfaceFinishes] = await pool.query('SELECT * FROM surface_finishes');
...
const updateShippingRates = asyncHandler(async (req, res) => {
    const {
        ...
        bulk_shipping_rates, baseMaterials, surfaceFinishes, copperWeights, pcbColors
    } = req.body;
    ...
    try {
        ...
        const [check] = await connection.query('SELECT id FROM orderpcbdefaultprice LIMIT 1');
        if (check.length > 0) {
            await connection.query(
                ...
                exchange_rate=?, vat_percent=?, build_time=?, dhl_service_fixed=? 
                WHERE id=?`,
                [base_price, price_per_cm2, extra_service_fee, profit_margin, exchange_rate, vat_percent, build_time, dhl_service_fixed, check[0].id]
            );
        } else {
            await connection.query(
        ...
        const updateTable = async (table, items) => {
            if (items) {
                for (const item of items) {
                    await connection.query(`UPDATE \`${table}\` SET price=? WHERE name=?`, [item.price, item.name]);
                }
            }
        };
...
const getOrderPCBByorderpaymentID = asyncHandler(async (req, res) => {
    const [rows] = await pool.query(`SELECT * FROM \`${tableName}\` WHERE paymentComfirmID = ?`, [req.params.id]);
