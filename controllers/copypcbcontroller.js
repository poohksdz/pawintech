...
const createcopyPCB = asyncHandler(async (req, res) => {
  try {
    ...
    const [cartRows] = await db.execute(`SELECT * FROM pcb_copy_carts WHERE id = ?`, [cartId])
    ...
    if (Number(cart.pcb_qty) < 5) {
      return res.status(400).json({ success: false, message: 'You order amount must not lower than 5 pices' })
    }
    ...
    while (!isUnique) {
      ...
      const [existing] = await db.execute(`SELECT id FROM pcb_copy_orders WHERE orderID = ?`, [orderID])
    ...
    const insertValues = [
      cart.projectname, cart.user_id, cart.pcb_qty, cart.notes || '', cart.copypcb_zip,
      cart.front_image_1, cart.front_image_2, cart.front_image_3, cart.front_image_4, cart.front_image_5,
      cart.front_image_6, cart.front_image_7, cart.front_image_8, cart.front_image_9, cart.front_image_10,
      cart.back_image_1, cart.back_image_2, cart.back_image_3, cart.back_image_4, cart.back_image_5,
      cart.back_image_6, cart.back_image_7, cart.back_image_8, cart.back_image_9, cart.back_image_10,
    ...
    await db.execute(`UPDATE pcb_copy_carts SET status = 'paid' WHERE id = ?`, [cartId])
...
const createcopyPCBbyAdmin = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    const frontCols = Array.from({ length: 10 }, (_, i) => frontImages[i] || '')
    ...
    const columns = [
      'projectname', 'compay_name', 'user_id', 'pcb_qty', 'notes', 'copypcb_zip',
      'front_image_1', 'front_image_2', 'front_image_3', 'front_image_4', 'front_image_5',
      'front_image_6', 'front_image_7', 'front_image_8', 'front_image_9', 'front_image_10',
      'back_image_1', 'back_image_2', 'back_image_3', 'back_image_4', 'back_image_5',
      'back_image_6', 'back_image_7', 'back_image_8', 'back_image_9', 'back_image_10',
      'service_type', 'status', 'confirmed_price', 'confirmed_reason', 'created_at', 'updated_at',
      ...
      'orderID', 'cartId', 'paymentComfirmID', 'userAddress', 'userPhone',
    ...
    const values = [
      ...
      data.transfer?.transferedDate
        ? new Date(data.transfer.transferedDate).toISOString().replace('T', ' ').slice(0, 19)
        : '',
      orderID,
      data.cartId || '',
      paymentComfirmID,
...
const updatecopyPCBById = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    const getFrontImage = (index) => {
      ...
      if (img && typeof img === 'string' && !img.startsWith('/')) return '/' + img;
      return img;
    };
    ...
    const values = [
      projectname, pcbQty, notes, copypcb_zip, confirmed_price,
      ...frontImages,
      ...backImages,
      billingName, billingPhone, billinggAddress, billingCity, billingPostalCode, billingCountry, billingTax,
      shippingName, shippingPhone, shippingAddress, shippingCity, shippingPostalCode, shippingCountry, receivePlace,
      userName, userEmail, updatedAt,
      id,
    ]
...
const updatePaymentcopyPCBById = asyncHandler(async (req, res) => {
  ...
  await db.execute(`UPDATE pcb_copy_orders SET status = ?, updated_at = NOW() WHERE id = ?`, [dbStatus, id]);
...
const deletecopyPCB = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    const [rows] = await db.execute('SELECT paymentSlip, copypcb_zip, front_image_1, front_image_2, front_image_3, front_image_4, front_image_5, front_image_6, front_image_7, front_image_8, front_image_9, front_image_10, back_image_1, back_image_2, back_image_3, back_image_4, back_image_5, back_image_6, back_image_7, back_image_8, back_image_9, back_image_10 FROM pcb_copy_orders WHERE id = ?', [id])
