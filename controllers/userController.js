...
const getUserProfile = asyncHandler(async (req, res) => {
  const [users] = await pool.query('SELECT * FROM users WHERE _id = ?', [req.user._id])
  const user = users[0]
...
const updateUserProfile = asyncHandler(async (req, res) => {
  ...
  if (user) {
    ...
    const shippingData = req.body.shippingAddress && Object.keys(req.body.shippingAddress).length > 0 ? req.body.shippingAddress : parseJSON(user.shippingAddress);
    const billingData = req.body.billingAddress && Object.keys(req.body.billingAddress).length > 0 ? req.body.billingAddress : parseJSON(user.billingAddress);
    ...
    await pool.query(sql, [name, email, password, rawAddress, shippingAddressStr, billingAddressStr, userId])

    // ดึงข้อมูลใหม่หลังจากอัปเดตเสร็จ
    const [updatedUserRows] = await pool.query('SELECT * FROM users WHERE _id = ?', [userId])
    ...
    res.json({
      ...
      isPCBAdmin: updatedUser.isPCBAdmin === 1 || updatedUser.isPCBAdmin === true,
...
const updateUserProfileShipping = asyncHandler(async (req, res) => {
  ...
  const shippingAddressJSON = req.body.shippingAddress ? JSON.stringify(req.body.shippingAddress) : null;
  const billingAddressJSON = req.body.billingAddress ? JSON.stringify(req.body.billingAddress) : null;
  ...
  try {
    ...
    res.json({ message: 'อัปเดตที่อยู่จัดส่งสำเร็จ' })
  } catch (e) {
    res.status(500).json({ message: 'เกิดข้อผิดพลาดในการบันทึกที่อยู่จัดส่ง' })
  }
})
...
const getUserById = asyncHandler(async (req, res) => {
  const [users] = await pool.query('SELECT * FROM users WHERE _id = ?', [req.params.id])
  const user = users[0]
...
const updateUser = asyncHandler(async (req, res) => {
  ...
  await pool.query(sql, [name, email, valIsAdmin, valIsStaff, valIsStore, valIsPCBAdmin, req.params.id])
...
const requestPasswordReset = asyncHandler(async (req, res) => {
  ...
  if (!user) {
    return res.status(200).json({ message: 'หากพบอีเมลในระบบ เราจะส่งลิงก์สำหรับเปลี่ยนรหัสผ่านไปให้' })
  }
  ...
  res.status(200).json({ message: 'หากพบอีเมลในระบบ เราจะส่งลิงก์สำหรับเปลี่ยนรหัสผ่านไปให้' })
})
...
const resetPassword = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    const [users] = await pool.query('SELECT * FROM users WHERE _id = ?', [decoded.id])
    const user = users[0]
    ...
    if (decoded.h !== user.password.substring(0, 10)) {
      return res.status(400).json({ message: 'ลิงก์นี้ถูกใช้งานไปแล้ว หรือไม่สามารถใช้งานได้อีก' })
    }
    ...
    // Update ลง DB
    const userId = user._id || user.id
    await pool.query('UPDATE users SET password = ? WHERE _id = ?', [hashedPassword, userId])
