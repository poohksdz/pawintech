...
const getassemblyCartPCBByDefault = asyncHandler(async (req, res) => {
  try {
    const [rows] = await pool.query('SELECT * FROM pcb_assembly_default_price WHERE id = ?', [1])
    if (rows.length === 0) return res.status(404).json({ success: false, message: 'Error default' })
...
const updateassemblyCartPCBByDefault = asyncHandler(async (req, res) => {
  try {
    const { smd_price, tht_price, stencil_price, setup_price, delivery_price } = req.body
    const sql = `UPDATE pcb_assembly_default_price SET smd_price=?, tht_price=?, stencil_price=?, setup_price=?, delivery_price=? WHERE id=?`
    const [result] = await pool.query(sql, [smd_price, tht_price, stencil_price, setup_price, delivery_price, 1])
    
    if (result.affectedRows === 0) return res.status(404).json({ success: false, message: 'No record found' })
...
const getassemblyCartPCBs = asyncHandler(async (req, res) => {
  try {
    const [rows] = await pool.query('SELECT * FROM pcb_assembly_carts ORDER BY created_at DESC')
...
const getassemblyCartPCBById = asyncHandler(async (req, res) => {
  try {
    const [rows] = await pool.query('SELECT * FROM pcb_assembly_carts WHERE id = ?', [req.params.id])
...
const getassemblyCartPCBByUserId = asyncHandler(async (req, res) => {
  ...
  try {
    const [rows] = await pool.query('SELECT * FROM pcb_assembly_carts WHERE user_id = ? ORDER BY created_at DESC', [user_id])
...
const getassemblyCartPCBByaccepted = asyncHandler(async (req, res) => {
  try {
    const [rows] = await pool.query("SELECT * FROM pcb_assembly_carts WHERE status = 'accepted' ORDER BY created_at DESC")
    res.status(200).json({ success: true, count: rows.length, data: rows })
  } catch (error) {
    res.status(500).json({ success: false, message: error.message })
  }
...
const createassemblyCartPCB = asyncHandler(async (req, res) => {
  try {
    ...
    const {
      projectname, width_mm, high_mm, count_smd, count_tht, board_types,
      total_columns, total_rows, smd_side, tht_side, gerber_zip, notes,
      user_id, userName, userEmail, image_tops, image_bottoms, alreadyHaveStencil,
      total_point_smd, total_point_tht, pcb_qty
    } = orderData

    // Get Default Price
    const [priceRows] = await pool.query('SELECT * FROM pcb_assembly_default_price WHERE id = 1')
    if (priceRows.length === 0) return res.status(500).json({ message: 'Default price not found' })
    const prices = priceRows[0]

    // Calculate Price
    let total_stencil_price = 0
    // เช็ค alreadyHaveStencil ว่าเป็น false หรือ 'false' หรือ 0
    const isNewStencil = alreadyHaveStencil === false || alreadyHaveStencil === 'false' || alreadyHaveStencil === 0;
    
    if (isNewStencil) {
      total_stencil_price = smd_side === 'Both' ? prices.stencil_price * 2 : prices.stencil_price
    }
    ...
    const topImages = [...(Array.isArray(image_tops) ? image_tops : []), ...Array(10).fill(null)].slice(0, 10);
    const bottomImages = [...(Array.isArray(image_bottoms) ? image_bottoms : []), ...Array(10).fill(null)].slice(0, 10);
    ...
    const values = [
      projectname, pcb_qty, 'pending', estimatedCost, null, width_mm, high_mm,
      count_smd, total_point_smd, smdCost, thtCost, vatPrice, count_tht, total_point_tht,
      board_types, total_columns, total_rows, smd_side, total_stencil_price, alreadyHaveStencil,
      tht_side, gerber_zip, notes,
...
const updateassemblyCartPCB = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    const allowed = [
      'projectname', 'pcb_qty', 'status', 'estimatedCost', 'confirmed_price', 'confirmed_reason',
      'width_mm', 'high_mm', 'count_smd', 'total_point_smd', 'count_tht', 'total_point_tht',
      'board_types', 'total_columns', 'total_rows', 'smd_side', 'tht_side', 'gerber_zip', 'notes',
      'stencil_price', 'alreadyHaveStencil', 'reasonUpdated', 'ConfirmAlreadyHaveStencil',
      'image_top_1', 'image_top_2', 'image_top_3', 'image_top_4', 'image_top_5',
      'image_top_6', 'image_top_7', 'image_top_8', 'image_top_9', 'image_top_10',
      'image_bottom_1', 'image_bottom_2', 'image_bottom_3', 'image_bottom_4', 'image_bottom_5',
      'image_bottom_6', 'image_bottom_7', 'image_bottom_8', 'image_bottom_9', 'image_bottom_10'
...
const updateAmountassemblyCartPCBById = asyncHandler(async (req, res) => {
  try {
    await pool.query('UPDATE pcb_assembly_carts SET pcb_qty = 0 WHERE id = ?', [req.params.id])
...
const deleteassemblyCartPCB = asyncHandler(async (req, res) => {
  try {
    await pool.query('DELETE FROM pcb_assembly_carts WHERE id = ?', [req.params.id])
