...
const getFolios = asyncHandler(async (req, res) => {
  try {
    ...
    const [rows] = await pool.query('SELECT *, ID as _id FROM folios ORDER BY ID DESC')
    res.json(rows)
  } catch (error) {
...
const getFolioById = asyncHandler(async (req, res) => {
  const { id } = req.params
  try {
    const [rows] = await pool.query('SELECT *, ID as _id FROM folios WHERE ID = ?', [id])
    if (rows.length > 0) {
      res.json(rows[0])
    } else {
...
const createFolio = asyncHandler(async (req, res) => {
  try {
    ...
    const [newFolio] = await pool.query('SELECT *, ID as _id FROM folios WHERE ID = ?', [result.insertId])
...
const deleteFolio = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    if (rows.length > 0) {
      const folio = rows[0]
      for (let i = 1; i <= 10; i++) {
        const imageKey = i === 1 ? 'imageOne' : `image${['Two', 'Three', 'Four', 'Five', 'Six', 'Seven', 'Eight', 'Nine', 'Ten'][i - 2]}`
