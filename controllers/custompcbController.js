...
const createCustomPCB = asyncHandler(async (req, res) => {
  try {
    ...
    const [cartRows] = await pool.query(
      `SELECT * FROM pcb_custom_carts WHERE id = ?`,
      [cartId],
    );
    ...
    const targetPrice =
      Number(cart.confirmed_price) || Number(cart.estimatedCost) || 0;
    ...
    const insertValues = [
      ...
      cart.notes || "",
      cart.dirgram_zip || "",
      cart.dirgram_image_1 || "",
      cart.dirgram_image_2 || "",
      cart.dirgram_image_3 || "",
      cart.dirgram_image_4 || "",
      cart.dirgram_image_5 || "",
      cart.dirgram_image_6 || "",
      cart.dirgram_image_7 || "",
      cart.dirgram_image_8 || "",
      cart.dirgram_image_9 || "",
      cart.dirgram_image_10 || "",
      targetPrice,
    ...
    await pool.query(
      `UPDATE pcb_custom_carts SET status = 'paid' WHERE id = ?`,
      [cartId],
    );
...
const createCustomPCBbyAdmin = asyncHandler(async (req, res) => {
  try {
    ...
    const insertValues = [
      orderData.projectname || "Admin Order",
      user_id,
      orderData.pcb_qty || 1,
      orderData.notes || "",
      orderData.diagram_zip || null,
      orderData.confirmed_price || 0,
      userName,
      userEmail,
...
const getCustomPCBById = asyncHandler(async (req, res) => {
  try {
    const [rows] = await pool.query(
      "SELECT * FROM pcb_custom_orders WHERE id = ?",
      [req.params.id],
    );
    if (rows.length === 0)
...
const deleteCustomPCB = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    const [rows] = await pool.query(
      "SELECT paymentSlip, dirgram_zip, dirgram_image_1, dirgram_image_2, dirgram_image_3, dirgram_image_4, dirgram_image_5, dirgram_image_6, dirgram_image_7, dirgram_image_8, dirgram_image_9, dirgram_image_10 FROM pcb_custom_orders WHERE id = ?",
      [id],
    );
    ...
    const [result] = await pool.query(
      "DELETE FROM pcb_custom_orders WHERE id = ?",
      [id],
    );
