...
const generateUniquePaymentConfirmID = async () => {
  ...
  while (!isUnique) {
    ...
    const [rows] = await pool.query('SELECT id FROM pcb_copy_carts WHERE paymentComfirmID = ?', [uniqueID])
...
const getcopyCartPCBs = asyncHandler(async (req, res) => {
  try {
    const [rows] = await pool.query('SELECT * FROM pcb_copy_carts ORDER BY created_at DESC')
...
const getcopyCartPCBById = asyncHandler(async (req, res) => {
  try {
    const [rows] = await pool.query('SELECT * FROM pcb_copy_carts WHERE id = ?', [req.params.id])
...
const getcopyCartPCBByUserId = asyncHandler(async (req, res) => {
  ...
  try {
    const [rows] = await pool.query('SELECT * FROM pcb_copy_carts WHERE user_id = ? ORDER BY created_at DESC', [user_id])
...
const createcopyCartPCB = asyncHandler(async (req, res) => {
  ...
  let finalZip = req.body.copypcb_zip || req.body.zipfile || req.body.file || req.body.zip || null;
  ...
  const getImagesArray = (keys, singlePrefix) => {
    ...
    return images.map(img => (img && typeof img === 'object' ? (img.path || img.url) : img));
  };

  const rawFrontImages = getImagesArray(['copypcbFrontImages', 'frontImages', 'images'], 'front_image_');
  const rawBackImages = getImagesArray(['copypcbBackImages', 'backImages'], 'back_image_');
  ...
  try {
    ...
    while (!isUnique) {
      ...
      const [rows] = await pool.query('SELECT id FROM pcb_copy_carts WHERE orderID = ?', [uniqueID])
    ...
    `
    const values = [
      projectname, user_id, pcb_qty, notes || null, finalZip,
      ...fImages, ...bImages, uniqueID
    ]

    const [result] = await pool.query(sql, values)
    res.status(201).json({ success: true, message: 'Order created', orderID: uniqueID, insertedId: result.insertId })
...
const updatecopyCartPCB = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    const values = [
      projectname, pcb_qty, notes, finalZip,
      ...fImages, ...bImages, id
    ];
...
const updateAmountcopyCartPCBById = asyncHandler(async (req, res) => {
  try {
    await pool.query('UPDATE pcb_copy_carts SET pcb_qty = 0 WHERE id = ?', [req.params.id])
...
const updateStatuscopyCartPCBById = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    if (confirmed_price !== undefined) { fields.push('confirmed_price=?'); values.push(confirmed_price); }
    if (confirmed_reason !== undefined) { fields.push('confirmed_reason=?'); values.push(confirmed_reason); }

    if (fields.length === 0) return res.status(400).json({ message: 'No fields to update' })

    values.push(id)
    await pool.query(`UPDATE pcb_copy_carts SET ${fields.join(', ')}, updated_at=NOW() WHERE id=?`, values)
...
const updatePaymentcopyCartPCBById = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    await pool.query(sql, [transferedAmount, transferedName, paymentSlip, transferedDate, paymentID, id])
    res.status(200).json({ success: true, message: 'Payment updated', paymentComfirmID: paymentID })
  } catch (error) {
    res.status(500).json({ success: false, message: error.message })
  }
})
...
const deletecopyCartPCB = asyncHandler(async (req, res) => {
  try {
    await pool.query('DELETE FROM pcb_copy_carts WHERE id = ?', [req.params.id])
