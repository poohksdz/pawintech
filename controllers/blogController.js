...
const createBlog = asyncHandler(async (req, res) => {
  ...
  if (!title || !content || !titleThai || !contentThai || !image) {
    res.status(400)
    throw new Error('Title, content, titleThai, contentThai, and image are required')
  }
...
const updateBlog = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    const query = `
      ...
      WHERE id = ?
    `
    await pool.query(query, [title, content, titleThai, contentThai, image, id])
...
const updateShowFrontBlog = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    await pool.query('UPDATE blogs SET showFront = ? WHERE id = ?', [showFront, id])
